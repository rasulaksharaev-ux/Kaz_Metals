...
async def _get_all_prices():
    """Возвращает (dict {sym: (name, price_kzt, unit)}, usd_kzt).
    XAU/XAG/XPT/XPD — из LBMA в KZT/г, CU — из Yahoo в KZT/кг."""
    prices, usd_kzt = await get_metal_prices()
    if usd_kzt:
        try:
            async with aiohttp.ClientSession() as session:
                _, cu_usd_lb = await _fetch_price_data(session, "HG=F")
                if cu_usd_lb and cu_usd_lb > 0:
                    prices["CU"] = ("🪙 Медь", round(cu_usd_lb * usd_kzt / 0.453592), "кг")
        except Exception:
            pass
    return prices, usd_kzt
...
async def open_prices_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Разовая команда — публикует цены открытия вручную (только admin)."""
    if update.message.chat.type != "private" or not is_admin(str(update.message.from_user.id)):
        return
    await update.message.reply_text("⏳ Публикую цены открытия...")
