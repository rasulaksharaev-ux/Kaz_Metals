...
def _gnews(query: str, gl: str = "RU") -> str:
    """Строит ссылку Google News RSS по читаемому запросу."""
    return (
...
_RSS_FEEDS = [
    ...
    # ── Специализированные финансовые ленты ──
        ...
        for feed_url, feed_domain in _RSS_FEEDS:
            try:
                ...
                for item in root.findall(".//item"):
                    ...
                    # Пропускаем не-русские заголовки (Google News с hl=ru уже на русском)
...
async def _do_fetch_and_post(bot) -> list:
    ...
    for art in articles:
        ...
        lines = [
            f"{cat_emoji} <b>{cat_label}</b> · {date_str}",
            "",
            f"<b>{_esc(art['title'])}</b>",
        ]
        if context:
            lines += ["", _esc(context)]
        lines += ["", f'↗ <a href="{art["url"]}">{_esc(art["domain"])}</a>']
        text = "\n".join(lines)
