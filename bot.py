...
# Разделены на основные (обязательно одно из них) и дополнительные
NEWS_KEYWORDS_PRIMARY = [
    # Металлы на русском
    "золото", "золот", "серебро", "серебр",
    "платина", "платин", "палладий", "палладий",
    ...
    # Монеты
    "монета золот", "монеты золот", "инвестиционн монет",
    # Казахстан специфика
    "нбрк золото", "нацбанк золото",
    # Английские (для kitco, bloomberg, reuters)
    "gold", "silver", "platinum", "palladium",
    "precious metal", "bullion", "jewel", "jewelry", "jewellery",
...
def get_news_emoji(title: str) -> str:
    ...
    t = title.lower()
    if any(w in t for w in ["золото", "gold"]):
        return "🥇"
    if any(w in t for w in ["серебро", "silver"]):
        return "🥈"
    if any(w in t for w in ["платина", "platinum"]):
        return "💎"
    if any(w in t for w in ["палладий", "palladium"]):
        return "🔩"
    if any(w in t for w in ["ювелир", "кольц", "серьг", "брасле", "подвеск", "украшен", "jewel"]):
        return "💍"
    if any(w in t for w in ["слиток", "аффинаж", "bullion"]):
        return "🏅"
    if any(w in t for w in ["монет", "coin"]):
        return "🪙"
...
# RSS-ленты — российские, казахстанские и международные источники
RSS_FEEDS = [
    # ===== КАЗАХСТАН =====
    ...
    ("https://regnum.ru/rss",                                   "regnum.ru"),
    ("https://www.mk.ru/rss/economics/",                        "mk.ru"),
    ("https://iz.ru/xml/rss/all.xml",                           "iz.ru"),

    # ===== МЕЖДУНАРОДНЫЕ — драгоценные металлы =====
    ("https://www.kitco.com/rss/",                              "kitco.com"),
    ("https://www.mining.com/feed/",                            "mining.com"),
    ("https://www.gold-eagle.com/rss.xml",                      "gold-eagle.com"),
    ("https://goldprice.org/rss/gold-news.xml",                 "goldprice.org"),
    ("https://www.silverseek.com/rss.xml",                      "silverseek.com"),
    ("https://www.bullionstar.com/blogs/feed/",                 "bullionstar.com"),

    # ===== МЕЖДУНАРОДНЫЕ — финансы/бизнес =====
    ("https://feeds.bloomberg.com/markets/news.rss",            "bloomberg.com"),
    ("https://feeds.bloomberg.com/economics/news.rss",          "bloomberg.com"),
...
async def fetch_metal_news() -> list:
    """Парсит RSS-ленты и отбирает новости по теме металлов/ювелирки/инвестиций."""
    found = []
    seen_titles = set()
    seen_domains = {}  # домен → количество новостей с него (макс 2 с одного)
    ...
    try:
        async with aiohttp.ClientSession() as session:
            for feed_url, domain in RSS_FEEDS:
                if len(found) >= 8:
                    break
                # Не больше 2 новостей с одного домена
                ...
                try:
                    ...
                    for item in items:
                        ...
                        if any(kw in title_lower for kw in NEWS_KEYWORDS_PRIMARY):
    ...
    logger.info(f"fetch_metal_news: найдено {len(found)} новостей из {len(RSS_FEEDS)} источников")
    return found


async def post_news_to_group(bot) -> None:
    """Публикует новости во вкладку Новости группы.
    Формат точно как на скрине: короткий заголовок + домен источника.
    """
    if not NEWS_TOPIC_ID:
        logger.warning("NEWS_TOPIC_ID не задан — публикация новостей пропущена.")
        return

    articles = await fetch_metal_news()
    if not articles:
        logger.warning("post_news_to_group: подходящих новостей не найдено")
        return

    for article in articles:
        # Формат: эмодзи + заголовок + домен источника
