...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ad_confirm":
        ...
        # Текст для группы — без типа продажи
        city_text = f"📍 {u.get('city','')}" if u.get('city') else ""
        ad_text = (
            f"{u.get('category','')}\n\n"
            f"{u.get('description','')}\n\n"
            f"💰 {u.get('price','')}\n"
        ...
        try:
            ...
            buy_markup = InlineKeyboardMarkup([[
                InlineKeyboardButton("💬 Хочу купить", url=f"https://t.me/KAZ_METALSbot?start=buy_{user_id}_{ad_id}")
            ]])
            if u.get("photo_id"):
