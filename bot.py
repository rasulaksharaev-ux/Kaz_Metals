...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ad_confirm":
        ...
        max_ads = get_max_ads_limit(user_id)
        if row and int(row.get("cnt", 0)) >= max_ads:
            verified_note = " (✅ верифицированным разрешено до 5)" if max_ads > MAX_ADS_DEFAULT else ""
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"⚠️ У вас уже {max_ads} активных объявлений — это максимум{verified_note}.\nУдалите одно в разделе <b>Мой профиль → Мои объявления</b>, затем добавьте новое.",
                parse_mode="HTML",
                reply_markup=main_menu_keyboard(),
            )
            return
    ...
    if data == "profile_ads":
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"📦 <b>Мои объявления</b> ({len(ads)}/{max_ads}):",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(rows),
        )
        return
    ...
    if data == "menu_safety":
        await context.bot.send_message(
            ...
            text=(
                ...
                f"• Лимит активных лотов поднимается с {MAX_ADS_DEFAULT} до {MAX_ADS_VERIFIED}\n"
                    ...
                    try:
                        await context.bot.send_message(
                            ...
                            text=(
                                ...
                                f"Бонус: ваш лимит активных объявлений увеличен с {MAX_ADS_DEFAULT} до {MAX_ADS_VERIFIED}."
                            ),
                        )
                    except Exception as e:
...
async def publish_ad(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: str):
    ...
    try:
        if u.get("photo_id"):
            msg = await context.bot.send_photo(chat_id=GROUP_ID, photo=u.get("photo_id"), caption=text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(buttons), **kwargs)
        else:
            msg = await context.bot.send_message(chat_id=GROUP_ID, text=text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(buttons), **kwargs)
