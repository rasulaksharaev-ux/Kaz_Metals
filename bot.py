...
async def fetch_metal_news() -> str:
    """Fetch top metal news from NewsAPI and return formatted text."""
    try:
        queries = ["gold price", "silver market", "precious metals"]
        articles = []
        async with aiohttp.ClientSession() as session:
            for q in queries:
                url = (
                    ...
                    f"&pageSize=2&apiKey={NEWS_API_KEY}"
                )
                    ...
                    if r.status == 200:
                        ...
                        for a in data.get("articles", [])[:1]:
                            ...
                            if title and url_link:
                                articles.append((title, url_link, source))

        if not articles:
            return ""

        lines = ["📰 Новости рынка металлов\n"]
        for title, url_link, source in articles[:4]:
            # Trim long titles
