...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("buy_") and not data.startswith("buy_confirm_"):
        ...
        # Отправляем подтверждение в личку покупателю
        ...
        await query.answer("Проверьте личные сообщения бота!", show_alert=True)
        return

    if data == "buy_cancel":
        await context.bot.send_message(chat_id=int(user_id), text="❌ Отменено.")
        return
