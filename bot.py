...
_RSS_FEEDS = [
    ...
    # Тема: ювелирные украшения казахстан
    (
        "https://news.google.com/rss/search"
...
def _best_link(item) -> str:
    link = (item.findtext("link") or "").strip()
    # Google News wraps links — берём из <source url="..."> или <link>
    source_el = item.find("source")
    if source_el is not None:
        src_url = source_el.get("url", "").strip()
        if src_url.startswith("http"):
            # source url — это сайт-источник (домен), а не статья
            # для отображения домена подойдёт, но для ссылки берём link
            pass
    ...
    if guid_el is not None:
        ...
        if g.startswith("http") and guid_el.get("isPermaLink", "true").lower() != "false":
            return g
    return link


def _display_domain(url: str, feed_domain: str) -> str:
    """
    Для Google/Yandex News возвращает реальный домен источника из URL статьи,
    а не 'google.news'/'yandex.news'.
    """
    from urllib.parse import urlparse
    try:
        d = urlparse(url).netloc.lower().lstrip("www.")
        if d and d not in ("news.google.com", "news.yandex.ru"):
            return d
    except Exception:
        pass
    return feed_domain
    ...
    async with aiohttp.ClientSession() as session:
        for feed_url, domain in _RSS_FEEDS:
            try:
                ...
                logger.error(f"RSS {domain}: {e}")
    return raw


def _deduplicate(raw: list) -> list:
    """
    Группирует похожие новости → из каждой группы берёт лучший источник
    (KZ > RU-fin > RU-сми). Возвращает топ-8 по score.
    """
    # Фильтруем уже опубликованное (DB)
    ...
    # Из каждой группы — лучший источник (меньший tier = лучше)
    best = []
    for grp in groups:
...
async def _do_fetch_and_post(bot) -> list:
    ...
    async with _NEWS_LOCK:
        ...
        posted = []
        for art in articles:
            domain_display = _display_domain(art["url"], art["domain"])
            text = (
                f"{art['emoji']} {art['title']}\n\n"
                f'<a href="{art["url"]}">{domain_display}</a>'
            )
            try:
...
def main():
    ...
    # Новости: 9:00 и 17:00 — БЕЗ run_once (он вызывал двойной постинг при старте)
    if NEWS_TOPIC_ID and GROUP_ID and app.job_queue is not None:
        ...
        logger.info(f"Автопубликация новостей: 9:00 и 17:00. Topic ID: {NEWS_TOPIC_ID}")
    else:
        logger.warning("Автопубликация новостей отключена.")
