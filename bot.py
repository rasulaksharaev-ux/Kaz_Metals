...
NEWS_TOPIC_ID = optional_int_env("NEWS_TOPIC_ID", 12)  # ID вкладки "Новости" в группе
...
# ── Приоритет источников: Tier 1 = Казахстан, Tier 2 = RU финансы, Tier 3 = RU СМИ ──
SOURCE_TIER = {
    # Казахстан — оригинальные источники
    "nationalbank.kz": 1, "kapital.kz": 1, "kursiv.kz": 1,
    "tengrinews.kz": 1, "inform.kz": 1, "zakon.kz": 1,
    "forbes.kz": 1, "bizmedia.kz": 1, "finprom.kz": 1,
    # Россия финансы/металлы
    "profinance.ru": 2, "1prime.ru": 2, "finmarket.ru": 2,
    "finanz.ru": 2, "gold.ru": 2, "metalinfo.ru": 2,
    # Россия крупные СМИ
    "rbc.ru": 3, "vedomosti.ru": 3, "kommersant.ru": 3,
    "interfax.ru": 3, "ria.ru": 3, "tass.ru": 3,
    "forbes.ru": 3, "expert.ru": 3, "banki.ru": 3,
    "mk.ru": 3, "iz.ru": 3, "investing.com": 3,
}

# ── RSS: Казахстан в самом начале ────────────────────────────────────────────
RSS_FEEDS = [
    # КАЗАХСТАН — высший приоритет (включая НацБанк РК)
    ("https://nationalbank.kz/rss/",                   "nationalbank.kz"),
    ("https://kapital.kz/rss.xml",                     "kapital.kz"),
    ("https://kursiv.kz/rss.xml",                      "kursiv.kz"),
    ("https://tengrinews.kz/rss/",                     "tengrinews.kz"),
    ("https://www.inform.kz/rss/",                     "inform.kz"),
    ("https://www.zakon.kz/rss/",                      "zakon.kz"),
    ("https://forbes.kz/rss/",                         "forbes.kz"),
    ("https://bizmedia.kz/feed/",                      "bizmedia.kz"),
    ("https://finprom.kz/rss",                         "finprom.kz"),
    # РОССИЯ — финансы/металлы
    ("https://www.profinance.ru/rss/news.xml",         "profinance.ru"),
    ("https://1prime.ru/rss/news/index.rss",           "1prime.ru"),
    ("https://www.finmarket.ru/rss/",                  "finmarket.ru"),
    ("https://www.finanz.ru/rss/novosti",              "finanz.ru"),
    ("https://gold.ru/rss/news.xml",                   "gold.ru"),
    ("https://www.metalinfo.ru/ru/rss/news.xml",       "metalinfo.ru"),
    # РОССИЯ — крупные СМИ
    ("https://www.rbc.ru/rss/news",                    "rbc.ru"),
    ("https://www.vedomosti.ru/rss/articles.xml",      "vedomosti.ru"),
    ("https://www.kommersant.ru/RSS/news.xml",         "kommersant.ru"),
    ("https://www.interfax.ru/rss.asp",                "interfax.ru"),
    ("https://ria.ru/export/rss2/index.xml",           "ria.ru"),
    ("https://tass.ru/rss/v2.xml",                     "tass.ru"),
    ("https://www.forbes.ru/rss",                      "forbes.ru"),
    ("https://expert.ru/rss/",                         "expert.ru"),
    ("https://www.banki.ru/xml/news.rss",              "banki.ru"),
    ("https://www.mk.ru/rss/economics/",               "mk.ru"),
    ("https://iz.ru/xml/rss/all.xml",                  "iz.ru"),
    ("https://ru.investing.com/rss/news_14.rss",       "investing.com"),
    ("https://ru.investing.com/rss/news_285.rss",      "investing.com"),
]

# ── Пулы эмодзи по категории (ротация чтобы не повторялись) ─────────────────
_EMOJI_COUNTERS: dict = {}
...
def get_news_emoji(title: str, index: int) -> str:
    t = title.lower()
    if any(w in t for w in ["золото", "золот"]):
        cat = "gold"
    elif any(w in t for w in ["серебро", "серебр"]):
        cat = "silver"
    elif any(w in t for w in ["платина", "платин"]):
        cat = "platinum"
    elif any(w in t for w in ["палладий"]):
        cat = "palladium"
    elif any(w in t for w in ["ювелир", "кольц", "серьг", "брасле", "подвеск", "украшен"]):
        cat = "jewelry"
    elif any(w in t for w in ["слиток", "аффинаж"]):
        cat = "bullion"
    elif any(w in t for w in ["монет"]):
        cat = "coin"
    elif any(w in t for w in ["лом", "скупка"]):
        cat = "scrap"
    elif any(w in t for w in ["нбрк", "нацбанк", "казахстан", "астана", "алматы"]):
        cat = "kz"
    elif any(w in t for w in ["цена", "курс", "котировк", "биржа", "рынок", "инвестиц"]):
        cat = "price"
    else:
        cat = "default"
...
def title_words(title: str) -> set:
    import re as _re
    stopwords = {
        "что", "это", "как", "для", "при", "будет", "после", "также",
        "свои", "своих", "этом", "млрд", "млн", "трлн", "году", "года",
        "рублей", "рублях", "тысяч", "более", "менее", "новый", "новые",
    }
    words = _re.findall(r'[а-яёa-z]{4,}', title.lower())
    return {w for w in words if w not in stopwords}


def is_similar_title(t1: str, t2: str, threshold: float = 0.42) -> bool:
...
def get_published_titles() -> list:
    try:
        rows = db_fetchall(
            "SELECT title FROM news_published WHERE published_at > NOW() - INTERVAL '7 days'"
        )
        return [r["title"] for r in rows if r.get("title")]
    except Exception as e:
        logger.error(f"get_published_titles error: {e}")
        return []


def save_published_news(title: str, domain: str) -> None:
    import hashlib
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO news_published (title_hash, title, domain) VALUES (%s, %s, %s) "
        ...
        logger.error(f"save_published_news error: {e}")


def parse_pub_date(date_str: str):
    """Парсит дату публикации из RSS (RFC 2822)."""
    try:
...
def is_recent(date_str: str, days: int = 3) -> bool:
    ...
    if dt is None:
        return True  # если дату не распознали — пропускаем, лучше взять
    now = datetime.now(timezone.utc)
    return (now - dt) <= timedelta(days=days)


def score_article(title: str) -> int:
    """Оценивает важность новости. Чем выше — тем интереснее аудитории."""
    t = title.lower()
    score = 0
    # Казахстан — максимальный приоритет
    if any(w in t for w in ["казахстан", "казахск", "нбрк", "нацбанк", "астана", "алматы"]):
        score += 120
    # Рекорды и движение цен
    if any(w in t for w in ["рекорд", "максимум", "минимум", "рост", "падение",
                              "обвал", "скачок", "впервые", "исторический"]):
        score += 80
    # Цены и рынок
    if any(w in t for w in ["цена", "курс", "котировк", "стоимость"]):
        score += 50
    # Крупные экономики
    if any(w in t for w in ["сша", "китай", "европа", "евросоюз", "мировой", "глобальн"]):
        score += 40
    # Инвестиции и резервы
    if any(w in t for w in ["инвестиц", "резервы", "запасы", "биржа", "рынок"]):
        score += 30
    # Ювелирка
    if any(w in t for w in ["ювелир", "кольц", "серьг", "брасле", "украшен"]):
        score += 20
    return score


def get_best_link(item) -> str:
    """Извлекает лучший URL из RSS-элемента (link или guid)."""
    # Сначала пробуем <link>
    link = (item.findtext("link") or "").strip()
    if link and link.startswith("http"):
        return link
    # Потом <guid isPermaLink="true">
    guid_el = item.find("guid")
    if guid_el is not None:
        guid = (guid_el.text or "").strip()
        is_permalink = (guid_el.get("isPermaLink", "true").lower() != "false")
        if guid and guid.startswith("http") and is_permalink:
            return guid
    return link


async def fetch_metal_news() -> list:
    """
    Парсит все RSS-ленты, собирает ВСЕ подходящие статьи,
    затем для каждой группы похожих новостей оставляет ЛУЧШИЙ источник
    (Казахстан > RU финансы > RU СМИ), сортирует по важности.
    """
    raw: list = []          # все кандидаты до группировки
    seen_titles: set = set()
    published_titles = get_published_titles()

    try:
        async with aiohttp.ClientSession() as session:
            for feed_url, domain in RSS_FEEDS:
                try:
                    async with session.get(
                        feed_url,
                    ...
                    for item in root.findall(".//item"):
                        ...
                        link     = get_best_link(item)

                        if not title or not link:
                            continue
                        if not is_recent(pub_date, days=3):
                            continue
                        if not is_russian(title):
                            continue

                        tl = title.lower()
                        if any(sw in tl for sw in NEWS_STOP_WORDS):
                            continue
                        if not any(kw in tl for kw in NEWS_KEYWORDS_PRIMARY):
                            continue
                        if title in seen_titles:
                            continue

                        # пропускаем уже опубликованное
                        if any(is_similar_title(title, p, threshold=0.60)
                    ...
                    logger.error(f"RSS error {domain}: {e}")

    except Exception as e:
        logger.error(f"fetch_metal_news error: {e}")

    # ── Группируем похожие новости → оставляем лучший источник ──────────────
    groups: list = []   # каждый элемент — список похожих статей
    for art in raw:
        placed = False
        for grp in groups:
            if is_similar_title(art["title"], grp[0]["title"], threshold=0.50):
                grp.append(art)
                placed = True
                break
        if not placed:
    ...
    # Из каждой группы берём статью с наименьшим tier (= лучший источник)
    best: list = []
    for grp in groups:
        grp.sort(key=lambda x: (x["tier"], -x["score"]))
        best.append(grp[0])

    # Сортируем по важности, топ-8
    best.sort(key=lambda x: x["score"], reverse=True)
    found = best[:8]

    # Добавляем эмодзи после сортировки
    for i, art in enumerate(found):
        art["emoji"] = get_news_emoji(art["title"], i)

    logger.info(
        f"fetch_metal_news: {len(raw)} кандидатов → "
        f"{len(groups)} групп → {len(found)} финальных"
    )
    return found


async def post_news_to_group(bot, articles: list) -> None:
    """Публикует список новостей. Ссылка кликабельна через HTML."""
    if not NEWS_TOPIC_ID:
        logger.warning("NEWS_TOPIC_ID не задан.")
        return
    for article in articles:
        from urllib.parse import urlparse
        domain = urlparse(article["url"]).netloc.lstrip("www.")
        # Формат: эмодзи + заголовок + кликабельная ссылка с доменом
        text = (
            f"{article['emoji']} {article['title']}\n\n"
            f'<a href="{article["url"]}">{domain}</a>'
        )
        try:
            ...
            save_published_news(article["title"], article["domain"])
        except Exception as e:
            logger.error(f"post_news_to_group error: {e}")


async def daily_news_job(context: ContextTypes.DEFAULT_TYPE):
    articles = await fetch_metal_news()
    await post_news_to_group(context.bot, articles)


async def publish_news_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /publish_news — ручная публикация (только для админа)."""
    if update.message.chat.type != "private":
        return
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
        return
    await update.message.reply_text("⏳ Ищу свежие новости за последние 3 дня...")
    articles = await fetch_metal_news()
    if not articles:
        published = get_published_titles()
        await update.message.reply_text(
            f"⚠️ Свежих новостей не найдено.\n\n"
            f"📋 В базе уже опубликовано: {len(published)} новостей\n\n"
            f"Возможные причины:\n"
            f"• Все подходящие новости за 3 дня уже публиковались\n"
            f"• RSS-ленты ещё не обновились\n\n"
            f"👉 Используй /reset_news чтобы сбросить историю и опубликовать заново"
        )
        return
    await post_news_to_group(context.bot, articles)
    lines = "\n".join([
        f"{a['emoji']} [{a['domain']}] {a['title'][:50]}..."
        for a in articles
    ])
    await update.message.reply_text(
        f"✅ Опубликовано {len(articles)} новостей:\n\n{lines}"
    )


async def reset_news_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /reset_news — сбрасывает историю публикаций (только для админа)."""
    if update.message.chat.type != "private":
        return
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
    ...
    try:
        ...
        conn.close()
        await update.message.reply_text(
            "✅ История новостей очищена.\n"
            "Теперь /publish_news опубликует свежие статьи заново."
        )
    except Exception as e:
        await update.message.reply_text(f"⚠️ Ошибка: {e}")
...
def main():
    ...
    # Автопубликация новостей во вкладку Новости группы (каждый день в 9:00 и 17:00)
    if NEWS_TOPIC_ID and GROUP_ID and app.job_queue is not None:
        app.job_queue.run_once(daily_news_job, when=30)  # первый запуск через 30 сек после старта
        app.job_queue.run_daily(daily_news_job, time=dt_time(hour=9, minute=0))
        app.job_queue.run_daily(daily_news_job, time=dt_time(hour=17, minute=0))
        logger.info(f"Автопубликация новостей включена. Topic ID: {NEWS_TOPIC_ID}")
    else:
        logger.warning("Автопубликация новостей отключена: проверьте NEWS_TOPIC_ID и GROUP_ID.")
