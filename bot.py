            ...
            try:
                ...
                for item in root.findall(".//item"):
                    ...
                    if h in seen or h in _SENT_HASHES:
                        continue
                    # Дубли проверяем только через _SENT_HASHES (в памяти)
                    seen.add(h)
                    raw.append({
                        "title":  title,
                        "desc":   desc,
...
async def _groq_filter(articles: list) -> list:
    ...
    prompt = (
        ...
        "Выбери 5-8 ЛУЧШИХ и УНИКАЛЬНЫХ заголовков.\n\n"
        "ПРАВИЛА:\n"
        "1. ДУБЛИ: одна история с разных сайтов - оставь ОДИН лучший вариант\n"
        "2. ПРИОРИТЕТ: Казахстан > США/Европа > Россия\n"
        "3. ПРИОРИТЕТ тем: цены/рекорды > ювелирка/часы/камни > общее\n"
        "4. УДАЛИ: кражи, криминал, таможня, спорт, советы 'как купить'\n"
        "5. ОСТАВЬ необычное если это реальное рыночное событие\n\n"
        f"Заголовки:\n{titles_text}\n\n"
        "Ответь ТОЛЬКО JSON-массивом номеров. Пример: [1,5,9,12]\n"
        "Если ни один не подходит - ответь []. Никаких пояснений."
    )
...
async def _do_fetch_and_post(bot) -> list:
    ...
    try:
        async with _asyncio.wait_for(_NEWS_LOCK.acquire(), timeout=30):
            pass
    except _asyncio.TimeoutError:
        logger.warning("_do_fetch_and_post: не удалось получить Lock за 30 сек")
        return []
...
def main():
    ...
    if NEWS_TOPIC_ID and GROUP_ID and app.job_queue is not None:
        app.job_queue.run_repeating(daily_news_job, interval=3600, first=60)
