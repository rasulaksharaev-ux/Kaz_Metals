...
_KW_STOP = [
    ...
    # Таможня / провоз
    "таможн", "досмотр", "провезти", "перевез", "перевозил",
    "изъяли", "конфисков", "штанах", "джинсах", "багаже",
    "шереметьево", "домодедово", "аэропорт",
    # Военное / политика
    "ракет", "военн", "обстрел", "атак", "войск",
]

# Стоп-фразы (проверяются целиком)
_STOP_PHRASES = [
    "золотой купол", "железный купол",
    "как купить золото", "как инвестировать",
    "инвестировать или нет", "стоит ли покупать",
    "стоит ли инвестировать", "почему вечная ценность",
    "как россиянам купить", "как казахстанцам купить",
    "нашли в штанах", "нашли в багаже", "нашли в джинсах",
    "пакетом бриллиантов", "с бриллиантами в",
]
        ...
        for feed_url, feed_domain in _RSS_FEEDS:
            try:
                ...
                for item in root.findall(".//item"):
                    ...
                    tl = title.lower()
                    # Стоп: (фото)/(видео) в конце — человеческий интерес, не рынок
                    if _re.search(r'\(фото\)|\(видео\)|\(фотогалерея\)', tl):
                        continue
