...
async def deviation_alert_job(context: ContextTypes.DEFAULT_TYPE):
    """Каждые 5 минут — мониторинг. Алерт если цена изменилась ≥0.3% от прошлой проверки."""
    global _PREV_CHECK_PRICES, _ALERTED_TODAY
    if not NEWS_TOPIC_ID or not GROUP_ID:
        return
    try:
        prices, usd_kzt = await _get_all_prices()
    ...
    if not prices:
        return
    # При первом запуске — сохраняем базу, не постим
    if not _PREV_CHECK_PRICES:
        _PREV_CHECK_PRICES = {sym: data[1] for sym, data in prices.items()}
        logger.info("deviation_alert: base prices set")
        return
    for sym, (name, price, open_kzt, chp, unit) in prices.items():
        prev = _PREV_CHECK_PRICES.get(sym)
        if not prev:
            _PREV_CHECK_PRICES[sym] = price
            continue
        ...
        try:
            ...
            async def _clear(k=key):
                await _asyncio.sleep(1800)
                _ALERTED_TODAY.discard(k)
            _asyncio.create_task(_clear())
            logger.info(f"alert: {name} {sign}{chg:.2f}%")
        except Exception as e:
...
def main():
    ...
    if GROUP_ID and app.job_queue is not None:
        ...
        app.job_queue.run_repeating(deviation_alert_job, interval=300, first=60)
