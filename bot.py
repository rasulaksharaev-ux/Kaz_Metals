...
async def force_alert_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Принудительно публикует текущие цены как алерт в вкладку."""
    if update.message.chat.type != "private" or not is_admin(str(update.message.from_user.id)):
        return
    try:
        prices, usd_kzt = await get_metal_prices()
    except Exception as e:
        await update.message.reply_text(f"⚠️ Ошибка получения цен: {e}")
        return
    if not prices:
        await update.message.reply_text("⚠️ Не удалось получить цены.")
        return
    lines = []
    for symbol, (name, price_kzt, unit) in prices.items():
        lines.append(f"{name}: <b>{price_kzt:,} ₸</b> / г")
    text = (
        "📊 <b>Актуальные цены на металлы</b>\n\n"
        + "\n".join(lines)
        + f"\n\n💱 USD/KZT: {usd_kzt:,}\n\n"
        + "Цены обновляются автоматически → @KAZ_METALSBOT"
    )
    try:
        await context.bot.send_message(
            chat_id=GROUP_ID,
...
def main():
    ...
    app.add_handler(CommandHandler("force_alert", force_alert_cmd))
