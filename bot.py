...
async def deviation_alert_job(context: ContextTypes.DEFAULT_TYPE):
    """Каждые 30 минут — алерт если chp (изменение за день) ≥1%."""
    global _ALERTED_TODAY, _ALERTED_TODAY_DATE
    if not NEWS_TOPIC_ID or not GROUP_ID:
        return
    # Ежедневный сброс по UTC-дате (не зависит от других джобов)
    ...
    try:
        ...
        logger.error(f"deviation_alert: {e}"); return
    if not prices:
        return
    for sym, (name, price, open_kzt, chp, unit) in prices.items():
        ...
        try:
            ...
            logger.info(f"alert: {name} {sign}{chg:.1f}%")
        except Exception as e:
            logger.error(f"deviation send: {e}")
