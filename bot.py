...
async def open_prices_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Публикует цены в нужном формате по времени Алматы (UTC+5)."""
    ...
    if almaty_hour < 12:
        label = "утренние цены"
        job = morning_prices_job
    elif almaty_hour < 18:
        label = "цены полудня"
        job = midday_prices_job
    else:
        label = "итог дня"
        job = evening_prices_job
    await update.message.reply_text(f"⏳ Публикую {label}...")
    await job(context)
