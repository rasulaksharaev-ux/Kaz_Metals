...
async def fetch_metal_news() -> str:
    """Fetch metal news from public Telegram channels via Bot API."""
    channels = [
        "ruinvestingcom",      # Investing.com на русском — пишут про золото каждый день
        "vestifinance_ru",     # Вести Финанс — финансовые новости
        "commodityinside",     # Commodity Inside — анализ цен на металлы
        "preciousmetals",      # Precious Metals — драгоценные металлы
        "rbc_news",            # РБК Новости — бизнес и экономика
        "bloomberg4you",       # Bloomberg на русском
    ]
    keywords = ["золото", "серебро", "платина", "палладий", "металл", "gold", "silver", "metal"]
    found = []
    ...
    try:
        async with aiohttp.ClientSession() as session:
            for channel in channels:
                ...
                # Use public channel preview via t.me/s/
                preview_url = f"https://t.me/s/{channel}"
                try:
                    headers = {"User-Agent": "Mozilla/5.0"}
                    async with session.get(preview_url, headers=headers,
                    ...
                    logger.error(f"Channel {channel} fetch error: {e}")
                if len(found) >= 3:
                    break

        if not found:
            logger.warning("No metal news found from channels")
            return ""
