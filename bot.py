...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("calc_") and len(data) == 8:
        ...
        states[user_id] = {"step": "calc_weight", "calc_metal": metal}
        await context.bot.send_message(
            chat_id=chat_id,
    ...
    if step == "calc_weight":
        try:
            ...
            if metal in prices:
                ...
                total = round(weight * price_per_gram)
                await update.message.reply_text(
                    f"🧮 Результат:\n\n"
                    f"{metal_names.get(metal, metal)}\n"
                    f"⚖️ Вес: {weight} г\n"
                    f"💰 Цена за грамм: {price_per_gram:,} KZT\n"
                    f"💵 Итого: {total:,} KZT\n\n"
                    f"📅 Курс USD/KZT: {usd_kzt}",
                    reply_markup=InlineKeyboardMarkup([
                        [InlineKeyboardButton("🔄 Другой вес", callback_data=f"calc_{metal}")],
                        [InlineKeyboardButton("🔙 Главное меню", callback_data="back_menu")]
                    ])
                )
            else:
