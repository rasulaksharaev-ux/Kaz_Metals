...
async def pin_discussions_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
        return
    # Принимаем topic_id из аргумента команды или из env-переменной
    args = context.args
    topic_id = int(args[0]) if args and args[0].lstrip("-").isdigit() else DISCUSSIONS_TOPIC_ID
    if not topic_id or not GROUP_ID:
        await update.message.reply_text(
            "⚠️ Укажи ID топика: /pin_discussions 1"
        )
        return
    text = (
        "💬 <b>Обсуждения</b>\n"
