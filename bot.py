...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "profile_ads":
        ...
        if not ads:
            ...
            return
        await context.bot.send_message(chat_id=chat_id, text=f"📦 <b>Мои объявления</b> ({len(ads)}/3 активных):", parse_mode="HTML")
        for ad in ads:
            ...
            kb = InlineKeyboardMarkup([
                [InlineKeyboardButton("🗑 Удалить", callback_data=f"delete_ad_{ad_id_str}"),
                 InlineKeyboardButton("✅ Продано", callback_data=f"mark_sold_{ad_id_str}")],
            ])
            try:
                if ad.get("photo_id"):
                ...
                logger.error(f"my_ads display: {e}")
        await context.bot.send_message(chat_id=chat_id, text="─────", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Главное меню", callback_data="back_menu")]]))
        return
