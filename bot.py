...
async def fetch_metal_news() -> list:
    """Получает новости о металлах через NewsAPI. Возвращает список словарей."""
    # Пробуем русскоязычные новости, затем английские
    queries = [
        ("золото серебро металлы инвестиции", "ru"),
        ("gold silver platinum precious metals", "en"),
    ]
    found = []
    try:
        async with aiohttp.ClientSession() as session:
                    ...
                    async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as r:
                        if r.status == 200:
                            ...
                            for a in articles:
                                ...
                                # Убираем суффикс источника из заголовка типа "Заголовок - Источник"
                                if " - " in title:
                                    title = title.rsplit(" - ", 1)[0].strip()
                                description = (a.get("description") or "").strip()
                                article_url = (a.get("url") or "").strip()
                                source_name = (a.get("source") or {}).get("name") or ""
                                if title and article_url and "[Removed]" not in title:
                                    found.append({
                                        "title": title,
                                        "description": description[:280] if description else "",
                                        "url": article_url,
                                        "source": source_name,
                                    })
                        else:
                            logger.error(f"NewsAPI status {r.status} for lang={lang}")
                except Exception as e:
                    logger.error(f"NewsAPI fetch error ({lang}): {e}")
    except Exception as e:
        logger.error(f"fetch_metal_news session error: {e}")

    if not found:
        logger.warning("fetch_metal_news: новостей не найдено")
    return found


async def post_news_to_group(bot) -> None:
    """Публикует свежие новости о металлах во вкладку Новости группы."""
    ...
    if not articles:
        logger.warning("post_news_to_group: нет новостей для публикации")
        return
    ...
    for article in articles:
        ...
        # Формат как на скрине: заголовок жирным, краткое описание, ссылка на источник
        if description:
            text = (
                f"📰 <b>{title}</b>\n\n"
                f"{description}\n\n"
...
async def publish_news_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(user_id):
        ...
        return
    await update.message.reply_text("⏳ Ищу новости и публикую...")
    await post_news_to_group(context.bot)
    await update.message.reply_text("✅ Новости опубликованы во вкладку Новости!")
