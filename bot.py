...
async def get_realtime_prices():
    """Котировки с Stooq (бесплатно, без ключа, все 4 металла одним CSV-запросом).
    ...
    # Маппинг наших символов → символы Stooq
    stooq_symbols = {
        "XAU": "xauusd",
        "XAG": "xagusd",
        "XPT": "xptusd",
    ...
    async with aiohttp.ClientSession() as session:
        ...
        if not usd_kzt:
            ...
            return results, 0
        # Stooq batch CSV: все 4 металла за один запрос
        # Формат: Symbol,Date,Time,Open,High,Low,Close,Volume
        symbols_csv = ",".join(stooq_symbols.values())
        url = f"https://stooq.com/q/l/?s={symbols_csv}&f=sd2t2ohlcv&h&e=csv"
        try:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as r:
                if r.status != 200:
                    logger.error(f"stooq HTTP {r.status}")
                    return results, round(usd_kzt)
                text = await r.text()
        except Exception as e:
            logger.error(f"stooq request error: {e}")
            return results, round(usd_kzt)
        # Парсим CSV
        try:
            import csv as _csv
            ...
            rows_by_symbol = {}
            for row in reader:
                sym_raw = (row.get("Symbol") or "").upper()  # например XAUUSD
                rows_by_symbol[sym_raw] = row
        except Exception as e:
            logger.error(f"stooq CSV parse error: {e}; raw: {text[:300]}")
            return results, round(usd_kzt)
        for sym, name in metal_names.items():
            stooq_sym = stooq_symbols[sym].upper()  # XAUUSD
            row = rows_by_symbol.get(stooq_sym)
            if not row:
                logger.warning(f"stooq: нет данных для {stooq_sym}")
                continue
            try:
                # Stooq может вернуть "N/D" если рынок закрыт или нет данных
                open_str = row.get("Open", "")
                close_str = row.get("Close", "")
                if not close_str or close_str == "N/D":
                    logger.warning(f"stooq: цена N/D для {stooq_sym} (рынок закрыт?)")
                    continue
                price_usd = float(close_str)
                open_usd = float(open_str) if open_str and open_str != "N/D" else 0
                if not price_usd:
                    continue
                price_kzt_g = round((price_usd / 31.1035) * usd_kzt)
                open_kzt_g = round((open_usd / 31.1035) * usd_kzt) if open_usd else price_kzt_g
                # chp считаем сами из open/close — Stooq не отдаёт готовое поле
                chp = None
                if open_usd and price_usd:
                    chp = ((price_usd - open_usd) / open_usd) * 100
                results[sym] = (name, price_kzt_g, open_kzt_g, chp, "г")
            except Exception as e:
                logger.error(f"stooq parse {stooq_sym}: {e}")
    if results:
...
async def test_alert_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сначала — отдельно дёргаем Stooq напрямую, чтобы показать сырой ответ
    diag_lines = ["🔧 <b>Диагностика источника цен (Stooq)</b>\n"]
    test_url = "https://stooq.com/q/l/?s=xauusd,xagusd,xptusd,xpdusd&f=sd2t2ohlcv&h&e=csv"
    try:
        async with aiohttp.ClientSession() as session:
