...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    in_group = await is_group_member(context.bot, user_id)

    if in_group and user and user.get("status") == "active":
    ...
    elif in_group and not user:
        # В группе но нет в базе — просим зарегистрироваться через бота
        states[user_id] = {"step": "start"}
        keyboard = InlineKeyboardMarkup([
            [InlineKeyboardButton("🚀 Зарегистрироваться", callback_data="show_rules")]
        ])
        await update.message.reply_text(
            "👋 Вы уже в группе KAZMETALS!\n\n"
            "Но нам нужны ваши данные для полного доступа.\n"
