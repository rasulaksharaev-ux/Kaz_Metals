...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and user.get("status") == "reset":
        ...
        await update.message.reply_text(
            ...
            "драгоценных металлов в Казахстане.",
            reply_markup=InlineKeyboardMarkup([
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "about":
        await context.bot.send_message(
            chat_id=chat_id,
            text="ℹ️ KAZ_METALS — доверенное сообщество участников\n"
                 "рынка драгоценных металлов Казахстана.\n\n"
                 "Здесь вы можете:\n"
    ...
    if data == "reg_start":
        ...
        if not username:
            await context.bot.send_message(
                ...
                text="❌ У вас нет username в Telegram!\n\n"
                     ...
                     "5. Вернитесь и нажмите Зарегистрироваться",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("✅ Зарегистрироваться", callback_data="reg_start")]
                ])
            )
        ...
        await context.bot.send_message(
            ...
            text="🎉 Регистрация завершена!\n\n"
                 "Последний шаг — вступите в группу KAZ_METALS 👇",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔓 Вступить в KAZ_METALS", url=GROUP_LINK)],
                [InlineKeyboardButton("✅ Я вступил — проверить", callback_data="check_join")],
            ])
            ...
            try:
                await context.bot.send_message(
                    ...
                    text="🔄 Ваша регистрация была сброшена администратором.\n\n"
                         "Пожалуйста пройдите регистрацию заново — напишите /start"
                )
            except Exception:
                pass
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"✅ Регистрация @{uname} сброшена! Пользователь уведомлён.",
