...
_AD_PRICE_RE = re.compile(r"\b\d{1,3}(?:[\s.,]\d{3})+\b|\b\d{5,}\b")
_AD_SELL_RE = re.compile(
    r"\b(продаю|продам|продаётся|продается|обмен|обменяю|торг|торгуюсь|"
    r"цена|стоит|стоимость|отдам|срочно прода|купи у меня|куплю|скуплю|выкуп)\b",
    re.IGNORECASE,
)
_AD_CURRENCY_RE = re.compile(r"(₸|\bтг\b|тенге|kzt|\bтыс\.|\bмлн\.)", re.IGNORECASE)


def _looks_like_ad(text: str, has_media: bool) -> bool:
    """Эвристика: похоже ли сообщение в Обсуждениях на объявление о продаже."""
    text_lower = (text or "").lower()
    has_sell_word = bool(_AD_SELL_RE.search(text_lower))
    has_price_num = bool(_AD_PRICE_RE.search(text_lower))
    has_currency = bool(_AD_CURRENCY_RE.search(text_lower))
    # фото/видео + (слова продажи ИЛИ упоминание цены/валюты) = объявление
    if has_media and (has_sell_word or has_price_num or has_currency):
        return True
    # текст без медиа: нужны и слова продажи, И сумма
    if not has_media and has_sell_word and (has_price_num or has_currency):
        return True
    return False
