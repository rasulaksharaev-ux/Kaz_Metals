...
async def get_realtime_prices():
    """Реальные котировки с goldapi.io.
    Возвращает ({sym: (name, price_kzt, chp, unit)}, usd_kzt).
    chp = % изменения от вчерашнего закрытия (из goldapi напрямую)."""
    metal_names = {
        "XAU": "🥇 Золото",
        "XAG": "🥈 Серебро",
        "XPT": "💎 Платина",
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for sym, name in metal_names.items():
            try:
                ...
                chp = data.get("chp")  # % от вчерашнего закрытия
                if not price_usd:
                    continue
                price_kzt_g = round((price_usd / 31.1035) * usd_kzt)
                results[sym] = (name, price_kzt_g, chp, "г")
            except Exception as e:
...
async def midday_prices_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym in order:
        ...
        name, price, chp, unit = prices[sym]
...
async def deviation_alert_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym, (name, price, chp, unit) in prices.items():
        if chp is None:
            continue
        chg = float(chp)
