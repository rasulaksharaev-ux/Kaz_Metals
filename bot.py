...
_RSS_FEEDS = [
    ...
    # ── Цены и рынок: KZ + мировой контекст (на русском) ──
...
def _deduplicate(raw: list, recent_titles: Optional[list] = None) -> list:
    ...
    for art in raw:
        if art["hash"] in seen_hashes:
            continue
        if any(_same_topic(art["title"], a["title"]) for a in accepted):
            continue
        if any(_same_topic(art["title"], t) for t in recent_titles):
            continue
...
def _load_published_into_cache(hours: int = 48) -> list:
...
async def _do_fetch_and_post(bot) -> list:
    """Сбор → дедуп (точный + смысловой + история) → Groq → баланс → пост."""
    recent_titles = _load_published_into_cache(hours=48)
    ...
    for art in articles:
        ...
        cat_emoji, cat_label = _CAT_LABELS.get(
            art.get("cat", "market"), ("📊", "Рынок металлов")
        )
        context = _clean_context(art.get("desc", ""), art["title"])

        lines = [
            f"▎ <b>{cat_label.upper()}</b>",
            "",
            f"<b>{_esc(art['title'])}</b>",
        ]
        if context:
            lines += ["", f"<i>{_esc(context)}</i>"]
        lines += [
            "",
            "─ ─ ─ ─ ─",
            f"🔗 <a href=\"{art['url']}\">{_esc(art['domain'])}</a>  ·  {date_str}",
        ]
        text = "\n".join(lines)
        ...
        try:
            ...
            # Сохраняем ТОЛЬКО после успешной отправки — и в память, и в БД.
