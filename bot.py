...
# ── RSS: сначала Казахстан, потом Россия ────────────────────────────────────
RSS_FEEDS = [
...
async def fetch_metal_news() -> list:
    """Парсит RSS: русский язык, тема металлов, последние 3 дня, без криминала, без дублей."""
    candidates = []
    seen_titles: set = set()
    seen_domains: dict = {}
            ...
            for feed_url, domain in RSS_FEEDS:
                ...
                try:
                    ...
                    for item in root.findall(".//item"):
                        ...
                        # 3. Стоп-слова (криминал и т.д.)
                        if any(sw in tl for sw in NEWS_STOP_WORDS):
                            continue

                        # 4. Обязательно по теме
                        if not any(kw in tl for kw in NEWS_KEYWORDS_PRIMARY):
                            continue

                        # 5. Точный дубль по заголовку
                        if title in seen_titles:
                            continue

                        # 6. Не дублировать ранее опубликованное (по смыслу)
                        if any(is_similar_title(title, p, threshold=0.60) for p in published_titles):
                            logger.info(f"Пропуск (дубль БД): {title[:60]}")
                            continue

                        # 7. Не дублировать в текущей выборке
                        if any(is_similar_title(title, c["title"], threshold=0.50) for c in candidates):
                            continue
    ...
    # Сортируем по важности, берём топ-8
    ...
    logger.info(f"fetch_metal_news: {len(found)} новостей (из {len(candidates)} кандидатов)")
    return found
...
async def publish_news_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(str(update.message.from_user.id)):
        ...
        return
    await update.message.reply_text("⏳ Ищу свежие новости по металлам...")
    articles = await fetch_metal_news()
    if not articles:
        await update.message.reply_text(
            "⚠️ Свежих новостей не найдено — все уже были опубликованы ранее.\n\n"
            "Это нормально: RSS-ленты обновляются несколько раз в день.\n"
            "Новые статьи появятся через несколько часов.\n\n"
            "Если хочешь принудительно сбросить историю — /reset_news"
        )
        return
    await post_news_to_group(context.bot, articles)
    lines = "\n".join([f"{a['emoji']} {a['title'][:55]}..." for a in articles])
    await update.message.reply_text(f"✅ Опубликовано {len(articles)} новостей:\n\n{lines}")
