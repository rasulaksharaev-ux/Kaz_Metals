...
MEDIA_CHANNEL_ID = -1003862533044
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle deep link offer_SELLERID_ADID — предложить цену
    if context.args and context.args[0].startswith("offer_"):
        ...
        await update.message.reply_text(
            ...
            "Введите вашу цену числом (например: 600000):"
        )
        return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirm_username":
        # Сразу отправляем заявку без имени/фамилии/телефона
        u = states.get(user_id, {})
        user_data = {
            "first_name": "",
            "last_name": "",
        ...
        try:
            ...
            if media_msg_id:
                buttons.append([InlineKeyboardButton("📸 Фото и видео",
                    url=f"https://t.me/kaz_metalsphoto/{media_msg_id}")])
            buttons.append([InlineKeyboardButton(
                f"💰 Купить за {price_text}",
                url=f"https://t.me/KAZ_METALSbot?start=buy_{user_id}_{ad_id}")])
            buttons.append([InlineKeyboardButton(
                "🤝 Предложить цену",
                url=f"https://t.me/KAZ_METALSbot?start=offer_{user_id}_{ad_id}")])
    ...
    if data == "menu_how":
        await context.bot.send_message(chat_id=chat_id,
            text="📋 Как работает KAZMETALS\n\n"
                 ...
                 "6. Объявление автоматически появится в группе\n\n"
                 "🔷 Для покупателя:\n"
                 "1. Зайдите в группу KAZ_METALS\n"
                 "2. Найдите нужное объявление\n"
                 "3. Доступны три действия:\n"
                 "   📸 Фото и видео — посмотреть доп. медиа\n"
                 "   💰 Купить за указанную цену\n"
                 "   🤝 Предложить свою цену\n"
                 "4. После подтверждения оба получат контакты\n\n"
                 "🔔 Подписки — получайте уведомления о новых объявлениях в выбранных категориях\n\n"
                 "📊 Цены на металлы — актуальный курс золота, серебра, платины и палладия\n\n"
                 "⭐ После сделки через 7 дней придёт запрос на оценку. Это формирует ваш рейтинг\n\n"
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "offer_price":
        ...
        await update.message.reply_text(
            f"✅ Ваше предложение {offer} отправлено продавцу!\n\nЖдите ответа.",
            reply_markup=main_menu_keyboard())
        states[user_id] = {"step": "menu"}
        return
