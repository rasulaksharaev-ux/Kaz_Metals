...
async def test_alert_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    и в личке показывает текущие цены, chp, пороги и что будет/не будет отправлено."""
    if update.message.chat.type != "private":
        return
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
    ...
    try:
        ...
        await update.message.reply_text(f"❌ Ошибка получения цен: {e}")
        return
    if not prices:
        await update.message.reply_text(
            "❌ Источник цен не вернул данные.\n\n"
            "Возможные причины:\n"
            "• TWELVEDATA_API_KEY не задан или невалидный\n"
            "• Превышен суточный лимит (800/день на free)\n"
            "• Railway блокирует домен api.twelvedata.com\n"
            "• НБРК (USD/KZT) недоступен\n\n"
            "Смотри подробности в логах Railway."
        )
        return
    # Сводка
    lines = [f"🧪 <b>Тест алертов</b>\n", f"USD/KZT: {usd_kzt:,}\n"]
    will_send = []
    for sym, (name, price, open_kzt, chp, unit) in prices.items():
        if chp is None and open_kzt and price:
            try:
        ...
        # Решение алерта
        if chp is None:
            decision = "⚠️ chp недоступен (нет open_price от goldapi)"
        else:
