...
# Категории приведены к названиям тем в группе KAZ_METALS.
# Если в Telegram у темы другой message_thread_id, его можно переопределить
# через Railway Variables, например: TOPIC_COINS=2, TOPIC_BARS=4 и т.д.
def optional_int_env(name: str, default: int = 0) -> int:
    ...
    try:
        ...
        logger.warning(f"{name} должен быть числом. Сейчас указано: {value!r}. Использую {default}.")
        return default
...
CATEGORY_TOPICS = {
    # Уже известные ID из предыдущей рабочей версии
    "📈 Инвестиции и цены": optional_int_env("TOPIC_PRICES", 12),
    ...
    # Новые темы. Если объявление не попадает в нужную тему,
    # укажи правильный ID в Railway Variables.
    ...
    # Старые названия оставлены как алиасы для объявлений/подписок,
    # которые могли быть созданы до обновления.
    "💍 Ювелирка": optional_int_env("TOPIC_JEWELRY", 8),
    "🥇 Слитки": optional_int_env("TOPIC_BARS", 4),
    "🗑 Лом и скупка": optional_int_env("TOPIC_SCRAP", 11),
    "🪙 Монеты драг": optional_int_env("TOPIC_COINS", 2),
}
...
def get_user_ads(user_id: str) -> List[dict]:
    return db_fetchall(
        "SELECT * FROM ads WHERE user_id = %s AND status = 'active' ORDER BY ad_id DESC",
        (str(user_id),),
    )


def mark_ad_deleted(ad_id: str, user_id: str) -> bool:
    try:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute(
            "UPDATE ads SET status = 'deleted' WHERE ad_id = %s AND user_id = %s",
            (int(ad_id), str(user_id)),
        )
        ...
        logger.error(f"mark_ad_deleted error: {e}")
        return False
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.args and context.args[0].startswith("buy_"):
        ...
        if seller_id == user_id:
            ...
            return
        ad = db_fetchone("SELECT * FROM ads WHERE ad_id = %s AND status = 'active'", (int(ad_id),)) if ad_id.isdigit() else None
        if not ad:
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "profile_ads":
        ...
        if not ads:
            await context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Назад в профиль", callback_data="menu_profile")]]),
            )
            return
        ...
        rows.append([InlineKeyboardButton("🔙 Назад в профиль", callback_data="menu_profile")])
        await context.bot.send_message(
            chat_id=chat_id,
            text="📦 Ваши активные объявления. Выберите объявление, которое хотите посмотреть или удалить:",
    ...
    if data.startswith("profile_ad_"):
        ...
        ad = db_fetchone(
            "SELECT * FROM ads WHERE ad_id = %s AND user_id = %s AND status = 'active'",
            (int(ad_id), str(user_id)),
        ) if ad_id.isdigit() else None
        if not ad:
            await context.bot.send_message(
                chat_id=chat_id,
                text="❌ Объявление не найдено или уже удалено.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 К моим объявлениям", callback_data="profile_ads")]]),
            )
            return
        await context.bot.send_message(
            chat_id=chat_id,
    ...
    if data.startswith("delete_ad_"):
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"Удалить объявление №{ad_id}? Это действие нельзя отменить.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Да, удалить", callback_data=f"delete_confirm_{ad_id}")],
                [InlineKeyboardButton("❌ Отмена", callback_data=f"profile_ad_{ad_id}")],
            ]),
        )
        return
    ...
    if data.startswith("delete_confirm_"):
        ...
        # Сначала пробуем удалить опубликованные сообщения из группы/медиа-канала.
        # Если у бота нет прав удалить сообщение, объявление всё равно скрывается в базе.
        for message_id in [m for m in str(ad.get("message_id", "")).split(",") if m.strip().isdigit()]:
            try:
                await context.bot.delete_message(chat_id=GROUP_ID, message_id=int(message_id.strip()))
            except Exception as e:
                logger.warning(f"Could not delete group message {message_id} for ad {ad_id}: {e}")

        for message_id in [m for m in str(ad.get("media_message_id", "")).split(",") if m.strip().isdigit()]:
            try:
                await context.bot.delete_message(chat_id=MEDIA_CHANNEL_ID, message_id=int(message_id.strip()))
            except Exception as e:
                logger.warning(f"Could not delete media message {message_id} for ad {ad_id}: {e}")

        ok = mark_ad_deleted(ad_id, user_id)
        if ok:
            await context.bot.send_message(
                chat_id=chat_id,
            ...
            await context.bot.send_message(chat_id=chat_id, text="⚠️ Не удалось удалить объявление. Попробуйте позже.")
        return
    ...
    if data.startswith("buy_confirm_"):
        ...
        ad = db_fetchone("SELECT * FROM ads WHERE ad_id = %s AND status = 'active'", (int(ad_id),)) if str(ad_id).isdigit() else None
        if not ad:
            await context.bot.send_message(chat_id=chat_id, text="❌ Объявление уже удалено или недоступно.")
            return
