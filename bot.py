...
def save_ad(ad_data):
    try:
        ...
        cur = conn.cursor()
        cur.execute("""
            INSERT INTO ads (user_id, username, type, category, description, price, photo_id, status, created_at, message_id)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s) RETURNING ad_id
        """, (
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "ad_confirm":
        ...
        # Маскируем username: @ras_0104 -> @ras***04
        def mask_username(uname):
            if not uname or len(uname) < 4:
                return "@***"
            return f"@{uname[:3]}***{uname[-2:]}"
    ...
    if data.startswith("buy_") and not data.startswith("buy_confirm_"):
        ...
        if seller_id == user_id:
            ...
            return
        await query.answer()
        # Показываем подтверждение прямо в группе
        await context.bot.send_message(
            chat_id=query.message.chat_id,
            text="Вы хотите приобрести этот товар?\n\n"
...
async def daily_prices(context: ContextTypes.DEFAULT_TYPE):
    ...
    now = datetime.now().strftime("%d.%m.%Y %H:%M")
    if prices:
        text = f"📊 Ежедневные цены на металлы\n🕐 {now}\n💵 USD/KZT: {usd_kzt}\n\n"
        for symbol, (name, price, unit) in prices.items():
            text += f"{name}: {price:,} KZT/{unit}\n"
        text += "\nЦены за 1 грамм"
        try:
            await context.bot.send_message(
                chat_id=GROUP_ID,
                text=text,
                message_thread_id=CATEGORY_TOPICS.get("📈 Инвестиции и цены")
            )
        except Exception as e:
            logger.error(f"Daily prices error: {e}")
...
def main():
    ...
    app.job_queue.run_daily(daily_prices, time=datetime.strptime("04:00", "%H:%M").time())
