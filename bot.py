...
async def open_prices_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.chat.type != "private" or not is_admin(str(update.message.from_user.id)):
        return
    almaty_hour = (datetime.utcnow() + timedelta(hours=5)).hour
    await update.message.reply_text("⏳ Публикую цены...")
    if almaty_hour < 12:
        await morning_prices_job(context)
    elif almaty_hour < 18:
        await morning_prices_job(context)   # заполняет _MORNING_PRICES для алертов
        await midday_prices_job(context)
    else:
        if not _MORNING_PRICES:
            await morning_prices_job(context)
        await evening_prices_job(context)
    await update.message.reply_text("✅ Готово. Алерты теперь активны.")
