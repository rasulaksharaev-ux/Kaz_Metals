...
_RSS_FEEDS = [
    ...
    # ── Международный контент на английском ──
    # Маркер "google.news.en" = переводим ВСЁ из этих лент, не глядя на домен-источника.
    # Иначе businesswire.com / prnewswire.com / hodinkee.com без маркера выбрасываются.
    # Металлы и цены
    (_gnews_en("gold price XAU record market forecast"),               "google.news.en"),
    (_gnews_en("silver platinum palladium price market"),              "google.news.en"),
    # Аукционы и ювелирные бренды
    (_gnews_en("luxury jewelry auction Sotheby Christie record sale"), "google.news.en"),
    (_gnews_en("Rolex Patek Audemars watch auction record"),           "google.news.en"),
    (_gnews_en("diamond gemstone rare record auction"),                "google.news.en"),
    ...
    (_gnews_en("copper price LME market outlook"),                     "google.news.en"),
    (_gnews_en("copper nickel zinc aluminium base metals market"),     "google.news.en"),
    (_gnews("медь цена LME рынок Казахстан", "KZ"),                   "google.news"),

    # ── Часы: специализированный контент ──
    (_gnews_en("luxury watch Rolex Patek Philippe new release 2025"),  "google.news.en"),
    (_gnews_en("watch auction record Sotheby Christie Antiquorum"),    "google.news.en"),
    (_gnews_en("Swiss watches Watches Wonders new collection"),        "google.news.en"),

    # ── Камни: специализированный контент ──
    (_gnews_en("diamond price Rapaport market report"),                "google.news.en"),
    (_gnews_en("colored gemstone emerald ruby sapphire auction record"),"google.news.en"),
    (_gnews_en("rare gemstone sold auction record price"),             "google.news.en"),
...
async def open_discussions_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Разрешает участникам писать в Обсуждениях (общий чат группы)."""
    if update.message.chat.type != "private":
        return
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
    ...
    try:
        ...
        await update.message.reply_text(
            "✅ Готово — участники могут писать в Обсуждениях.\n"
            "Все остальные топики по-прежнему закрыты."
        )
    except Exception as e:
        await update.message.reply_text(f"⚠️ Ошибка: {e}")
...
def main():
    ...
    app.add_handler(CommandHandler("open_discussions", open_discussions_cmd))
