...
async def morning_prices_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сохраняем только цену для сравнения в полдень/вечер
    ...
    for sym in order:
        if sym in prices:
            name, price_kzt, chp, unit = prices[sym]
            lines.append(_fmt_price(name, price_kzt, unit))
    lines.append(f"\n💱 USD/KZT: {usd_kzt:,}")
    try:
        await context.bot.send_message(
...
async def midday_prices_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym in order:
        ...
        same_session = (_MORNING_SET_TIME and
                        (datetime.utcnow()-_MORNING_SET_TIME).total_seconds() < 600)
        if sym in _MORNING_PRICES and _MORNING_PRICES[sym] and not same_session:
