...
BOT_USERNAME = clean_env_value(os.environ.get("BOT_USERNAME")) or "ALTYN_TRUSTbot"
MEDIA_CHANNEL_USERNAME = clean_env_value(os.environ.get("MEDIA_CHANNEL_USERNAME")) or "altyn_trustphoto"
...
async def fetch_usd_kzt(session: aiohttp.ClientSession) -> Optional[float]:
    ...
    try:
        async with session.get(
            ...
            headers={"User-Agent": "ALTYN_TRUSTBot/1.0"},
        ) as r:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user and user.get("status") == "active" and user.get("joined") == "yes":
        ...
        await update.message.reply_text("🏅 Главное меню ALTYN_TRUST", reply_markup=main_menu_keyboard())
        return
    ...
    if user and user.get("status") == "active" and user.get("joined") != "yes":
        ...
        await update.message.reply_text(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔓 Вступить в ALTYN_TRUST", url=GROUP_LINK)],
    ...
    await update.message.reply_text(
        "🏅 Добро пожаловать в ALTYN_TRUST!\n\n"
...
async def rules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "4. ALTYN_TRUST является информационным посредником.\n"
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "about":
        await context.bot.send_message(
            chat_id=chat_id,
            text="ℹ️ ALTYN_TRUST — платформа для объявлений и обсуждений по металлам.\n\n"
    ...
    if data == "admin_delete_all_confirm":
        if not is_admin(user_id):
            await context.bot.send_message(chat_id=chat_id, text="⛔ Доступно только владельцу.")
            return
        await context.bot.send_message(chat_id=chat_id, text="🗑 Удаляю все объявления...")
        try:
            conn = get_conn()
            cur = conn.cursor(cursor_factory=psycopg2.extras.RealDictCursor)
            ...
            for ad in all_ads:
                ...
                deleted_count += 1
            cur2 = conn.cursor()
            cur2.execute("UPDATE ads SET status = 'deleted' WHERE status = 'active'")
            conn.commit()
            cur.close()
            cur2.close()
            conn.close()
            await context.bot.send_message(
                chat_id=chat_id,
                text=f"✅ Готово!\n\n🗑 Удалено объявлений: {deleted_count}\n⚠️ Не удалось удалить из группы: {failed_count}\n\nСчётчик объявлений начнётся с 0 для новых публикаций (старые ID останутся в базе как deleted).",
    ...
    if data.startswith("buy_confirm_"):
        ...
        if deal_id and context.application.job_queue is not None:
            context.application.job_queue.run_once(review_prompt_job, when=timedelta(days=1), data={"deal_id": str(deal_id)}, name=f"review_{deal_id}")
        await context.bot.send_message(
            chat_id=int(user_id),
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "awaiting_phone":
        ...
        await context.bot.send_message(
            ...
            text=f"🆕 Новый пользователь ALTYN_TRUST\n{data.get('first_name','')} {data.get('last_name','')}\n@{username}\n📞 {text}\nID: {user_id}",
        )
        return
...
def main():
    ...
    logger.info("ALTYN_TRUST Bot запущен!")
