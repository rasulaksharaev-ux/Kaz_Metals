...
async def _fetch_price_data(session, symbol: str):
    """Возвращает (change_pct, current_price_usd) или (None, None)."""
    url = (
    ...
    try:
        ...
        logger.error(f"_fetch_price_data {symbol}: {e}")
        return None, None
    ...
    async with aiohttp.ClientSession() as session:
        for symbol, name, emoji_up, emoji_down in _ALERT_METALS:
            ...
            if lbma_key and lbma_key in prices_kzt:
                ...
                price_line = f"Цена: {p:,} ₸ / г\n"
            elif symbol == "HG=F" and usd_kzt:
                # Медь: Yahoo даёт USD/фунт → конвертируем в KZT/г
                try:
                    _, cu_usd = await _fetch_price_data(session, "HG=F")
                    if cu_usd:
                        cu_kzt = round((cu_usd / 453.592) * usd_kzt)
                        price_line = f"Цена: {cu_kzt:,} ₸ / г\n"
                except Exception:
                    pass
            rate_line = f"Курс USD/KZT: {usd_kzt:,}\n" if usd_kzt else ""
            text = (
                f"{emoji} <b>{name} {sign}{abs_change:.1f}%</b>\n"
                f"С вчерашнего закрытия\n\n"
                f"{price_line}"
                f"{rate_line}"
            )
            try:
...
async def check_alerts_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Публикует текущее изменение всех металлов принудительно."""
    if update.message.chat.type != "private":
        return
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
        return
    if not NEWS_TOPIC_ID or not GROUP_ID:
        await update.message.reply_text("⚠️ NEWS_TOPIC_ID или GROUP_ID не заданы.")
        return
    await update.message.reply_text("⏳ Получаю данные...")
    try:
        prices_kzt, usd_kzt = await get_metal_prices()
    ...
    async with aiohttp.ClientSession() as session:
        for symbol, name, emoji_up, emoji_down in _ALERT_METALS:
            ...
            if not price_line and symbol == "HG=F" and usd_kzt:
                try:
                    _, cu_usd = await _fetch_price_data(session, "HG=F")
                    if cu_usd:
                        cu_kzt = round((cu_usd / 453.592) * usd_kzt)
            ...
            try:
                ...
                logger.error(f"check_alerts_cmd: {e}")
    await update.message.reply_text(f"✅ Опубликовано {sent} алертов.")
