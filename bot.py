...
async def get_realtime_prices() -> Tuple[Dict[str, Tuple[str, int, str]], int]:
    """Реальные котировки с goldapi.io (обновляются в реальном времени).
    goldapi.io разрешён на Railway. Конвертация USD/oz → KZT/г по курсу НБРК."""
    metal_names = {
        "XAU": "🥇 Золото",
        "XAG": "🥈 Серебро",
        "XPT": "💎 Платина",
    ...
    results: Dict[str, Tuple[str, int, str]] = {}
    if not GOLDAPI_KEY:
        logger.error("GOLDAPI_KEY не задан — реальные цены недоступны")
        return results, 0
        ...
        for sym, name in metal_names.items():
            try:
                ...
                async with session.get(
                    ...
                    if r.status != 200:
                        logger.error(f"goldapi {sym}: status {r.status}")
                        continue
                    data = await r.json()
                price_usd_oz = data.get("price") or data.get("price_usd")
                if not price_usd_oz:
                    continue
                price_kzt_g = round((float(price_usd_oz) / 31.1035) * usd_kzt)
                results[sym] = (name, price_kzt_g, "г")
            except Exception as e:
...
async def _get_all_prices():
    """Реальные котировки XAU/XAG/XPT/XPD с Yahoo Finance в KZT/г."""
...
async def morning_prices_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym in order:
        if sym in prices:
            lines.append(_fmt_price(*prices[sym]))
    lines.append(f"\n💱 USD/KZT: {usd_kzt:,}")
    try:
        await context.bot.send_message(
...
async def midday_prices_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym in order:
        ...
        name, price, unit = prices[sym]
        chg = None
        if sym in _MORNING_PRICES and _MORNING_PRICES[sym][1]:
            chg = (price - _MORNING_PRICES[sym][1]) / _MORNING_PRICES[sym][1] * 100
...
async def deviation_alert_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym, (name, price, unit) in prices.items():
        if sym not in _MORNING_PRICES or not _MORNING_PRICES[sym][1]:
            continue
        chg = (price - _MORNING_PRICES[sym][1]) / _MORNING_PRICES[sym][1] * 100
        ...
        text = (
            f"⚡️ {name} {action}\n\n"
            f"{sign}{chg:.1f}% с открытия сегодня\n"
            f"Цена: {price:,} ₸ / {unit}\n"
            f"USD/KZT: {usd_kzt:,}"
        )
        try:
