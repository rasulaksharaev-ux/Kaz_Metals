...
def extract_domain(url: str) -> str:
    """Извлекает домен из URL, например https://rbc.ru/... → rbc.ru"""
    try:
...
async def fetch_metal_news() -> list:
    """Получает русскоязычные новости о металлах через NewsAPI."""
    found = []
    try:
        async with aiohttp.ClientSession() as session:
            url = (
                "https://newsapi.org/v2/everything"
                "?q=золото+серебро+металлы+нефть+рубль+инвестиции"
                "&language=ru"
                "&sortBy=publishedAt"
                "&pageSize=10"
                ...
                async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as r:
                    if r.status == 200:
                        ...
                        for a in articles:
                            ...
                            # Убираем суффикс источника: "Заголовок - Источник" → "Заголовок"
                            if " - " in title:
                        ...
                        logger.error(f"NewsAPI status {r.status}")
            except Exception as e:
...
async def post_news_to_group(bot) -> None:
    ...
    Формат: короткий заголовок + домен источника (как на скрине).
    """
