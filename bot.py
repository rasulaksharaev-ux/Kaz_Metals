...
_ALERT_THRESHOLD_PCT = 1.5   # % от цены открытия для алерта (средние и сильные отклонения)
...
async def midday_prices_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for sym in order:
        ...
        if sym in _MORNING_PRICES and _MORNING_PRICES[sym]:
            chg = (price - _MORNING_PRICES[sym]) / _MORNING_PRICES[sym] * 100
        else:
...
async def deviation_alert_job(context: ContextTypes.DEFAULT_TYPE):
    """Каждый час — алерт если цена отклонилась от утренней на ≥2.5%."""
    if not NEWS_TOPIC_ID or not GROUP_ID or not _MORNING_PRICES:
        return
    try:
        prices, usd_kzt = await _get_all_prices()
    except Exception as e:
    ...
    for sym, (name, price, open_kzt, chp, unit) in prices.items():
        if chp is None:
            continue
        chg = float(chp)
        if abs(chg) < _ALERT_THRESHOLD_PCT:
            continue
        key = f"{sym}_{'up' if chg>0 else 'down'}"
        if key in _ALERTED_TODAY:
            continue
        action = "резко вырос" if chg > 0 else "резко упал"
        sign = "+" if chg > 0 else ""
        text = (
            f"⚡️ {name} {action}\n\n"
            f"{sign}{chg:.1f}% от вчерашнего закрытия\n"
            f"Цена: {price:,} ₸ / {unit}\n"
            f"USD/KZT: {usd_kzt:,}"
        )
        try:
            await context.bot.send_message(
                chat_id=GROUP_ID, message_thread_id=NEWS_TOPIC_ID, text=text)
            _ALERTED_TODAY.add(key)
            logger.info(f"deviation alert: {name} {sign}{chg:.1f}%")
        except Exception as e:
...
async def open_prices_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.chat.type != "private" or not is_admin(str(update.message.from_user.id)):
        return
    await update.message.reply_text("⏳ Публикую утро, полдень и вечер...")
    await morning_prices_job(context)        # цена открытия + база для алертов
    await _asyncio.sleep(1)
    await midday_prices_job(context)          # текущая vs открытие
    await _asyncio.sleep(1)
    await evening_prices_job(context)         # текущая vs открытие
    await update.message.reply_text("✅ Готово. Алерты активны (сработают при ±2.5% от открытия).")
...
def main():
    ...
    if GROUP_ID and app.job_queue is not None:
        ...
        app.job_queue.run_daily(morning_prices_job, time=dt_time(hour=4, minute=0))
        app.job_queue.run_daily(midday_prices_job, time=dt_time(hour=9, minute=0))
        app.job_queue.run_daily(evening_prices_job, time=dt_time(hour=16, minute=0))
        app.job_queue.run_repeating(deviation_alert_job, interval=3600, first=300)
