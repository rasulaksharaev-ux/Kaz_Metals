...
async def get_realtime_prices():
    """Котировки с twelvedata.com (free план 800 req/день, intraday).
    ...
    if not TWELVEDATA_API_KEY:
        logger.error("TWELVEDATA_API_KEY не задан в Railway Variables")
        return results, 0
    async with aiohttp.ClientSession() as session:
        usd_kzt = await fetch_usd_kzt(session)
        if not usd_kzt:
            ...
            return results, 0
        # Twelvedata batch: один запрос на все 4 металла
        ...
        try:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=15)) as r:
                if r.status != 200:
                    logger.error(f"twelvedata HTTP {r.status}")
                    return results, round(usd_kzt)
                data = await r.json()
        except Exception as e:
            logger.error(f"twelvedata request error: {e}")
            return results, round(usd_kzt)
        # При запросе нескольких символов twelvedata возвращает dict вида
        # {"XAU/USD": {...}, "XAG/USD": {...}, ...}
        # При запросе одного — плоский объект. Нормализуем оба варианта.
        if isinstance(data, dict) and data.get("symbol"):
            # одиночный ответ (на случай если один символ)
            data = {data["symbol"]: data}
        if isinstance(data, dict) and data.get("code") and data.get("message"):
            # ошибка от API (например, превышен лимит, неверный ключ)
            logger.error(f"twelvedata API error: code={data.get('code')} message={data.get('message')}")
            return results, round(usd_kzt)
        for sym, name in metal_names.items():
            key = f"{sym}/USD"
            item = data.get(key) if isinstance(data, dict) else None
            if not item or not isinstance(item, dict):
                logger.warning(f"twelvedata: нет данных для {key}")
                continue
            try:
                price_usd = float(item.get("close") or item.get("price") or 0)
                open_usd  = float(item.get("open") or 0)
                pct = item.get("percent_change")
                chp = float(pct) if pct is not None else None
                if not price_usd:
                    logger.warning(f"twelvedata: price=0 для {key}")
                    continue
                price_kzt_g = round((price_usd / 31.1035) * usd_kzt)
                open_kzt_g  = round((open_usd / 31.1035) * usd_kzt) if open_usd else price_kzt_g
                # Fallback: если chp нет, считаем сами
                if chp is None and open_usd and price_usd:
                    chp = ((price_usd - open_usd) / open_usd) * 100
                results[sym] = (name, price_kzt_g, open_kzt_g, chp, "г")
            except Exception as e:
                logger.error(f"twelvedata parse {key}: {e}")
    if results:
...
async def test_alert_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Сначала — отдельно дёргаем Twelvedata напрямую, чтобы показать сырой ответ
    diag_lines = ["🔧 <b>Диагностика источника цен</b>\n"]
    if not TWELVEDATA_API_KEY:
        diag_lines.append("❌ TWELVEDATA_API_KEY не задан в Railway Variables!")
        # Покажем какие переменные бот ВООБЩЕ видит — это поможет понять
        # где переменная (в правильном сервисе? имя без опечатки?)
        visible_vars = sorted([k for k in os.environ.keys() if not k.startswith("_") and len(k) < 50])
        # Скрываем системные переменные Railway/Linux
        system_prefixes = ("PATH", "HOME", "LANG", "LC_", "PWD", "SHLVL", "TERM", "PYTHON", "PIP_", "HOST", "PORT", "RAILWAY_INTERNAL", "NIXPACKS", "_")
        user_vars = [v for v in visible_vars if not any(v.startswith(p) for p in system_prefixes)]
        # Маркируем TWELVE-подобные имена (может опечатка)
        twelve_like = [v for v in user_vars if "TWELVE" in v.upper() or "DATA" in v.upper() or "GOLD" in v.upper() or "API" in v.upper()]
        diag_lines.append(f"\n<b>Переменные которые бот ВИДИТ</b> (без значений):")
        if user_vars:
            diag_lines.append(f"<code>{', '.join(user_vars[:30])}</code>")
        else:
            diag_lines.append("<i>(пусто — это очень странно, возможно бот в неправильном сервисе)</i>")
        if twelve_like:
            diag_lines.append(f"\n<b>Похожие на ключ:</b> <code>{', '.join(twelve_like)}</code>")
        diag_lines.append("\n<b>Проверь:</b>")
        diag_lines.append("1. Имя точно <code>TWELVEDATA_API_KEY</code> (без опечаток, заглавные)")
        diag_lines.append("2. Переменная в сервисе <b>бота</b>, не в Postgres")
        diag_lines.append("3. После сохранения сделан <b>Restart/Redeploy</b> сервиса")
        await update.message.reply_text("\n".join(diag_lines), parse_mode="HTML")
        return
    diag_lines.append(f"✓ Ключ найден: {TWELVEDATA_API_KEY[:6]}…{TWELVEDATA_API_KEY[-4:]} (длина {len(TWELVEDATA_API_KEY)})")
    test_url = f"https://api.twelvedata.com/quote?symbol=XAU/USD,XAG/USD,XPT/USD,XPD/USD&apikey={TWELVEDATA_API_KEY}"
    try:
        async with aiohttp.ClientSession() as session:
        ...
        diag_lines.append(f"\n<b>Сырой ответ (первые 1500 символов):</b>\n<pre>{raw_text}</pre>")
    except Exception as e:
        diag_lines.append(f"❌ Не удалось подключиться к api.twelvedata.com")
        diag_lines.append(f"Ошибка: <code>{type(e).__name__}: {e}</code>")
        diag_lines.append("\nЭто значит Railway блокирует домен. Скажи мне — переключусь на план Б.")
        await update.message.reply_text("\n".join(diag_lines), parse_mode="HTML")
        return
