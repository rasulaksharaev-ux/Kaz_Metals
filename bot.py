...
ALERT_TOPIC_ID = optional_int_env("ALERT_TOPIC_ID", NEWS_TOPIC_ID)  # можно отдельную вкладку
ALERT_THRESHOLD = float(os.environ.get("ALERT_THRESHOLD", "2.0"))  # % изменения для алерта
...
# ── Источники: Tier 1 = Казахстан (приоритет), 2 = RU-финансы, 3 = RU-СМИ ──
KZ_DOMAINS = {
    "nationalbank.kz", "kapital.kz", "kursiv.kz",
    "tengrinews.kz", "inform.kz", "zakon.kz",
    "forbes.kz", "bizmedia.kz", "finprom.kz",
}

# Международные агрегаторы — тематически отфильтрованные, хороший источник
_WORLD_AGGREGATORS = {"google.news", "yandex.news"}

def _source_tier(domain: str) -> int:
    if domain in KZ_DOMAINS:
        return 1
    if domain in _WORLD_AGGREGATORS:
        return 2   # международный агрегатор — выше RU-финансов
    ru_finance = {"profinance.ru", "1prime.ru", "finmarket.ru", "finanz.ru", "gold.ru", "metalinfo.ru"}
    if domain in ru_finance:
        return 3
    return 4
...
_RSS_FEEDS = [
    ...
    # ══ МИР — Google News на русском (агрегирует Bloomberg, Reuters, FT и др.) ══
    # Тема: золото и серебро
    (
        "https://news.google.com/rss/search"
        "?q=%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%BE+%D1%81%D0%B5%D1%80%D0%B5%D0%B1%D1%80%D0%BE"
        "+%D0%BF%D0%BB%D0%B0%D1%82%D0%B8%D0%BD%D0%B0"
        "&hl=ru&gl=KZ&ceid=KZ:ru",
        "google.news"
    ),
    # Тема: НБРК драгметаллы Казахстан
    (
        "https://news.google.com/rss/search"
        "?q=%D0%9D%D0%91%D0%A0%D0%9A+%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%BE"
        "+%D0%9A%D0%B0%D0%B7%D0%B0%D1%85%D1%81%D1%82%D0%B0%D0%BD"
        "&hl=ru&gl=KZ&ceid=KZ:ru",
        "google.news"
    ),
    # Тема: ювелирные изделия Казахстан
    (
        "https://news.google.com/rss/search"
        "?q=%D1%8E%D0%B2%D0%B5%D0%BB%D0%B8%D1%80%D0%BD%D1%8B%D0%B5"
        "+%D0%BA%D0%B0%D0%B7%D0%B0%D1%85%D1%81%D1%82%D0%B0%D0%BD"
        "+%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%BE"
        "&hl=ru&gl=KZ&ceid=KZ:ru",
        "google.news"
    ),
    # Тема: цена золота инвестиции (мировой охват)
    (
        "https://news.google.com/rss/search"
        "?q=%D1%86%D0%B5%D0%BD%D0%B0+%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%B0"
        "+%D0%B4%D1%80%D0%B0%D0%B3%D0%BC%D0%B5%D1%82%D0%B0%D0%BB%D0%BB%D1%8B"
        "&hl=ru&gl=RU&ceid=RU:ru",
        "google.news"
    ),
    # Тема: слитки монеты НацБанк
    (
        "https://news.google.com/rss/search"
        "?q=%D1%81%D0%BB%D0%B8%D1%82%D0%BA%D0%B8+%D0%BC%D0%BE%D0%BD%D0%B5%D1%82%D1%8B"
        "+%D0%B4%D1%80%D0%B0%D0%B3%D0%BC%D0%B5%D1%82%D0%B0%D0%BB%D0%BB%D1%8B"
        "&hl=ru&gl=KZ&ceid=KZ:ru",
        "google.news"
    ),

    # ══ МИР — Яндекс.Новости (очень широкий охват рус. изданий) ══
    (
        "https://news.yandex.ru/search.rss"
        "?text=%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%BE+%D1%81%D0%B5%D1%80%D0%B5%D0%B1%D1%80%D0%BE"
        "+%D0%B4%D1%80%D0%B0%D0%B3%D0%BC%D0%B5%D1%82%D0%B0%D0%BB%D0%BB%D1%8B"
        "&rss=1",
        "yandex.news"
    ),
    (
        "https://news.yandex.ru/search.rss"
        "?text=%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%BE+%D0%BA%D0%B0%D0%B7%D0%B0%D1%85%D1%81%D1%82%D0%B0%D0%BD"
        "&rss=1",
        "yandex.news"
    ),
    # Часы — люкс и драгметаллы
    (
        "https://news.google.com/rss/search"
        "?q=%D1%80%D0%BE%D1%81%D0%BA%D0%BE%D1%88%D0%BD%D1%8B%D0%B5+%D1%87%D0%B0%D1%81%D1%8B"
        "+%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%BE+%D1%81%D0%B5%D1%80%D0%B5%D0%B1%D1%80%D0%BE"
        "&hl=ru&gl=RU&ceid=RU:ru",
        "google.news"
    ),
    (
        "https://news.google.com/rss/search"
        "?q=rolex+patek+philippe+audemars+%D1%87%D0%B0%D1%81%D1%8B+%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%BE"
        "&hl=ru&gl=RU&ceid=RU:ru",
        "google.news"
    ),
    # Ювелирные украшения — кольца, серьги, браслеты
    (
        "https://news.google.com/rss/search"
        "?q=%D1%8E%D0%B2%D0%B5%D0%BB%D0%B8%D1%80%D0%BD%D1%8B%D0%B5"
        "+%D0%BA%D0%BE%D0%BB%D1%8C%D1%86%D0%B0+%D1%81%D0%B5%D1%80%D1%8C%D0%B3%D0%B8"
        "+%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%BE"
        "&hl=ru&gl=RU&ceid=RU:ru",
        "google.news"
    ),
    (
        "https://news.google.com/rss/search"
        "?q=%D1%8E%D0%B2%D0%B5%D0%BB%D0%B8%D1%80%D0%BD%D1%8B%D0%B9+%D1%80%D1%8B%D0%BD%D0%BE%D0%BA"
        "+%D0%BA%D0%B0%D0%B7%D0%B0%D1%85%D1%81%D1%82%D0%B0%D0%BD"
        "&hl=ru&gl=KZ&ceid=KZ:ru",
        "google.news"
    ),
    ...
    (
        "https://news.google.com/rss/search"
        "?q=%D0%B1%D1%80%D0%B8%D0%BB%D0%BB%D0%B8%D0%B0%D0%BD%D1%82%D1%8B"
        "+%D0%B4%D1%80%D0%B0%D0%B3%D0%BE%D1%86%D0%B5%D0%BD%D0%BD%D1%8B%D0%B5"
        "+%D0%BA%D0%B0%D0%BC%D0%BD%D0%B8"
        "&hl=ru&gl=RU&ceid=RU:ru",
        "google.news"
    ),
...
_KW_REQUIRED = [
    # ── Золото ──
    "золото", "золот", "цена золота", "курс золота",
    "запасы золота", "резервы золота", "рынок золота",
    # ── Серебро ──
    "серебро", "серебр", "цена серебра",
    # ── Платина / Палладий ──
    ...
    # ── Монеты ──
    "монета золот", "монеты золот", "коллекционная монета",
    "инвестиционная монета", "нбрк монет", "нацбанк монет",
    # ── Лом и скупка ──
    "лом золота", "лом серебра", "скупка золота",
    "скупка серебра", "скупка драгоценн", "сдать лом",
    # ── Ювелирные украшения (Кольца / Серьги / Подвески / Браслеты) ──
    "ювелир", "ювелирн",
    "кольцо", "кольца", "обручальн",
    "серьги", "серьга", "серёжк",
    "браслет", "подвеска", "подвески", "цепочка",
    "украшени", "украшение из золота", "украшение из серебра",
    # ── Камни ──
    "бриллиант", "алмаз", "рубин", "изумруд", "сапфир",
    "жемчуг", "янтарь", "аметист", "топаз", "цирконий",
    "драгоценный камень", "драгоценные камни", "gemstone", "diamond",
    # ── Часы ──
    "золотые часы", "платиновые часы", "серебряные часы",
    "роскошные часы", "люксовые часы", "швейцарские часы",
    "ювелирные часы", "коллекционные часы",
    "rolex", "ролекс", "patek philippe", "патек", "audemars piguet",
    "vacheron constantin", "cartier часы", "breguet", "hublot",
    "richard mille", "a. lange", "jaeger",
    # ── Казахстан специфика ──
    "нбрк золото", "нацбанк золото",
]

# Стоп-слова (одиночные)
_KW_STOP = [
    "контрабанд", "осудили", "осуждён", "арестован",
    "уголовн", "задержан", "мошенничест", "кража", "ограбл",
]

# Стоп-фразы (проверяются целиком в заголовке)
_STOP_PHRASES = [
    "золотой купол",          # военная система ПРО США
    "железный купол",         # израильская система
    "как купить золото",      # советы — не новость
    "как инвестировать",
    "инвестировать или нет",
    "стоит ли покупать",
    "стоит ли инвестировать",
    "почему вечная ценность",
    "как россиянам купить",
    "как казахстанцам купить",
]
...
def _parse_item(item, feed_domain: str) -> tuple:
    ...
    return clean_title, link, real_domain


def _words(title: str) -> set:
    import re as _re
    stop = {"что","это","как","для","при","будет","после","также","свои",
            "этом","млрд","млн","году","года","рублей","более","менее"}
    return {w for w in _re.findall(r'[а-яёa-z]{4,}', title.lower()) if w not in stop}


def _similar(t1: str, t2: str, thr: float = 0.55) -> bool:
    w1, w2 = _words(t1), _words(t2)
    if not w1 or not w2:
        return False
    return len(w1 & w2) / len(w1 | w2) >= thr


def _score(title: str) -> int:
    t = title.lower()
    s = 0
    if any(w in t for w in _KW_KZ):
        s += 120
    if any(w in t for w in ["рекорд", "максимум", "минимум", "впервые", "исторически"]):
        s += 80
    if any(w in t for w in ["цена", "курс", "котировк", "стоимость"]):
        s += 50
    if any(w in t for w in ["сша", "китай", "европа", "евросоюз", "мировой"]):
        s += 40
    if any(w in t for w in ["инвестиц", "резервы", "запасы", "биржа"]):
        s += 30
    return s
...
async def _collect_raw() -> list:
    """Парсит все RSS и возвращает список сырых кандидатов."""
    raw = []
    seen: set = set()
    async with aiohttp.ClientSession() as session:
        for feed_url, feed_domain in _RSS_FEEDS:
            try:
                ...
                for item in root.findall(".//item"):
                    ...
                    if not _is_recent(pub_date, days=3):
                        continue

                    title, link, real_domain = _parse_item(item, feed_domain)
                    ...
                    if any(ph in tl for ph in _STOP_PHRASES):
                        continue
                    if not any(kw in tl for kw in _KW_REQUIRED):
                        continue

                    # Новости о Казахстане — ТОЛЬКО из казахстанских источников
                    is_kz_news = any(kw in tl for kw in _KW_KZ)
                    if is_kz_news and real_domain not in KZ_DOMAINS:
                        continue
                    h = _title_hash(title)
                    ...
                    raw.append({
                        ...
                        "domain": real_domain,
                        "hash":   h,
                        "tier":   _source_tier(feed_domain),  # tier по ленте, не по реальному домену
                        "score":  _score(title),
                    })
...
def _deduplicate(raw: list) -> list:
    """
    Группирует похожие новости → лучший источник → топ-8 по важности.
    Без искусственных лимитов по категориям.
    """
    published = _get_published_titles()
    filtered = [
        a for a in raw
        if not any(_similar(a["title"], p, thr=0.60) for p in published)
    ]
    ...
    for art in filtered:
        placed = False
        for grp in groups:
            if _similar(art["title"], grp[0]["title"], thr=0.50):
                grp.append(art)
                placed = True
                break
        if not placed:
...
async def _do_fetch_and_post(bot) -> list:
    """
    Основная функция: collect → deduplicate → post.
    Защищена Lock от параллельного запуска.
    Возвращает список опубликованных статей.
    """
    async with _NEWS_LOCK:
        raw = await _collect_raw()
        logger.info(f"news: собрано {len(raw)} кандидатов")
        articles = _deduplicate(raw)
        logger.info(f"news: к публикации {len(articles)}")
...
def main():
    ...
    # Ценовые алерты: каждые 2 часа + сброс в полночь
    if ALERT_TOPIC_ID and GROUP_ID and app.job_queue is not None:
        ...
        logger.info(f"Ценовые алерты включены (порог {ALERT_THRESHOLD}%). Topic ID: {ALERT_TOPIC_ID}")
