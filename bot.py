...
# ═══════════════════════════════════════════════════════════════════════════
#  ФИЛЬТРАЦИЯ НОВОСТЕЙ - двухуровневая система без платных API
# ═══════════════════════════════════════════════════════════════════════════

# Уровень A: standalone - одного слова достаточно (почти всегда рыночная новость)
_KW_STANDALONE = [
    "золото", "золот",
    "серебро", "серебр",
    "платина", "платин",
    "палладий",
    "драгоценн", "драгметалл",
    "слиток", "слитк", "аффинаж",
    "лом золота", "скупка золота", "лом серебра",
    "монета золот", "монеты золот",
    "нбрк", "нацбанк казахстан",
    "цена золота", "курс золота", "цена серебра",
    "запасы золота", "резервы золота",
]

# Уровень B: слабые слова - требуют рыночного контекста рядом
_KW_WEAK = [
    "бриллиант", "алмаз", "рубин", "изумруд",
    "сапфир", "жемчуг", "янтарь",
    "ювелир", "кольцо", "кольца", "серьги",
    "браслет", "подвеска", "украшени",
    "часы", "rolex", "ролекс", "patek",
    "audemars", "hublot", "breguet",
    "vacheron", "cartier", "richard mille",
]

# Рыночный контекст - должен быть рядом со слабым словом
_KW_MARKET_CONTEXT = [
    "цена", "стоимость", "продали", "продан",
    "аукцион", "рекорд", "рынок", "биржа",
    "котировк", "торги", "коллекц", "бренд",
    "выпустил", "запустил", "тренд", "спрос",
    "млн", "млрд", "долларов", "тысяч",
    "карат", "инвестиц", "люкс", "эксклюзив",
    "новинка", "релиз", "коллаборац",
]

# Стоп-слова - всегда блокируют
_KW_STOP = [
    "контрабанд", "осудили", "осуждён", "арестован",
    "уголовн", "задержан", "мошенничест",
    "изъяли", "конфисков", "таможн", "досмотр",
    "ракет", "военн", "обстрел",
]

# Стоп-фразы - всегда блокируют
_STOP_PHRASES = [
    "золотой купол", "железный купол",
    "как купить золото", "как инвестировать",
    "инвестировать или нет", "стоит ли покупать",
    "как россиянам купить", "как казахстанцам купить",
]

# Regex-паттерны: "нашла алмаз в парке", "[камень] в джинсах"
import re as _re
_STOP_PATTERNS = [
    _re.compile(r'нашл[аиео]?\s+\w*\s*(брилли|алмаз|золот|самород)', _re.I),
    _re.compile(r'обнаружил\w*\s+\w*\s*(брилли|алмаз|золот)', _re.I),
    _re.compile(r'(брилли|алмаз|золот|камн)\w*\s+в\s+(кармане|джинс|штан|сумк|носк|кулак|багаж)', _re.I),
    _re.compile(r'пытал\w+\s+(вывезти|провезти|перевезти)', _re.I),
    _re.compile(r'(вывез|провез|перевез)\w*\s+\w*(золот|брилли|алмаз)', _re.I),
    _re.compile(r'\(фото\)|\(видео\)|\(фотогалерея\)', _re.I),
    _re.compile(r'туристк?\w*\s+(нашл|привёз|вывез)', _re.I),
]


def _is_relevant(title: str) -> bool:
    """
    Проверяет релевантность заголовка.
    Уровень A: одно сильное слово > OK.
    Уровень B: слабое слово + рыночный контекст > OK.
    Иначе > отклонить.
    """
    tl = title.lower()
    ...
    # Уровень A: сильное слово - само по себе достаточно
    if any(kw in tl for kw in _KW_STANDALONE):
        return True

    # Уровень B: слабое слово требует рыночного контекста
    has_weak = any(kw in tl for kw in _KW_WEAK)
    has_context = any(kw in tl for kw in _KW_MARKET_CONTEXT)
...
async def _do_fetch_and_post(bot) -> list:
    """collect - deduplicate - post. Защищена Lock."""
