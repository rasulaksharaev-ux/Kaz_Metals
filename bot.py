...
async def fetch_metal_news() -> list:
    """Парсит RSS: только русский, только тема металлов, без криминала, без дублей."""
    found = []
    seen_titles: set = set()
    seen_domains: dict = {}
            ...
            for feed_url, domain in RSS_FEEDS:
                ...
                try:
                    ...
                    for item in root.findall(".//item"):
                        ...
                        # 2. Стоп-слова (криминал и т.д.)
                        if any(sw in tl for sw in NEWS_STOP_WORDS):
                            continue

                        # 3. Обязательно по теме
                        if not any(kw in tl for kw in NEWS_KEYWORDS_PRIMARY):
                            continue

                        # 4. Не дублировать уже опубликованное (по смыслу)
                        # Порог 0.60 — требует высокого совпадения слов
                        if any(is_similar_title(title, p, threshold=0.60) for p in published_titles):
                            logger.info(f"Пропуск (дубль БД): {title[:60]}")
                            continue

                        # 5. Не дублировать в текущей выборке
                        if any(is_similar_title(title, f["title"], threshold=0.50) for f in found):
                            continue

                        seen_titles.add(title)
                        seen_domains[domain] = seen_domains.get(domain, 0) + 1
                        emoji = get_news_emoji(title, len(found))
                        found.append({
                            "title":  title,
                            "url":    link,
                            "domain": domain,
                        ...
                        # НЕ сохраняем здесь — только после реальной отправки
    ...
    logger.info(f"fetch_metal_news: {len(found)} новостей")
    return found


async def post_news_to_group(bot, articles: list) -> None:
    """Публикует список новостей в группу и сохраняет в БД только после успешной отправки."""
