...
async def get_metal_prices() -> Tuple[Dict[str, Tuple[str, int, str]], int]:
    """Источник цен: LBMA, курс USD/KZT: Национальный Банк Казахстана."""
    usd_kzt = None
    try:
...
def ad_type_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("Продажа", callback_data="adtype_sale"), InlineKeyboardButton("Аукцион", callback_data="adtype_auction")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "about":
        await context.bot.send_message(
            chat_id=chat_id,
            text="ℹ️ KAZMETALS — платформа для объявлений, аукционов и обсуждений по металлам.\n\n"
    ...
    if data.startswith("adtype_"):
        ad_type = "продажа" if data == "adtype_sale" else "аукцион"
