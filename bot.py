...
async def fetch_metal_news() -> str:
    """Fetch metal news from free RSS feeds."""
    import xml.etree.ElementTree as ET
    rss_feeds = [
        ("https://feeds.reuters.com/reuters/businessNews", "Reuters"),
        ("https://www.kitco.com/rss/kitconews.xml", "Kitco"),
        ("https://goldprice.org/feeds/news", "GoldPrice"),
    ]
    articles = []
    keywords = ["gold", "silver", "platinum", "palladium", "metal", "precious"]
    try:
        async with aiohttp.ClientSession() as session:
            for feed_url, source in rss_feeds:
                try:
                    headers = {"User-Agent": "Mozilla/5.0"}
                    async with session.get(feed_url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)) as r:
                        if r.status == 200:
                            text = await r.text()
        ...
        for title, link, source in articles[:4]:
