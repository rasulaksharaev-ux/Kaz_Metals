...
async def news_debug_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /news_debug - показывает что собрал бот до Groq фильтрации."""
    if update.message.chat.type != "private":
        return
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
        return
    await update.message.reply_text("⏳ Собираю данные со всех источников...")
    raw = await _collect_raw()
    if not raw:
        await update.message.reply_text(
            "❌ Ни одной статьи не собрано.\n\n"
            "Возможные причины:\n"
            "• RSS-ленты недоступны с сервера Railway\n"
            "• Все статьи уже были опубликованы\n"
            "• Фильтр слишком строгий\n\n"
            "Попробуй /reset_news и повтори."
        )
        return
    lines = [f"📦 Собрано {len(raw)} статей:\n"]
    for i, a in enumerate(raw[:15], 1):
        lines.append(f"{i}. [{a['domain']}] {a['title'][:60]}")
    if len(raw) > 15:
        lines.append(f"...и ещё {len(raw)-15}")
    await update.message.reply_text("\n".join(lines))
