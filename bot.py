...
# Источник цен металлов — twelvedata.com (бесплатный план 800 req/день, intraday).
# GOLDAPI_KEY оставлен для совместимости, но больше не используется.
TWELVEDATA_API_KEY = clean_env_value(os.environ.get("TWELVEDATA_API_KEY"))
GOLDAPI_KEY = clean_env_value(os.environ.get("GOLDAPI_KEY"))
ADMIN_CHAT_ID = must_int_env("ADMIN_CHAT_ID")
ADMIN_CHAT_ID_2 = optional_int_env("ADMIN_CHAT_ID_2", 0)  # второй админ (опционально)
...
def init_db() -> None:
    ...
    alter_sql = [
        ...
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS verified_at TEXT DEFAULT ''",
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS reg_step TEXT DEFAULT ''",
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS phone_verified_at TEXT DEFAULT ''",
        "ALTER TABLE users ADD COLUMN IF NOT EXISTS user_city TEXT DEFAULT ''",
        ...
        "ALTER TABLE complaints ADD COLUMN IF NOT EXISTS deal_id TEXT DEFAULT ''",
        ...
        "ALTER TABLE complaints ADD COLUMN IF NOT EXISTS media_file_ids TEXT DEFAULT ''",
        "ALTER TABLE complaints ADD COLUMN IF NOT EXISTS status TEXT DEFAULT 'open'",
        "ALTER TABLE complaints ADD COLUMN IF NOT EXISTS resolved_at TEXT DEFAULT ''",
        "ALTER TABLE complaints ADD COLUMN IF NOT EXISTS admin_notes TEXT DEFAULT ''",
    ]
    for sql in alter_sql:
        cur.execute(sql)

    # Миграция: переименование категории "♻️ Лом и скупка" → "♻️ Лом"
    try:
        cur.execute("UPDATE ads SET category = %s WHERE category = %s", ("♻️ Лом", "♻️ Лом и скупка"))
    except Exception as e:
        logger.error(f"category rename migration: {e}")

    # Миграция: все существующие юзеры (joined='yes') считаются полностью
    # зарегистрированными — их не нужно прогонять через новый flow
    try:
        cur.execute("UPDATE users SET reg_step='done' WHERE joined='yes' AND (reg_step IS NULL OR reg_step='')")
    except Exception as e:
        logger.error(f"reg_step migration: {e}")
...
def refresh_user_meta(tg_user) -> None:
    """Обновляет username/first_name/last_name в БД из объекта telegram.User.
    Вызывать в начале каждого хендлера, чтобы данные всегда были актуальными."""
    if not tg_user:
        return
    try:
        conn = get_conn()
        ...
        logger.error(f"refresh_user_meta error: {e}")
...
COMPLAINT_CATEGORIES = {
    ...
    "fake":      "⚠️ Поддельный товар",
    "no_reply":  "🚫 Не выходит на связь / пропал",
    "rude":      "😡 Неадекватное поведение",
    "other":     "📌 Другое",
}
...
def save_complaint(data: dict) -> Optional[int]:
    try:
        ...
        cur.execute(
            ...
            INSERT INTO complaints
              (ad_id, deal_id, from_user_id, seller_id, category, reason, description, media_file_ids, status, created_at)
            ...
            (
                ...
                str(data.get("from_user_id", "")),
                str(data.get("seller_id", "")),
                str(data.get("category", "")),
                COMPLAINT_CATEGORIES.get(str(data.get("category", "")), ""),
                str(data.get("description", "")),
                _json.dumps(data.get("media_file_ids", [])),
        ...
        logger.error(f"save_complaint error: {e}")
        return None


def get_complaint(complaint_id: int) -> Optional[dict]:
    return db_fetchone("SELECT * FROM complaints WHERE complaint_id = %s", (int(complaint_id),))


def list_complaints(status_filter: Optional[str] = None, limit: int = 5, offset: int = 0) -> List[dict]:
    if status_filter and status_filter != "all":
        return db_fetchall(
            "SELECT * FROM complaints WHERE status = %s ORDER BY complaint_id DESC LIMIT %s OFFSET %s",
            (status_filter, limit, offset),
        )
    return db_fetchall(
        "SELECT * FROM complaints ORDER BY complaint_id DESC LIMIT %s OFFSET %s",
        (limit, offset),
    )


def count_complaints(status_filter: Optional[str] = None) -> int:
    if status_filter and status_filter != "all":
        row = db_fetchone("SELECT COUNT(*) AS c FROM complaints WHERE status = %s", (status_filter,))
    else:
        row = db_fetchone("SELECT COUNT(*) AS c FROM complaints")
    return int((row or {}).get("c", 0))


def update_complaint(complaint_id: int, fields: dict) -> None:
    if not fields:
        return
    ...
    try:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute(f"UPDATE complaints SET {set_clause} WHERE complaint_id = %s", (*values, int(complaint_id)))
        conn.commit()
        cur.close()
        conn.close()
    except Exception as e:
        logger.error(f"update_complaint error: {e}")


# ─── Верификация ────────────────────────────────────────────────────────────

VERIFICATION_DEALS_REQUIRED = 5  # Сколько успешных сделок нужно для бэджа
MAX_ADS_DEFAULT = 999
MAX_ADS_VERIFIED = 999


def is_user_verified(user_id: str) -> bool:
    u = get_user(str(user_id)) or {}
    return bool(u.get("is_verified"))


def verified_badge(user_id: str) -> str:
    """Возвращает '✅' если юзер верифицирован, иначе ''."""
    return " ✅" if is_user_verified(user_id) else ""


def get_max_ads_limit(user_id: str) -> int:
    return MAX_ADS_VERIFIED if is_user_verified(user_id) else MAX_ADS_DEFAULT


def _count_successful_deals(user_id: str) -> int:
    """Сколько у юзера успешных сделок (в любой роли).
    Критерий: deal_id есть в reviews с rating >= 4 (на юзера ставили положительный отзыв),
    ИЛИ сделка имеет status = 'completed' и хотя бы одна положительная оценка по ней."""
    row = db_fetchone(
        """
        SELECT COUNT(DISTINCT d.deal_id) AS c
        FROM deals d
        JOIN reviews r ON r.deal_id = CAST(d.deal_id AS TEXT) AND r.rating >= 4
...
def recalc_verification(user_id: str) -> bool:
    """Пересчитывает статус верификации. Возвращает True если статус сменился на verified."""
    u = get_user(str(user_id)) or {}
    if not u:
        return False
    already = bool(u.get("is_verified"))
    cnt = _count_successful_deals(str(user_id))
    should_be = cnt >= VERIFICATION_DEALS_REQUIRED
    if should_be and not already:
        try:
            conn = get_conn()
            cur = conn.cursor()
            cur.execute(
                "UPDATE users SET is_verified = TRUE, verified_at = %s WHERE user_id = %s",
            ...
            logger.error(f"recalc_verification set error: {e}")
            return False
    return False


def revoke_verification(user_id: str, reason: str = "") -> None:
    """Снимает верификацию (например, при подтверждённой жалобе)."""
    try:
        conn = get_conn()
        cur = conn.cursor()
        cur.execute(
            "UPDATE users SET is_verified = FALSE, verified_at = %s WHERE user_id = %s",
            (f"revoked:{reason}:{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", str(user_id)),
        )
        conn.commit()
        cur.close()
        conn.close()
    except Exception as e:
        logger.error(f"revoke_verification error: {e}")
...
_PRICES_CACHE = None         # ({sym: 5-tuple}, usd_kzt)
_PRICES_CACHE_TIME = None     # datetime последнего запроса


async def get_realtime_prices():
    """Котировки с Stooq (бесплатно, без ключа). Stooq batch не поддерживает
    spot-металлы — дёргаем 4 параллельных запроса.
    Возвращает ({sym: (name, price_kzt, open_kzt, chp, unit)}, usd_kzt).
    Кэш 120 сек."""
    global _PRICES_CACHE, _PRICES_CACHE_TIME
    if _PRICES_CACHE and _PRICES_CACHE_TIME:
        if (datetime.utcnow() - _PRICES_CACHE_TIME).total_seconds() < 120:
    ...
    async def _fetch_one(session, sym: str, stooq_sym: str) -> Optional[Tuple[str, float, float]]:
        """Возвращает (sym, price_usd, open_usd) или None при ошибке."""
        url = f"https://stooq.com/q/l/?s={stooq_sym}&f=sd2t2ohlcv&h&e=csv"
        try:
            async with session.get(url, timeout=aiohttp.ClientTimeout(total=10)) as r:
                if r.status != 200:
                    logger.warning(f"stooq {stooq_sym}: HTTP {r.status}")
                    return None
                text = await r.text()
        except Exception as e:
            logger.error(f"stooq {stooq_sym} request: {e}")
            return None
        try:
            import csv as _csv
            ...
            if not row:
                logger.warning(f"stooq {stooq_sym}: пустой CSV")
                return None
            close_str = row.get("Close", "")
            open_str = row.get("Open", "")
            if not close_str or close_str == "N/D":
                logger.warning(f"stooq {stooq_sym}: цена N/D (рынок закрыт?)")
                return None
            price_usd = float(close_str)
            open_usd = float(open_str) if open_str and open_str != "N/D" else 0.0
            return (sym, price_usd, open_usd)
        except Exception as e:
            logger.error(f"stooq {stooq_sym} parse: {e}; raw: {text[:200]}")
            return None

    async with aiohttp.ClientSession() as session:
        usd_kzt = await fetch_usd_kzt(session)
        if not usd_kzt:
            logger.error("fetch_usd_kzt вернул 0 — USD/KZT не получен")
            return results, 0
        # Параллельно дёргаем все 4 металла
        import asyncio as _asyncio
        tasks = [_fetch_one(session, sym, stooq_sym) for sym, stooq_sym in stooq_symbols.items()]
        fetched = await _asyncio.gather(*tasks)
        for item in fetched:
            if not item:
                continue
            sym, price_usd, open_usd = item
            if not price_usd:
                continue
            name = metal_names[sym]
            price_kzt_g = round((price_usd / 31.1035) * usd_kzt)
            open_kzt_g = round((open_usd / 31.1035) * usd_kzt) if open_usd else price_kzt_g
            chp = None
            if open_usd and price_usd:
                chp = ((price_usd - open_usd) / open_usd) * 100
            results[sym] = (name, price_kzt_g, open_kzt_g, chp, "г")

    if results:
        _PRICES_CACHE = (results, round(usd_kzt))
        _PRICES_CACHE_TIME = datetime.utcnow()
    return results, round(usd_kzt)
...
def main_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📋 Как это работает", callback_data="menu_how"),
         InlineKeyboardButton("🛡 Безопасность", callback_data="menu_safety")],
        [InlineKeyboardButton("📞 Администратор", callback_data="menu_admin")],
    ])


MENU_BANNER_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "menu_banner.png")
_MENU_BANNER_FILE_ID = None


def _menu_caption(user) -> str:
    user = user or {}
    name = f"{user.get('first_name','')} {user.get('last_name','')}".strip() or "Участник"
    username = (user.get("username") or "").lstrip("@")
    verified = bool(user.get("is_verified"))
    head = f"👤 <b>{name}</b>" + (" ✅" if verified else "")
    lines = [head]
    if username:
        lines.append(f"🆔 @{username}")
    if verified:
        lines.append("✅ Проверенный участник")
    lines.append(f"⭐ Рейтинг: {user.get('rating', 0)} · Отзывов: {user.get('reviews_count', 0)}")
    lines.append("")
...
async def send_main_menu(context, chat_id, user_id):
    """Главное меню: баннер + статистика участника + кнопки. При сбое — текстом."""
    global _MENU_BANNER_FILE_ID
    caption = _menu_caption(get_user(user_id))
    try:
        if _MENU_BANNER_FILE_ID:
            await context.bot.send_photo(
                chat_id=chat_id, photo=_MENU_BANNER_FILE_ID, caption=caption,
                parse_mode="HTML", reply_markup=main_menu_keyboard(),
            )
            return
        with open(MENU_BANNER_PATH, "rb") as f:
            msg = await context.bot.send_photo(
                chat_id=chat_id, photo=f, caption=caption,
                parse_mode="HTML", reply_markup=main_menu_keyboard(),
            )
        if msg and msg.photo:
            _MENU_BANNER_FILE_ID = msg.photo[-1].file_id
    except Exception as e:
        logger.error(f"send_main_menu fallback to text: {e}")
...
WELCOME_CAPTION = (
    "Чтобы подать объявление или купить, пройдите быструю регистрацию.\n\n"
    "Выберите действие ниже 👇"
)
_WELCOME_FILE_ID = None


async def send_welcome(update: Update, reply_markup):
    """Отправляет брендовое приветствие с картинкой. При сбое — обычным текстом."""
    global _WELCOME_FILE_ID
    try:
        if _WELCOME_FILE_ID:
            await update.message.reply_photo(
                photo=_WELCOME_FILE_ID, caption=WELCOME_CAPTION,
                parse_mode="HTML", reply_markup=reply_markup,
            )
            return
        with open(WELCOME_PHOTO_PATH, "rb") as f:
            msg = await update.message.reply_photo(
                photo=f, caption=WELCOME_CAPTION,
                parse_mode="HTML", reply_markup=reply_markup,
            )
        if msg and msg.photo:
            _WELCOME_FILE_ID = msg.photo[-1].file_id
    except Exception as e:
        logger.error(f"send_welcome fallback to text: {e}")
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    refresh_user_meta(update.message.from_user)
    ...
    if user and user.get("status") in ("active",) and user.get("joined") == "yes":
        ...
        if not in_group:
            ...
            await update.message.reply_text(
                "⛔ Вы были исключены из группы QazTreasure.\n\n"
                "Для получения доступа вступите в группу снова.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔓 Вступить в QazTreasure", url=GROUP_LINK)],
                    [InlineKeyboardButton("🏠 Главное меню", callback_data="check_join")],
                ]),
            )
            return

        states[user_id] = {"step": "menu"}
        await send_main_menu(context, update.message.chat_id, user_id)
        return
    ...
    if user and user.get("status") == "active" and user.get("joined") != "yes":
        states[user_id] = {"step": "awaiting_join"}
        await update.message.reply_text(
            "✅ Вы зарегистрированы. Теперь вступите в группу и нажмите «🏠 Главное меню».",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🔓 Вступить в QazTreasure", url=GROUP_LINK)],
                [InlineKeyboardButton("🏠 Главное меню", callback_data="check_join")],
            ]),
...
async def rules_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "4. QazTreasure является информационным посредником.\n"
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(str(update.message.from_user.id)):
        ...
        return
    users_count = db_fetchone("SELECT COUNT(*) AS c FROM users WHERE joined='yes'") or {"c": 0}
    verified_count = db_fetchone("SELECT COUNT(*) AS c FROM users WHERE is_verified = TRUE") or {"c": 0}
    ads_count = db_fetchone("SELECT COUNT(*) AS c FROM ads") or {"c": 0}
    open_complaints = db_fetchone("SELECT COUNT(*) AS c FROM complaints WHERE status = 'open'") or {"c": 0}
    blocked_count = db_fetchone("SELECT COUNT(*) AS c FROM users WHERE status = 'blocked'") or {"c": 0}
    # Реальное число участников группы (через Telegram) — может быть больше чем
    # зарегистрированных в боте: часть вступает по ссылке не открывая бота
    group_count = "—"
    if GROUP_ID:
        try:
            group_count = await context.bot.get_chat_member_count(chat_id=GROUP_ID)
        except Exception as e:
            logger.error(f"get_chat_member_count: {e}")
    await update.message.reply_text(
        "🛠 Админ-панель QazTreasure\n\n"
        f"👥 Участников в группе: {group_count}\n"
        f"🤖 Зарегистрировано в боте: {users_count['c']}\n"
        f"✅ Верифицированных: {verified_count['c']}\n"
        f"📦 Объявления: {ads_count['c']}\n"
        f"⚠️ Жалоб открыто: {open_complaints['c']}\n"
        f"🚫 Заблокированных: {blocked_count['c']}",
        reply_markup=InlineKeyboardMarkup([
            ...
            [InlineKeyboardButton("🗑 Удалить объявления", callback_data="admin_delete_menu")],
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    data = query.data
    refresh_user_meta(query.from_user)

    # ─── Подача жалобы (пользователь) ───────────────────────────────────────
    if data == "complain_pick_deal" or data.startswith("complain_pick_page_"):
        # Список сделок юзера для выбора кому пожаловаться
        page = 0
        if data.startswith("complain_pick_page_"):
            try:
                page = int(data.replace("complain_pick_page_", ""))
            except ValueError:
                page = 0
        per_page = 10
        deals = db_fetchall(
            ...
            LIMIT %s OFFSET %s
            """,
            (user_id, user_id, per_page + 1, page * per_page),
        )
        if not deals and page == 0:
            await context.bot.send_message(
                ...
                text=(
                    "🚨 Подать жалобу\n\n"
                    "У вас пока нет сделок, на которые можно оставить жалобу.\n\n"
                    "Жаловаться можно только после того как:\n"
                    "• вы купили чьё-то объявление\n"
                    "• или у вас купили объявление"
                ),
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 В профиль", callback_data="menu_profile")]]),
            )
            return
        has_more = len(deals) > per_page
        deals = deals[:per_page]
        rows = []
        for d in deals:
            is_buyer = str(d.get("buyer_id")) == user_id
            other_id = d.get("seller_id") if is_buyer else d.get("buyer_id")
            other = get_user(other_id) or {}
            other_username = other.get("username") or other_id or "—"
            ad_id = d.get("ad_id") or "—"
            created = (d.get("created_at") or "")[:10]  # YYYY-MM-DD
            role_emoji = "🛒" if is_buyer else "💰"
            label = f"{role_emoji} @{other_username} — Лот №{ad_id} — {created}"
            rows.append([InlineKeyboardButton(label[:60], callback_data=f"complain_pick_{d.get('deal_id')}")])
        # Пагинация
        nav = []
        if page > 0:
            nav.append(InlineKeyboardButton("◀ Назад", callback_data=f"complain_pick_page_{page-1}"))
        if has_more:
            nav.append(InlineKeyboardButton("Дальше ▶", callback_data=f"complain_pick_page_{page+1}"))
        if nav:
            rows.append(nav)
        rows.append([InlineKeyboardButton("🔙 В профиль", callback_data="menu_profile")])
        await context.bot.send_message(
            chat_id=chat_id,
            text=(
                "🚨 Подать жалобу\n\n"
                "Выберите участника, с которым была сделка:\n\n"
                "🛒 = вы покупали   💰 = у вас покупали"
            ),
            reply_markup=InlineKeyboardMarkup(rows),
        )
        return

    if data.startswith("complain_pick_") and not data.startswith("complain_pick_page_") and data != "complain_pick_deal":
        # Формат: complain_pick_<deal_id> — пользователь выбрал конкретную сделку
        try:
            deal_id = int(data.replace("complain_pick_", ""))
        except ValueError:
            return
        deal = db_fetchone("SELECT * FROM deals WHERE deal_id = %s", (deal_id,))
        if not deal:
            await context.bot.send_message(chat_id=chat_id, text="Сделка не найдена.")
            return
        if user_id not in (str(deal.get("seller_id")), str(deal.get("buyer_id"))):
            await context.bot.send_message(chat_id=chat_id, text="Это не ваша сделка.")
            return
        # Определяем на кого жалуемся
        target_id = str(deal.get("seller_id")) if str(deal.get("buyer_id")) == user_id else str(deal.get("buyer_id"))
        ad_id = str(deal.get("ad_id") or "")
        # Запускаем стандартный флоу подачи жалобы
        states[user_id] = {
            "step": "complaint_category",
            "complaint_seller_id": target_id,
            "complaint_ad_id": ad_id,
            "complaint_deal_id": str(deal_id),
        }
        target = get_user(target_id) or {}
        kb = [[InlineKeyboardButton(label, callback_data=f"complaint_cat_{key}")] for key, label in COMPLAINT_CATEGORIES.items()]
        kb.append([InlineKeyboardButton("❌ Отмена", callback_data="complaint_cancel")])
        await context.bot.send_message(
            chat_id=chat_id,
            text=(
                f"🚨 Жалоба на @{target.get('username','')}\n"
                f"Лот: №{ad_id or '—'} | Сделка: №{deal_id}\n\n"
                "На что жалуетесь?"
            ),
    ...
    if data.startswith("complaint_start_"):
        # Формат: complaint_start_<seller_id>_<ad_id>_<deal_id>
        parts = data.replace("complaint_start_", "").split("_")
        if len(parts) < 3:
            return
        seller_id, ad_id, deal_id = parts[0], parts[1], parts[2]
        states[user_id] = {
            "step": "complaint_category",
            "complaint_seller_id": seller_id,
            "complaint_ad_id": ad_id,
            "complaint_deal_id": deal_id,
        }
        kb = [[InlineKeyboardButton(label, callback_data=f"complaint_cat_{key}")] for key, label in COMPLAINT_CATEGORIES.items()]
        kb.append([InlineKeyboardButton("❌ Отмена", callback_data="complaint_cancel")])
        await context.bot.send_message(
            chat_id=chat_id,
            text="🚨 Жалоба на продавца\n\nНа что жалуетесь?",
            reply_markup=InlineKeyboardMarkup(kb),
        )
        return

    if data.startswith("complaint_cat_"):
        cat = data.replace("complaint_cat_", "")
        if cat not in COMPLAINT_CATEGORIES:
            return
        st = states.get(user_id) or {}
        st["complaint_category"] = cat
        st["step"] = "complaint_text"
        st["complaint_media"] = []
        states[user_id] = st
        await context.bot.send_message(
            chat_id=chat_id,
            text=(
                f"Категория: {COMPLAINT_CATEGORIES[cat]}\n\n"
                "Опишите ситуацию текстом. После описания вы сможете приложить до 3 скриншотов.\n\n"
                "Чтобы пропустить описание — напишите /skip"
            ),
        )
        return
    ...
    if data == "complaint_send":
        st = states.get(user_id) or {}
        if st.get("step") != "complaint_review":
            return
        cid = save_complaint({
            "ad_id": st.get("complaint_ad_id"),
            "deal_id": st.get("complaint_deal_id"),
            "from_user_id": user_id,
            "seller_id": st.get("complaint_seller_id"),
            "category": st.get("complaint_category"),
            "description": st.get("complaint_text", ""),
            "media_file_ids": st.get("complaint_media", []),
        })
        states[user_id] = {"step": "menu"}
        if not cid:
            await context.bot.send_message(chat_id=chat_id, text="⚠️ Не удалось отправить жалобу. Попробуйте позже.")
            return
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"✅ Жалоба №{cid} отправлена админу. С вами свяжутся в течение 24 часов.",
            reply_markup=main_menu_keyboard(),
        )
        # Уведомление админам в личку
        complainant = get_user(user_id) or {}
        target = get_user(st.get("complaint_seller_id")) or {}
        cat_label = COMPLAINT_CATEGORIES.get(st.get("complaint_category", ""), "—")
        notif = (
            f"🚨 Новая жалоба #{cid}\n"
            f"Категория: {cat_label}\n"
            f"От: @{complainant.get('username','')} (id {user_id})\n"
            f"На: @{target.get('username','')} (id {st.get('complaint_seller_id')})\n"
            f"Лот: №{st.get('complaint_ad_id') or '—'} | Сделка: №{st.get('complaint_deal_id') or '—'}\n\n"
            f"Описание:\n{st.get('complaint_text', '') or '—'}"
        )
        view_kb = InlineKeyboardMarkup([[InlineKeyboardButton("📄 Открыть жалобу", callback_data=f"cmpl_view_{cid}")]])
        admin_ids = [ADMIN_CHAT_ID]
        try:
            if ADMIN_CHAT_ID_2:
                admin_ids.append(ADMIN_CHAT_ID_2)
        except NameError:
            pass
        for aid in admin_ids:
            try:
                await context.bot.send_message(chat_id=int(aid), text=notif, reply_markup=view_kb)
                for mid in (st.get("complaint_media") or [])[:3]:
                    try:
                        await context.bot.send_photo(chat_id=int(aid), photo=mid)
                    except Exception:
                        pass
            except Exception as e:
    ...
    if data == "complaint_cancel":
        states[user_id] = {"step": "menu"}
        await context.bot.send_message(chat_id=chat_id, text="❌ Подача жалобы отменена.", reply_markup=main_menu_keyboard())
        return

    if data == "complaint_skip_media":
        st = states.get(user_id) or {}
        if st.get("step") != "complaint_media":
            return
        # Переход к подтверждению
        st["step"] = "complaint_review"
        states[user_id] = st
        cat_label = COMPLAINT_CATEGORIES.get(st.get("complaint_category", ""), "—")
        target = get_user(st.get("complaint_seller_id")) or {}
        text = (
            f"🚨 Подтверждение жалобы\n\n"
            f"На: @{target.get('username','')}\n"
            f"Лот: №{st.get('complaint_ad_id') or '—'}\n"
            f"Категория: {cat_label}\n"
            f"Фото: {len(st.get('complaint_media') or [])} шт.\n\n"
            f"Описание:\n{st.get('complaint_text', '') or '—'}"
        )
        await context.bot.send_message(
            chat_id=chat_id,
            text=text,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Отправить", callback_data="complaint_send")],
                [InlineKeyboardButton("❌ Отмена", callback_data="complaint_cancel")],
            ]),
        )
        return
    ...
    if data == "about":
        await context.bot.send_message(
            chat_id=chat_id,
            text="ℹ️ QazTreasure - доверенное сообщество участников рынка драгоценных металлов Казахстана.\n\n"
                 "Здесь вы можете:\n"
    ...
    if data.startswith("reg_city_"):
        choice = data[len("reg_city_"):]
        if choice == "other":
            states[user_id] = {**(states.get(user_id) or {}), "step": "reg_city_custom"}
            await context.bot.send_message(
                chat_id=chat_id,
                text="Напишите название вашего города:",
            )
            return
        # Конкретный город (Алматы / Астана / Шымкент)
        await _finalize_registration(query, context, user_id, choice)
        return
    ...
    if data == "reg_start":
        ...
        if not username:
            ...
            return
        # Начало многошаговой регистрации: имя → телефон → город
        states[user_id] = {
            "step": "reg_name",
            "reg_username": username,
            "reg_first_name": query.from_user.first_name or "",
            "reg_last_name": query.from_user.last_name or "",
        }
        await context.bot.send_message(
            chat_id=chat_id,
            text=(
                "1️⃣ <b>Как к вам обращаться?</b>\n\n"
                "Напишите ваше имя — оно будет использоваться "
                "в карточках лотов и при общении с покупателями."
            ),
            parse_mode="HTML",
        )
        return
    ...
    if data == "check_join":
        ...
        if ok:
            already_joined = (get_user(user_id) or {}).get("joined") == "yes"
            update_user_field(user_id, "joined", "yes")
            states[user_id] = {"step": "menu"}
            # Однократное welcome-сообщение с правилами безопасности
            if not already_joined:
                try:
                    await context.bot.send_message(
                        chat_id=chat_id,
                        text=(
                            "🛡 <b>Несколько слов о безопасности</b>\n\n"
                            "QazTreasure — это закрытая площадка с системой репутации и верификации. "
                            "Прежде чем начать, важно знать главное:\n\n"
                            f"<b>✅ Проверенный участник</b> — статус, который участник получает после "
                            f"{VERIFICATION_DEALS_REQUIRED}+ успешных сделок с положительными оценками. "
                            "Иконка ✅ возле ника = надёжный продавец/покупатель.\n\n"
                            "<b>Базовые правила безопасной сделки:</b>\n"
                            "• Дорогие лоты — только личная встреча в публичном месте\n"
                            "• Не платите вперёд незнакомцам без рейтинга\n"
                            "• Просите оригинальные документы (box/papers, сертификаты)\n"
                            "• Слишком низкая цена = повод насторожиться\n\n"
                            "<b>Что-то пошло не так?</b> 🚨 Подать жалобу — в разделе 👤 Мой профиль. "
                            "Админ разбирает обращения в течение 24 часов.\n\n"
                            "Полный гид — в кнопке 🛡 Безопасность главного меню."
                        ),
                        parse_mode="HTML",
                    )
                except Exception as e:
                    logger.error(f"welcome safety: {e}")
            await send_main_menu(context, chat_id, user_id)
        else:
            await context.bot.send_message(
                chat_id=chat_id,
                text="🔒 Сначала вступите в группу — без этого главное меню недоступно.\n\nВступите и снова нажмите «🏠 Главное меню».",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("🔓 Вступить в QazTreasure", url=GROUP_LINK)],
                    [InlineKeyboardButton("🏠 Главное меню", callback_data="check_join")],
                ]),
    ...
    if data == "menu_profile":
        ...
        if not user:
            ...
            return
        badge = " ✅" if user.get("is_verified") else ""
        verified_line = "\n✅ Проверенный участник" if user.get("is_verified") else ""
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"👤 Профиль\n\n{user.get('first_name','')} {user.get('last_name','')}\n"
                 f"@{user.get('username','')}{badge}\n"
            ...
            reply_markup=InlineKeyboardMarkup([
                ...
                [InlineKeyboardButton("⭐ Моя репутация", callback_data="profile_rep")],
                [InlineKeyboardButton("🚨 Подать жалобу", callback_data="complain_pick_deal")],
                [InlineKeyboardButton("🔙 Главное меню", callback_data="back_menu")],
            ]),
        )
        return
    ...
    if data == "profile_ads":
        ...
        await context.bot.send_message(
            chat_id=chat_id,
            text=f"📦 <b>Мои объявления</b> ({len(ads)}):",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(rows),
        )
        return
    ...
    if data == "menu_rep" or data == "profile_rep":
        user = get_user(user_id) or {}
        rating = user.get("rating", 0)
        reviews = int(user.get("reviews_count", 0) or 0)
        if user.get("is_verified"):
            status_line = "Статус: ✅ Проверенный участник"
        else:
            done = _count_successful_deals(user_id)
            left = max(0, VERIFICATION_DEALS_REQUIRED - done)
            if left > 0:
                status_line = (
                    f"Статус: 🔘 Обычный участник\n"
                    f"До статуса «✅ Проверенный» осталось: {left} успешных сделок\n"
                    f"(успешных сделок сейчас: {done}/{VERIFICATION_DEALS_REQUIRED})"
                )
            else:
                status_line = "Статус: ✅ Проверенный участник"
        text = (
    ...
    if data == "menu_how":
        await context.bot.send_message(
            ...
            text="📋 Как работает QazTreasure\n\n"
                 ...
                 "🔷 Для покупателя\n"
                 "1. Зайдите в группу QazTreasure\n"
                 ...
                 "После каждой сделки оба участника получают запрос на оценку. Накопленный рейтинг виден в каждом объявлении. После 5+ успешных сделок с положительными оценками вы получаете статус ✅ Проверенный участник — подробнее в разделе 🛡 Безопасность.\n\n"
                 "🔷 Цены на металлы\n"
                 "В боте — кнопка 📊 Цены на металлы с актуальным курсом золота, серебра, платины и палладия в KZT за грамм (LBMA + Нацбанк), по запросу.\n\n"
                 "⚖️ Правовая оговорка\n"
                 "QazTreasure - платформа-посредник. Мы соединяем продавцов и покупателей, но не являемся стороной сделок. Безопасность участников - наш приоритет.",
            reply_markup=main_menu_keyboard(),
        )
        return
    ...
    if data == "menu_safety":
        await context.bot.send_message(
            ...
            text=(
                ...
                "Безопасность участников — приоритет №1 QazTreasure. "
                "Ниже — правила площадки и советы как защитить себя от мошенников.\n\n"

                "═══════════════════════════\n"
                "✅ Что такое «Проверенный участник»\n"
                "═══════════════════════════\n\n"
                "Это автоматический статус доверия. Он показывает что человек "
                "уже провёл несколько успешных сделок на платформе и заслужил репутацию.\n\n"
                f"<b>Как получить:</b>\n"
                f"• Завершить минимум {VERIFICATION_DEALS_REQUIRED} сделок\n"
                "• Получить положительную оценку (4 или 5 ⭐) хотя бы от одной стороны в каждой сделке\n"
                "• Не иметь подтверждённых жалоб\n\n"
                "<b>Что даёт статус:</b>\n"
                "• Иконка ✅ возле ника во всех ваших объявлениях\n"
                "• Покупатели видят что вы надёжный продавец и охотнее идут на сделку\n\n"
                "<b>Как теряется:</b>\n"
                "При подтверждённой админом жалобе статус снимается. Можно заработать заново "
                "после новых успешных сделок без нарушений.\n\n"

                "═══════════════════════════\n"
                "⚠️ Как защитить себя от мошенников\n"
                "═══════════════════════════\n\n"
                "1. <b>Смотрите рейтинг и статус</b>. Бэдж ✅ + рейтинг 4.5+ и 5+ сделок = "
                "надёжный участник. Новички без сделок — повышенный риск.\n\n"
                "2. <b>Проверяйте товар лично</b>. Для дорогих лотов (часы, монеты, "
                "украшения от 300 000 ₸) — только личная встреча в публичном месте. "
                "Никаких «отправлю курьером после предоплаты».\n\n"
                "3. <b>Не платите вперёд незнакомцам</b>. Полная предоплата — главная схема "
                "мошенников. Договаривайтесь на оплату при встрече или часть наличными при получении.\n\n"
                "4. <b>Просите документы</b>. Для часов — оригинальные коробка и бумаги (box/papers). "
                "Для монет и слитков — сертификат. Для украшений — пробу на изделии. "
                "Если документов нет — цена должна быть ниже рыночной.\n\n"
                "5. <b>Сверяйте серийные номера</b>. На часах — серийный номер на корпусе "
                "должен совпадать с тем что в документах. На монетах — год выпуска и проба.\n\n"
                "6. <b>Не покупайте по «слишком хорошей» цене</b>. Если Rolex предлагают на 40% дешевле "
                "рынка — почти всегда подделка или ворованное.\n\n"

                "═══════════════════════════\n"
                "🚨 Если столкнулись с проблемой\n"
                "═══════════════════════════\n\n"
                "Нажмите 🚨 Подать жалобу в разделе 👤 Мой профиль. Выберите "
                "сделку и опишите ситуацию — админ свяжется с вами в течение 24 часов и разберёт."
            ),
            parse_mode="HTML",
    ...
    if data == "admin_users":
        if not is_admin(user_id):
            await context.bot.send_message(chat_id=chat_id, text="⛔ Доступно только владельцу.")
            return
        rows = db_fetchall("SELECT user_id, first_name, last_name, username, status FROM users WHERE joined='yes' ORDER BY registered_at DESC LIMIT 20")
        if not rows:
    ...
    if data == "admin_complaints" or data.startswith("cmpl_filter_") or data.startswith("cmpl_page_"):
        if not is_admin(user_id):
            await context.bot.send_message(chat_id=chat_id, text="⛔ Доступно только владельцу.")
            return
        # Определяем фильтр и страницу
        status_filter = "open"  # по умолчанию показываем открытые
        page = 0
        if data.startswith("cmpl_filter_"):
            status_filter = data.replace("cmpl_filter_", "")
            page = 0
        elif data.startswith("cmpl_page_"):
            parts = data.replace("cmpl_page_", "").split("_")
            if len(parts) == 2:
                status_filter, page_str = parts[0], parts[1]
                try:
                    page = int(page_str)
                except ValueError:
                    page = 0
        per_page = 5
        total = count_complaints(status_filter)
        total_all = count_complaints(None)
        open_n = count_complaints("open")
        review_n = count_complaints("in_review")
        resolved_n = count_complaints("resolved") + count_complaints("rejected")
        items = list_complaints(status_filter, limit=per_page, offset=page * per_page)
        header = (
            f"⚠️ Жалобы (всего: {total_all} | открытых: {open_n})\n"
            f"Фильтр: {COMPLAINT_STATUSES.get(status_filter, status_filter)} — {total} шт.\n"
        )
        if not items:
            text = header + "\nПусто."
        else:
            lines = [header]
            for c in items:
                cat = COMPLAINT_CATEGORIES.get(c.get("category", ""), c.get("reason", "—"))
                st_emoji = COMPLAINT_STATUSES.get(c.get("status", ""), "").split(" ")[0]
                lines.append(
                    f"\n#{c.get('complaint_id')} {st_emoji} {c.get('created_at','')[:16]}\n"
                    f"{cat}\nЛот: №{c.get('ad_id') or '—'}"
                )
            text = "\n".join(lines)
        # Кнопки фильтра + карточки + пагинация
        filter_row = [
            InlineKeyboardButton(f"🔴 Открытые ({open_n})", callback_data="cmpl_filter_open"),
            InlineKeyboardButton(f"🟡 В работе ({review_n})", callback_data="cmpl_filter_in_review"),
        ]
        filter_row2 = [
            InlineKeyboardButton(f"🟢 Закрытые ({resolved_n})", callback_data="cmpl_filter_resolved"),
            InlineKeyboardButton(f"📋 Все ({total_all})", callback_data="cmpl_filter_all"),
        ]
        keyboard = [filter_row, filter_row2]
        for c in items:
            keyboard.append([InlineKeyboardButton(f"📄 Открыть #{c.get('complaint_id')}", callback_data=f"cmpl_view_{c.get('complaint_id')}")])
        # пагинация
        nav = []
        if page > 0:
            nav.append(InlineKeyboardButton("◀ Назад", callback_data=f"cmpl_page_{status_filter}_{page-1}"))
        if (page + 1) * per_page < total:
            nav.append(InlineKeyboardButton("Дальше ▶", callback_data=f"cmpl_page_{status_filter}_{page+1}"))
        if nav:
            keyboard.append(nav)
        keyboard.append([InlineKeyboardButton("🔙 В админку", callback_data="menu_admin")])
        await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=InlineKeyboardMarkup(keyboard))
        return

    if data.startswith("cmpl_view_"):
        if not is_admin(user_id):
            await context.bot.send_message(chat_id=chat_id, text="⛔ Доступно только владельцу.")
            return
        try:
            cid = int(data.replace("cmpl_view_", ""))
        except ValueError:
            return
        c = get_complaint(cid)
        if not c:
            await context.bot.send_message(chat_id=chat_id, text="Жалоба не найдена.")
            return
        complainant = get_user(c.get("from_user_id")) or {}
        target = get_user(c.get("seller_id")) or {}
        cat = COMPLAINT_CATEGORIES.get(c.get("category", ""), c.get("reason", "—"))
        status_label = COMPLAINT_STATUSES.get(c.get("status", ""), c.get("status", ""))
        text = (
            f"⚠️ Жалоба #{c.get('complaint_id')}\n"
            f"Статус: {status_label}\n"
            f"Категория: {cat}\n"
            f"От: @{complainant.get('username','')} (id {c.get('from_user_id')})\n"
            f"На: @{target.get('username','')} (id {c.get('seller_id')})\n"
            f"Лот: №{c.get('ad_id') or '—'} | Сделка: №{c.get('deal_id') or '—'}\n"
            f"Дата: {c.get('created_at','')}\n\n"
        ...
        try:
            import json as _json
            media_ids = _json.loads(c.get("media_file_ids") or "[]")
        except Exception:
            media_ids = []
        for mid in media_ids[:3]:
            try:
                await context.bot.send_photo(chat_id=chat_id, photo=mid)
            except Exception:
                pass
        keyboard = []
        if c.get("status") in ("open", "in_review"):
            keyboard = [
                [InlineKeyboardButton("📨 Запросить детали у продавца", callback_data=f"cmpl_act_ask_{c.get('complaint_id')}")],
                [InlineKeyboardButton("🚫 Заблокировать продавца", callback_data=f"cmpl_act_block_{c.get('complaint_id')}")],
                [InlineKeyboardButton("🗑 Снять объявление", callback_data=f"cmpl_act_remove_{c.get('complaint_id')}")],
                [InlineKeyboardButton("✅ Закрыть (обоснована)", callback_data=f"cmpl_act_resolve_{c.get('complaint_id')}")],
                [InlineKeyboardButton("❌ Отклонить (необоснована)", callback_data=f"cmpl_act_reject_{c.get('complaint_id')}")],
            ]
        keyboard.append([InlineKeyboardButton("🔙 К списку жалоб", callback_data="admin_complaints")])
        await context.bot.send_message(chat_id=chat_id, text=text, reply_markup=InlineKeyboardMarkup(keyboard))
        return

    if data.startswith("cmpl_act_"):
        if not is_admin(user_id):
            await context.bot.send_message(chat_id=chat_id, text="⛔ Доступно только владельцу.")
            return
        # Формат: cmpl_act_<action>_<id>
        rest = data.replace("cmpl_act_", "")
        # action может быть из нескольких слов (например "remove"), id — последняя часть
        ...
        if not c:
            await context.bot.send_message(chat_id=chat_id, text="Жалоба не найдена.")
            return
        target_id = c.get("seller_id")
        ad_id = c.get("ad_id")
        now = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        if action == "ask":
            update_complaint(cid, {"status": "in_review", "admin_notes": f"Запрошены детали у продавца ({now})"})
            try:
                await context.bot.send_message(
                    chat_id=int(target_id),
                    text=f"⚠️ На ваше объявление #{ad_id} поступила жалоба. Админ запросил подробности — ответьте в личку @{ADMIN_USERNAME}.",
                )
                await context.bot.send_message(chat_id=chat_id, text=f"📨 Жалоба #{cid} переведена в 'В работе'. Продавцу отправлен запрос деталей.")
            except Exception as e:
                await context.bot.send_message(chat_id=chat_id, text=f"⚠️ Не удалось уведомить продавца: {e}")
            return
        if action == "block":
            try:
                conn = get_conn(); cur = conn.cursor()
                cur.execute("UPDATE users SET status = 'blocked' WHERE user_id = %s", (str(target_id),))
                conn.commit(); cur.close(); conn.close()
            except Exception as e:
                logger.error(f"block user error: {e}")
            revoke_verification(str(target_id), reason=f"complaint_{cid}")
            update_complaint(cid, {"status": "resolved", "resolved_at": now, "admin_notes": f"Продавец заблокирован ({now})"})
            await context.bot.send_message(chat_id=chat_id, text=f"🚫 Пользователь {target_id} заблокирован. Жалоба #{cid} закрыта.")
            return
        if action == "remove":
            if ad_id and str(ad_id).isdigit():
                try:
                    conn = get_conn(); cur = conn.cursor()
                    cur.execute("UPDATE ads SET status = 'removed' WHERE ad_id = %s", (int(ad_id),))
                    conn.commit(); cur.close(); conn.close()
                except Exception as e:
                    logger.error(f"remove ad error: {e}")
            update_complaint(cid, {"status": "resolved", "resolved_at": now, "admin_notes": f"Объявление {ad_id} снято ({now})"})
            await context.bot.send_message(chat_id=chat_id, text=f"🗑 Объявление №{ad_id} снято. Жалоба #{cid} закрыта.")
            return
        if action == "resolve":
            revoke_verification(str(target_id), reason=f"complaint_{cid}")
            update_complaint(cid, {"status": "resolved", "resolved_at": now, "admin_notes": f"Закрыта как обоснованная ({now})"})
            await context.bot.send_message(chat_id=chat_id, text=f"✅ Жалоба #{cid} закрыта как обоснованная. Верификация продавца снята (если была).")
            return
        if action == "reject":
            update_complaint(cid, {"status": "rejected", "resolved_at": now, "admin_notes": f"Отклонена как необоснованная ({now})"})
            await context.bot.send_message(chat_id=chat_id, text=f"❌ Жалоба #{cid} отклонена.")
            return
        return
    ...
    if data == "admin_delete_menu":
        ...
        await context.bot.send_message(
            ...
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🎯 Выбрать конкретное", callback_data="admin_delete_ad")],
                [InlineKeyboardButton("💥 Удалить ВСЕ объявления", callback_data="admin_delete_all_ads")],
                [InlineKeyboardButton("🔙 В админку", callback_data="menu_admin")],
            ]),
        )
        return
    ...
    if data.startswith("buy_confirm_"):
        ...
        # Напоминание об оценке управляется через review_due_at в БД (одноразово).
        # Дублирующий run_once через 3 дня убран, чтобы не отправлять повторно.
        seller_badge = verified_badge(seller_id)
        buyer_badge = verified_badge(user_id)
        await context.bot.send_message(
            chat_id=int(user_id),
            text=f"✅ Контакты продавца:\n{seller.get('first_name','')} {seller.get('last_name','')}\n@{seller.get('username','')}{seller_badge}\n\nСвяжитесь между собой 🤝",
        )
        await context.bot.send_message(
            chat_id=int(seller_id),
            text=f"📩 Покупатель заинтересовался объявлением #{ad_id}:\n{buyer.get('first_name','')} {buyer.get('last_name','')}\n@{buyer.get('username','')}{buyer_badge}\n\nСвяжитесь между собой 🤝",
        )
    ...
    if data.startswith("offer_accept_"):
        ...
        # Напоминание управляется через review_due_at в БД (одноразово).
        # Дублирующий run_once через 3 дня убран.
    ...
    if data.startswith("deal_notdone_"):
        ...
        # Больше не напоминаем — обнуляем due
        try:
            conn = get_conn()
            cur = conn.cursor()
            cur.execute("UPDATE deals SET review_due_at = '' WHERE deal_id = %s", (int(deal_id),))
            conn.commit()
            cur.close()
            conn.close()
        except Exception as e:
            logger.error(f"deal_notdone clear due error: {e}")
        await context.bot.send_message(chat_id=chat_id, text="✅ Понял, сделка отложена. Больше напоминать не буду — если потом захотите оценить, откройте сделку в «Мои объявления».")
        return
    ...
    if data.startswith("rating_"):
        ...
        # Двойная защита: обнуляем due сразу после оценки, чтобы напоминание точно не повторилось
        try:
            conn_d = get_conn(); cur_d = conn_d.cursor()
            cur_d.execute("UPDATE deals SET review_due_at = '' WHERE deal_id = %s", (int(deal_id),))
            conn_d.commit(); cur_d.close(); conn_d.close()
        except Exception as e:
            logger.error(f"clear review_due_at on rating: {e}")
        await context.bot.send_message(chat_id=chat_id, text="✅ Спасибо, оценка сохранена.")
        # Пересчёт верификации обоих участников сделки (рейтинг изменился)
        for uid in (to_user_id, str(deal.get("seller_id")), str(deal.get("buyer_id"))):
            try:
                became_verified = recalc_verification(uid)
                if became_verified:
                    try:
                        await context.bot.send_message(
                            chat_id=int(uid),
                            text=(
                                "🎉 Поздравляем! Вы получили статус ✅ Проверенный участник.\n\n"
                                f"Это значит, что у вас уже {VERIFICATION_DEALS_REQUIRED}+ успешных сделок с положительными оценками. "
                                "Теперь возле вашего ника во всех объявлениях будет стоять иконка ✅ — покупатели сразу видят что вы надёжный продавец."
                            ),
                        )
                    except Exception as e:
                        logger.error(f"notify verified {uid}: {e}")
            except Exception as e:
                logger.error(f"recalc_verification {uid}: {e}")
        return
...
async def publish_ad(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: str):
    u = states.get(user_id, {})
    # Валидация: state мог потеряться (перезапуск бота, таймаут сессии и т.д.)
    # Не публикуем пустые лоты — это плодит карточки с тире вместо данных
    required = {
        "category": "категория",
    ...
    missing = [label for key, label in required.items() if not str(u.get(key, "")).strip()]
    if missing:
        states[user_id] = {"step": "menu"}
        await context.bot.send_message(
            chat_id=chat_id,
            text=(
                "⚠️ Не удалось опубликовать объявление — данные потерялись.\n\n"
                f"Не хватает: {', '.join(missing)}.\n\n"
                "Скорее всего сессия устарела (бот перезапускался или прошло много времени). "
                "Пожалуйста, начните подачу заново через главное меню."
            ),
            reply_markup=main_menu_keyboard(),
        )
        logger.warning(f"publish_ad: пустой state у user_id={user_id}, missing={missing}")
        return
    user = get_user(user_id) or {}
    ...
    seller_deals = int(user.get("reviews_count", 0) or 0)
    # Цена с разделителями: "1 650 000 ₸"
    raw_price = str(u.get("price", "")).strip()
    price_is_num = False
    try:
        price_int = int(raw_price)
        price_fmt = f"{price_int:,}".replace(",", " ") + " ₸"
        price_btn = f"{price_int:,}".replace(",", " ") + " ₸"
        price_is_num = True
    except (ValueError, TypeError):
        price_fmt = raw_price if raw_price else "—"
        price_btn = raw_price or "—"
    # Строка продавца: рейтинг и сделки показываем всегда
    seller_line = f"⭐ {seller_rating} · {seller_deals} сделок"
    # Плашка верификации (только для verified)
    verified_header = (
        "<b>✅ ВЕРИФИЦИРОВАННЫЙ ПРОДАВЕЦ</b>\n"
        "━━━━━━━━━━━━━━━━━━━\n\n"
    ) if user.get("is_verified") else ""
    # Категория (с эмодзи) — берём из самого названия категории, оно уже с эмодзи
    category_label = u.get("category", "")
    # Экранируем HTML-спецсимволы в пользовательском вводе
    import html as _html
    desc_safe = _html.escape(u.get("description", "") or "")
    category_safe = _html.escape(category_label)
    city_safe = _html.escape(u.get("city", "") or "")
    text = (
        f"{verified_header}"
        f"<b>{category_safe}</b>\n\n"
        f"{desc_safe}\n\n"
    ...
    buy_label = f"💰 Купить за {price_btn}" if price_is_num else "💰 Купить / Связаться"
    buttons.append([InlineKeyboardButton(buy_label, url=f"https://t.me/{BOT_USERNAME}?start=buy_{user_id}_{ad_id}")])
    ...
    try:
        # Telegram лимит caption у фото = 1024 символа. У текстового сообщения = 4096.
        # Если caption длиннее лимита — фото отправляем без подписи, а текст с кнопками — отдельным сообщением.
        if u.get("photo_id"):
            if len(text) <= 1024:
                msg = await context.bot.send_photo(
                    chat_id=GROUP_ID, photo=u.get("photo_id"),
                    caption=text, parse_mode="HTML",
                    reply_markup=InlineKeyboardMarkup(buttons), **kwargs,
                )
            else:
                # Фото отдельно, без подписи и без кнопок
                await context.bot.send_photo(
                    chat_id=GROUP_ID, photo=u.get("photo_id"), **kwargs,
                )
                # Текст с кнопками — отдельное сообщение в том же топике
        ...
        # Более информативный текст ошибки
        err_text = str(e)[:200]
        await context.bot.send_message(
            chat_id=chat_id,
            text=(
                "⚠️ Не удалось опубликовать объявление.\n\n"
                f"Причина: <code>{_html.escape(err_text)}</code>\n\n"
                "Возможные причины:\n"
                "• Описание содержит спецсимволы которые ломают форматирование\n"
                "• У бота нет прав публиковать в нужный топик\n"
...
# ─── Регистрация: contact-handler + завершение ─────────────────────────

async def contact_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Обрабатывает поделение телефоном через KeyboardButton(request_contact=True)."""
    if update.message.chat.type != "private":
        return
    ...
    if step != "reg_phone":
        return  # игнорим если не на шаге регистрации телефона
    # Берём номер привязанный к этому Telegram-аккаунту, не произвольный
    if str(contact.user_id) != user_id:
        await update.message.reply_text(
            "⚠️ Передан чужой контакт. Поделитесь именно своим номером "
            "через кнопку внизу."
        )
        return
    ...
    await update.message.reply_text(
        "✅ Телефон получен.\n\n"
        "3️⃣ <b>Шаг 3 из 3 — ваш город</b>\n\n"
        "В каком городе вы находитесь? Это нужно чтобы покупатели видели "
        "где можно встретиться для сделки.",
        parse_mode="HTML",
    ...
    await update.message.reply_text(
        "Выберите ваш город:",
        reply_markup=InlineKeyboardMarkup([
            [InlineKeyboardButton("Алматы", callback_data="reg_city_Алматы")],
            [InlineKeyboardButton("Астана", callback_data="reg_city_Астана")],
            [InlineKeyboardButton("Шымкент", callback_data="reg_city_Шымкент")],
            [InlineKeyboardButton("Другой город", callback_data="reg_city_other")],
        ]),
    )


async def _finalize_registration(update_or_query, context: ContextTypes.DEFAULT_TYPE, user_id: str, city: str):
    """Сохраняем юзера в БД и переводим к шагу вступления в группу."""
    st = states.get(user_id) or {}
    name = st.get("reg_name", "")
    phone = st.get("reg_phone_number", "")
    username = st.get("reg_username", "")
    first_name = st.get("reg_first_name", "")
    last_name = st.get("reg_last_name", "")
    if not all([name, phone, city]):
        # На случай если шаги пропустились
        await context.bot.send_message(
            chat_id=int(user_id),
            text="⚠️ Регистрация не завершена. Начните заново через /start.",
        )
        return
    # Сохраняем в БД
    save_user(user_id, {
    ...
    # Дозаписываем дополнительные поля
    try:
        conn = get_conn()
        cur = conn.cursor()
        now_iso = datetime.utcnow().isoformat()
        cur.execute(
            "UPDATE users SET phone_verified_at=%s, user_city=%s, reg_step=%s WHERE user_id=%s",
            (now_iso, city, "done", user_id),
        )
        conn.commit()
        cur.close()
        conn.close()
    except Exception as e:
        logger.error(f"finalize_registration update: {e}")
    # Уведомление админу — теперь с именем, телефоном и городом
    try:
        await context.bot.send_message(
            chat_id=ADMIN_CHAT_ID,
            text=(
                f"🆕 Новый участник — QazTreasure\n\n"
                f"👤 Имя: {name}\n"
                f"📱 Телефон: {phone}\n"
                f"📍 Город: {city}\n"
                f"🔗 Username: @{username}\n"
    ...
    # Очищаем reg_* поля state, переводим в awaiting_join
    states[user_id] = {"step": "awaiting_join"}
    # Готово — переходим к вступлению в группу
    welcome_text = (
        f"✅ <b>Регистрация завершена!</b>\n\n"
        f"{name}, добро пожаловать в QazTreasure 🇰🇿\n\n"
        f"Ваш профиль:\n"
        f"👤 Имя: {name}\n"
        f"📱 Телефон: {phone}\n"
        f"📍 Город: {city}\n\n"
        f"Осталось вступить в группу:\n"
        f"1️⃣ Нажмите «Вступить в QazTreasure»\n"
        f"2️⃣ Вступите в группу\n"
        f"3️⃣ Вернитесь и нажмите «Главное меню»"
    )
    await context.bot.send_message(
        chat_id=int(user_id),
        text=welcome_text,
...
async def text_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "reg_name":
        name_clean = text[:50].strip()
        if len(name_clean) < 2:
            await update.message.reply_text(
                "Имя слишком короткое. Напишите минимум 2 символа."
            )
            return
        st = states.get(user_id) or {}
        st["reg_name"] = name_clean
        st["step"] = "reg_phone"
        states[user_id] = st
        from telegram import KeyboardButton, ReplyKeyboardMarkup
        await update.message.reply_text(
            f"Приятно познакомиться, {name_clean}!\n\n"
            f"2️⃣ <b>Шаг 2 из 3 — телефон</b>\n\n"
            "Для связи и подтверждения сделок нужен ваш номер.\n\n"
            "Нажмите кнопку «Отправить телефон» внизу — "
            "номер привяжется автоматически.",
            parse_mode="HTML",
    ...
    # ─── Регистрация: шаг 2 — попытка напечатать телефон руками ────────────
    if step == "reg_phone":
        from telegram import KeyboardButton, ReplyKeyboardMarkup
        await update.message.reply_text(
            "⚠️ Номер можно передать только через кнопку "
            "«Отправить телефон» внизу экрана. "
            "Это привяжет ваш Telegram-номер автоматически.",
            reply_markup=ReplyKeyboardMarkup(
                [[KeyboardButton("Отправить телефон", request_contact=True)]],
                resize_keyboard=True, one_time_keyboard=False,
            ),
    ...
    # ─── Регистрация: шаг 3 — ввод кастомного города ───────────────────────
    if step == "reg_city_custom":
        city_clean = text[:50].strip()
        if len(city_clean) < 2:
            await update.message.reply_text("Название города слишком короткое.")
            return
        await _finalize_registration(update, context, user_id, city_clean)
        return

    # Шаг описания жалобы
    if step == "complaint_text":
        st = states.get(user_id) or {}
        if text.lower() == "/skip":
            st["complaint_text"] = ""
        else:
            st["complaint_text"] = text[:1500]
        st["step"] = "complaint_media"
        states[user_id] = st
        await update.message.reply_text(
            "📎 Если есть скриншоты/фото в подтверждение — отправьте до 3 штук.\n\n"
            "Когда закончите или если фото не нужны — нажмите кнопку ниже.",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➡️ Далее (без фото)", callback_data="complaint_skip_media")],
                [InlineKeyboardButton("❌ Отмена", callback_data="complaint_cancel")],
            ]),
        )
        return
    ...
    if step == "awaiting_phone":
        ...
        await context.bot.send_message(
            chat_id=ADMIN_CHAT_ID,
            text=f"🆕 Новый пользователь QazTreasure\n{data.get('first_name','')} {data.get('last_name','')}\n@{username}\n📞 {text}\nID: {user_id}",
        )
        return
    ...
    if step == "ad_description":
        ...
        await update.message.reply_text("💰 Введите цену только цифрами, например 150000:")
        return

    if step == "ad_price":
        clean = text.replace(" ", "").replace("₸", "").strip()
        if not (clean.isascii() and clean.isdigit()) or int(clean) <= 0:
            await update.message.reply_text(
                "⚠️ Цена должна быть только числом, например: 150000.\n"
                "Введите цену цифрами:"
            )
            return
        states[user_id]["price"] = clean
...
# ─── Антиспам объявлений в топике Обсуждения ────────────────────────────────

_DISCUSSIONS_VIOLATIONS: Dict[str, List[float]] = {}  # user_id -> [unix_timestamps]

_AD_PRICE_RE = re.compile(
    # "100 000", "1.500.000" (с разделителями) ИЛИ 5+ цифр подряд ИЛИ "5000$"/"2500 ₸"/"3500 тг"
    r"\b\d{1,3}(?:[\s.,]\d{3})+\b"
    r"|\b\d{5,}\b"
    r"|\b\d{2,}\s*(?:\$|₸|€|₽|тг|kzt|usd|eur|rub|тенге|тыс|млн)\b",
    re.IGNORECASE,
)
_AD_SELL_RE = re.compile(
    r"(прода[юёмта]|продаётся|продается|продажа|на продаж|"
    r"обмен|обменя[юе]|"
    r"торг|торгуюс|"
    r"цена|стоит|стоимост|"
    r"отда[мю]|"
    r"срочно|"
    r"купи у меня|купл[юе]|скупка|скупл[юе]|выкуп|выкупа[юе]|"
    r"б/у|новые и б/у|"
    r"полный комплект|в плёнках|в пленках)",
    re.IGNORECASE,
)
_AD_CURRENCY_RE = re.compile(
    r"(₸|\bтг\b|тенге|kzt|"
    r"\$|usd|долл|"
    r"€|eur|евро|"
    r"₽|rub|руб|"
    r"\bтыс\.|\bмлн\.|\bтыс\b|\bмлн\b)",
    re.IGNORECASE,
)
# Внешние URL (кроме t.me/телеграм) — типичный признак коммерческого поста
_AD_EXTERNAL_URL_RE = re.compile(
    r"https?://(?!(?:t\.me|telegram\.me|telegram\.org))[^\s]+",
    re.IGNORECASE,
)


def _looks_like_ad(text: str, has_media: bool) -> bool:
    """Эвристика: похоже ли сообщение в Обсуждениях на объявление о продаже."""
    text_lower = (text or "").lower()
    sell_matches = _AD_SELL_RE.findall(text_lower)
    has_sell_word = bool(sell_matches)
    has_price_num = bool(_AD_PRICE_RE.search(text_lower))
    has_currency = bool(_AD_CURRENCY_RE.search(text_lower))
    has_external_url = bool(_AD_EXTERNAL_URL_RE.search(text or ""))
    # фото/видео + (любой признак продажи) = объявление
    if has_media and (has_sell_word or has_price_num or has_currency):
        return True
    # текст без медиа: нужны слова продажи + (цена ИЛИ внешняя ссылка ИЛИ ≥2 разных слов продажи)
    if not has_media and has_sell_word:
        if has_price_num or has_currency or has_external_url:
            return True
        # 2+ разных совпадений по словам продажи — типа "покупка и продажа"
        if len(set(sell_matches)) >= 2:
            return True
    return False


async def _delete_warn_msg_job(context: ContextTypes.DEFAULT_TYPE):
    """Удаляет своё предупреждение через N секунд, чтобы не засорять чат."""
...
async def discussions_anti_ad_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Удаляет сообщения похожие на объявления в топике Обсуждения, ведёт счётчик
    нарушений и кикает после 3 нарушений за сутки."""
    msg = update.message
    if not msg or not msg.from_user:
        return
    if msg.chat_id != GROUP_ID:
        return
    # Где живут Обсуждения: в отдельном топике (DISCUSSIONS_TOPIC_ID) или в General (без thread_id)
    if DISCUSSIONS_TOPIC_ID:
        if msg.message_thread_id != DISCUSSIONS_TOPIC_ID:
            return
    else:
        # General-топик: message_thread_id отсутствует или совпадает с базовым (1)
        if msg.message_thread_id not in (None, 1):
            return
    user_id = str(msg.from_user.id)
    if is_admin(user_id):
        return
    text = msg.text or msg.caption or ""
    has_media = bool(msg.photo) or bool(msg.video) or bool(msg.document) or bool(msg.animation)
    if not _looks_like_ad(text, has_media):
        return
    # Удаляем
    try:
        await context.bot.delete_message(chat_id=msg.chat_id, message_id=msg.message_id)
    except Exception as e:
        logger.error(f"anti-ad delete error: {e}")
        return
    # Счётчик нарушений за 24 часа
    import time as _time
    now = _time.time()
    timestamps = _DISCUSSIONS_VIOLATIONS.setdefault(user_id, [])
    timestamps[:] = [t for t in timestamps if now - t < 86400]
    timestamps.append(now)
    count_24h = len(timestamps)
    # Уведомление в чат с авто-удалением через 30 сек
    username = msg.from_user.username
    mention = f"@{username}" if username else f"<a href=\"tg://user?id={user_id}\">пользователь</a>"
    warn_text = (
        f"⚠️ {mention}, объявления в Обсуждениях запрещены.\n"
        f"Подача только через бота → @KAZ_METALSBOT. Сообщение удалено.\n\n"
        f"Нарушений за сутки: {count_24h}/3"
    )
    try:
        warn_send_kwargs = {"chat_id": GROUP_ID, "text": warn_text, "parse_mode": "HTML"}
        if DISCUSSIONS_TOPIC_ID:
            warn_send_kwargs["message_thread_id"] = DISCUSSIONS_TOPIC_ID
        warn_msg = await context.bot.send_message(**warn_send_kwargs)
        if context.application.job_queue:
            context.application.job_queue.run_once(
                _delete_warn_msg_job,
                when=30,
                data={"chat_id": GROUP_ID, "message_id": warn_msg.message_id},
                name=f"anti_ad_warn_{warn_msg.message_id}",
            )
    except Exception as e:
        logger.error(f"anti-ad warn error: {e}")
    # Кик при достижении лимита
    if count_24h >= 3:
        try:
            # ban + unban = kick (юзер может вернуться по новой ссылке, но текущее членство снимается)
            ...
            kick_send_kwargs = {
                "chat_id": GROUP_ID,
                "text": f"🚫 {mention} удалён из группы — 3 нарушения правил за сутки.",
                "parse_mode": "HTML",
            }
            if DISCUSSIONS_TOPIC_ID:
                kick_send_kwargs["message_thread_id"] = DISCUSSIONS_TOPIC_ID
            await context.bot.send_message(**kick_send_kwargs)
            timestamps.clear()
            logger.info(f"anti-ad: kicked user {user_id} after 3 violations")
        except Exception as e:
            logger.error(f"anti-ad kick error: {e}")


async def group_member_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отслеживает изменения членства в группе QazTreasure.
    Когда юзер выходит/кикается — ставим joined='no', чтобы он перестал считаться
    активным в админке. Когда возвращается — joined='yes'."""
    ...
    user_id = str(user.id)
    # Активные статусы — внутри группы
    inside = new_status in ("member", "administrator", "creator", "restricted")
    # Вышел/кикнут/забанен — снаружи
    outside = new_status in ("left", "kicked", "banned")
    try:
        conn = get_conn()
        cur = conn.cursor()
        if inside:
            cur.execute("UPDATE users SET joined='yes' WHERE user_id=%s", (user_id,))
            logger.info(f"group_member: {user_id} joined ({new_status})")
        elif outside:
            cur.execute("UPDATE users SET joined='no' WHERE user_id=%s", (user_id,))
            logger.info(f"group_member: {user_id} left ({new_status})")
        # Параллельно обновим username — раз есть User объект, грех не использовать
        if user.username or user.first_name:
            cur.execute(
                "UPDATE users SET username=%s, first_name=%s, last_name=%s WHERE user_id=%s",
                (user.username or "", user.first_name or "", user.last_name or "", user_id),
            )
        conn.commit()
        cur.close()
        conn.close()
    except Exception as e:
        logger.error(f"group_member_handler db: {e}")
...
async def photo_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    user_id = str(update.message.from_user.id)
    step = states.get(user_id, {}).get("step")
    # Шаг прикрепления фото к жалобе
    if step == "complaint_media":
        st = states.get(user_id) or {}
        media = st.setdefault("complaint_media", [])
        if len(media) >= 3:
            await update.message.reply_text(
                "Уже добавлено 3 фото (максимум). Нажмите «Далее» чтобы продолжить.",
                reply_markup=InlineKeyboardMarkup([
                    [InlineKeyboardButton("➡️ Далее", callback_data="complaint_skip_media")],
                    [InlineKeyboardButton("❌ Отмена", callback_data="complaint_cancel")],
                ]),
            )
            return
        ...
        states[user_id] = st
        await update.message.reply_text(
            f"📎 Фото добавлено ({len(media)}/3). Можете прислать ещё или нажать «Далее».",
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("➡️ Далее", callback_data="complaint_skip_media")],
                [InlineKeyboardButton("❌ Отмена", callback_data="complaint_cancel")],
            ]),
...
async def send_review_prompt(bot, deal: dict) -> None:
    ...
    # Подгружаем лот для описания
    ad = None
    if ad_id and ad_id.isdigit():
        ad = db_fetchone("SELECT * FROM ads WHERE ad_id = %s", (int(ad_id),))
    category = (ad or {}).get("category", "")
    description = (ad or {}).get("description", "")
    price = (ad or {}).get("price", "")
    # Краткое превью описания (макс 120 символов)
    desc_preview = description[:120] + ("…" if len(description) > 120 else "")
    seller = get_user(deal.get("seller_id")) or {}
    buyer = get_user(deal.get("buyer_id")) or {}
    rating_kb = InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{i} ⭐", callback_data=f"rating_{deal_id}_{i}") for i in range(1, 6)]
    ])

    def _build_text(other_username: str) -> str:
        parts = [f"⭐ Оцените сделку №{deal_id}"]
        if category or price:
            parts.append("")
            parts.append(f"Лот №{ad_id}: {category}" + (f" — {price} KZT" if price else ""))
        if desc_preview:
            parts.append(desc_preview)
        parts.append("")
        parts.append(f"Сделка с @{other_username or '—'}")
        parts.append("")
        parts.append("Поставьте оценку от 1 до 5 ⭐:")
        return "\n".join(parts)
    ...
    if deal.get("seller_id") and not deal.get("seller_reviewed"):
        try:
            await bot.send_message(
                chat_id=int(deal.get("seller_id")),
                text=_build_text(buyer.get("username", "")),
    ...
    if deal.get("buyer_id") and not deal.get("buyer_reviewed"):
        try:
            await bot.send_message(
                chat_id=int(deal.get("buyer_id")),
                text=_build_text(seller.get("username", "")),
                reply_markup=rating_kb,
            )
        except Exception as e:
...
async def review_reminder_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    for deal in deals:
        await send_review_prompt(context.bot, deal)
        # Одно напоминание и всё — обнуляем due, чтобы не выбралось снова
        try:
            conn = get_conn()
            cur = conn.cursor()
            cur.execute("UPDATE deals SET review_due_at = '' WHERE deal_id = %s", (deal.get("deal_id"),))
            conn.commit()
            cur.close()
            conn.close()
        except Exception as e:
            logger.error(f"review_reminder clear due error: {e}")
...
# ── Ценовые публикации: утро / полдень / вечер + алерты отклонений ───────────
_MORNING_PRICES: dict = {}   # {sym: (name, price_kzt, unit)}
_MORNING_USD_KZT: int = 0
_ALERTED_TODAY: set = set()  # {sym_up / sym_down}
_ALERTED_TODAY_DATE: Optional[str] = None  # UTC дата последнего сброса (YYYY-MM-DD)
_ALERT_THRESHOLDS = {
    "XAU": 1.0,
    "XAG": 1.0,
    "XPT": 1.0,
...
def _today_str():
    n = datetime.now()
    return f"{n.day} {_MONTHS_RU[n.month]}"


def _fmt_price(name, price, unit, chg=None):
    line = f"{name} — {price:,} ₸ / {unit}"
    if chg is not None:
        s = "+" if chg >= 0 else ""
        line += f"  ({s}{chg:.1f}%)"
    return line
...
async def _get_all_prices():
    """Реальные котировки XAU/XAG/XPT/XPD с goldapi.io в KZT/г."""
    return await get_realtime_prices()


async def morning_prices_job(context: ContextTypes.DEFAULT_TYPE):
    """09:00 Алматы (04:00 UTC) — цены открытия дня."""
    global _MORNING_PRICES, _MORNING_USD_KZT, _ALERTED_TODAY
    if not NEWS_TOPIC_ID or not GROUP_ID:
        return
    try:
        prices, usd_kzt = await _get_all_prices()
    except Exception as e:
        logger.error(f"morning_prices: {e}"); return
    _MORNING_PRICES = {sym: data[1] for sym, data in prices.items()}
    _MORNING_USD_KZT = usd_kzt
    _ALERTED_TODAY = set()
    # Сохраняем в БД чтобы пережить деплой
    try:
        import json as _json
        today_key = f"morning_prices_{datetime.utcnow().strftime('%Y%m%d')}"
        set_setting(today_key, _json.dumps({
            "prices": _MORNING_PRICES, "usd_kzt": usd_kzt
        }))
    except Exception as _e:
        logger.error(f"save morning to db: {_e}")
    order = ["XAU","XAG","XPT","XPD"]
    lines = [f"🌅 Доброе утро · {_today_str()}\n"]
    for sym in order:
        if sym in prices:
            name, price_kzt, open_kzt, chp, unit = prices[sym]
            lines.append(_fmt_price(name, open_kzt, unit))
    lines.append(f"\n💱 USD/KZT: {usd_kzt:,}")
    try:
        await context.bot.send_message(
            chat_id=GROUP_ID, message_thread_id=NEWS_TOPIC_ID, text="\n".join(lines))
        logger.info("morning_prices posted")
    except Exception as e:
        logger.error(f"morning_prices send: {e}")


async def midday_prices_job(context: ContextTypes.DEFAULT_TYPE):
    """14:00 Алматы (09:00 UTC) — полдень."""
    if not NEWS_TOPIC_ID or not GROUP_ID:
        return
    try:
        prices, usd_kzt = await _get_all_prices()
    except Exception as e:
        logger.error(f"midday_prices: {e}"); return
    # Если утренние цены потерялись после деплоя — загружаем из БД
    if not _MORNING_PRICES:
        try:
            import json as _json
            today_key = f"morning_prices_{datetime.utcnow().strftime('%Y%m%d')}"
            saved = get_setting(today_key)
            if saved:
                data = _json.loads(saved)
                _MORNING_PRICES.update({k: int(v) for k,v in data.get("prices",{}).items()})
                _MORNING_USD_KZT = data.get("usd_kzt", 0)
                logger.info("morning prices loaded from DB")
        except Exception as _e:
            logger.error(f"load morning from db: {_e}")
    order = ["XAU","XAG","XPT","XPD"]
    lines = [f"☀️ Полдень · {_today_str()}\n"]
    for sym in order:
        if sym not in prices: continue
        name, price, open_kzt, chp, unit = prices[sym]
        chg = float(chp) if chp is not None else None
        lines.append(_fmt_price(name, price, unit, chg))
    lines.append(f"\n💱 USD/KZT: {usd_kzt:,}")
    try:
        await context.bot.send_message(
            chat_id=GROUP_ID, message_thread_id=NEWS_TOPIC_ID, text="\n".join(lines))
        logger.info("midday_prices posted")
    except Exception as e:
        logger.error(f"midday_prices send: {e}")


async def evening_prices_job(context: ContextTypes.DEFAULT_TYPE):
    """21:00 Алматы (16:00 UTC) — итог дня."""
    if not NEWS_TOPIC_ID or not GROUP_ID:
        return
    try:
        prices, usd_kzt = await _get_all_prices()
    except Exception as e:
        logger.error(f"evening_prices: {e}"); return
    # Если утренние цены потерялись после деплоя — загружаем из БД
    if not _MORNING_PRICES:
        try:
            import json as _json
    ...
    try:
        await context.bot.send_message(
            chat_id=GROUP_ID, message_thread_id=NEWS_TOPIC_ID, text="\n".join(lines))
        logger.info("evening_prices posted")
    except Exception as e:
        logger.error(f"evening_prices send: {e}")


async def deviation_alert_job(context: ContextTypes.DEFAULT_TYPE):
    """Каждые 30 минут — алерт если chp (изменение за день) ≥ порог."""
    global _ALERTED_TODAY, _ALERTED_TODAY_DATE
    if not NEWS_TOPIC_ID or not GROUP_ID:
        logger.warning(f"deviation_alert: skip — NEWS_TOPIC_ID={NEWS_TOPIC_ID}, GROUP_ID={GROUP_ID}")
        return
    # Ежедневный сброс по UTC-дате
    today_utc = datetime.utcnow().strftime("%Y-%m-%d")
    if _ALERTED_TODAY_DATE != today_utc:
        _ALERTED_TODAY = set()
        _ALERTED_TODAY_DATE = today_utc
        logger.info(f"deviation_alert: daily reset for {today_utc}")
    try:
        prices, usd_kzt = await _get_all_prices()
    except Exception as e:
        logger.error(f"deviation_alert: _get_all_prices error: {e}"); return
    if not prices:
        logger.warning("deviation_alert: empty prices — goldapi не вернул данные (проверь GOLDAPI_KEY/лимит)")
        return
    # Лог текущих chp по всем металлам — видно что джоб реально получает
    chp_summary = []
    for sym, (name, price, open_kzt, chp, unit) in prices.items():
        # Fallback: если goldapi не отдал chp, считаем сами из price/open
        if chp is None and open_kzt and price:
            try:
                chp = ((float(price) - float(open_kzt)) / float(open_kzt)) * 100
            except Exception:
                chp = None
        chp_str = f"{float(chp):+.2f}%" if chp is not None else "N/A"
        thr = _ALERT_THRESHOLDS.get(sym, 1.5)
        chp_summary.append(f"{sym} {chp_str} (порог {thr}%)")
    logger.info(f"deviation_alert: {' | '.join(chp_summary)} | already_alerted={sorted(_ALERTED_TODAY)}")
    # Основной цикл
    for sym, (name, price, open_kzt, chp, unit) in prices.items():
        if chp is None and open_kzt and price:
            try:
        ...
        if chp is None:
            continue
        chg = float(chp)
        if abs(chg) < _ALERT_THRESHOLDS.get(sym, 1.5):
            continue
        ...
        sign = "+" if chg > 0 else ""
        text = (
            f"{emoji} <b>{name} {sign}{chg:.1f}%</b> за сегодня\n\n"
            f"Цена: <b>{price:,} ₸</b> / {unit}\n"
            f"USD/KZT: {usd_kzt:,}"
        )
        try:
            await context.bot.send_message(
            ...
            logger.info(f"alert SENT: {name} {sign}{chg:.1f}%")
        except Exception as e:
            logger.error(f"deviation send error: {e}")


async def test_alert_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /test_alert — для админа. Прогоняет deviation_alert_job вручную
    и в личке показывает текущие цены, chp, пороги и сырой ответ Twelvedata."""
    if update.message.chat.type != "private":
        return
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
        return
    await update.message.reply_text("⏳ Тестирую алерт-джоб...")

    # Сначала — отдельно дёргаем Stooq на одном металле, чтобы показать сырой ответ
    diag_lines = ["🔧 <b>Диагностика источника цен (Stooq)</b>\n"]
    test_url = "https://stooq.com/q/l/?s=xauusd&f=sd2t2ohlcv&h&e=csv"
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(test_url, timeout=aiohttp.ClientTimeout(total=15)) as r:
                status = r.status
                raw_text = (await r.text())[:1500]
        diag_lines.append(f"✓ HTTP-статус: {status}")
        diag_lines.append(f"\n<b>Сырой CSV-ответ (первые 1500 символов):</b>\n<pre>{raw_text}</pre>")
    except Exception as e:
        diag_lines.append(f"❌ Не удалось подключиться к stooq.com")
        diag_lines.append(f"Ошибка: <code>{type(e).__name__}: {e}</code>")
        diag_lines.append("\nЗначит Railway блокирует stooq.com. Скажи мне — придумаем план Б.")
        await update.message.reply_text("\n".join(diag_lines), parse_mode="HTML")
        return
    # Также проверим НБРК (USD/KZT)
    try:
        async with aiohttp.ClientSession() as session:
            usd_kzt = await fetch_usd_kzt(session)
        diag_lines.append(f"\n✓ USD/KZT из НБРК: {usd_kzt}")
    except Exception as e:
        diag_lines.append(f"\n❌ НБРК (USD/KZT) недоступен: {type(e).__name__}: {e}")
    await update.message.reply_text("\n".join(diag_lines), parse_mode="HTML")

    # Теперь обычный прогон логики алерта
    try:
        prices, usd_kzt = await _get_all_prices()
    except Exception as e:
        await update.message.reply_text(f"❌ Ошибка получения цен через get_realtime_prices: {e}")
        return
    if not prices:
        await update.message.reply_text(
            "❌ get_realtime_prices вернул пустой результат.\n"
            "Смотри диагностику выше — там видна точная причина."
        )
        return
    # Сводка
    lines = [f"📊 <b>Текущие цены и решения по алертам</b>\n", f"USD/KZT: {usd_kzt:,}\n"]
    will_send = []
    for sym, (name, price, open_kzt, chp, unit) in prices.items():
        if chp is None and open_kzt and price:
            try:
        ...
        thr = _ALERT_THRESHOLDS.get(sym, 1.5)
        if chp is None:
            decision = "⚠️ chp недоступен (нет open от Twelvedata)"
        else:
            abs_chg = abs(float(chp))
            if abs_chg < thr:
                decision = f"⏸ ниже порога {thr}%"
            else:
                direction = "up" if float(chp) > 0 else "down"
                key = f"{sym}_{direction}"
                if key in _ALERTED_TODAY:
                    decision = f"✋ уже отправлен сегодня (ключ {key})"
                else:
                    decision = f"✅ ОТПРАВИТСЯ при следующем прогоне"
                    will_send.append(name)
        lines.append(f"{name}: цена {price:,} ₸/{unit}, chp {chp_str}, порог {thr}%")
        lines.append(f"  → {decision}\n")
    if will_send:
        lines.append(f"\n🚀 При следующем прогоне джоба отправятся: {', '.join(will_send)}")
    else:
        lines.append("\n💤 Сейчас никаких алертов не будет — рынок ниже порогов или уже отправлены.")
    lines.append(f"\nОтправленные сегодня: {sorted(_ALERTED_TODAY) or '—'}")
    await update.message.reply_text("\n".join(lines), parse_mode="HTML")



async def set_morning_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Вручную задаёт утренние цены как базу для сравнения.
    Использование: /set_morning 70500 1185 30400 21100
    (золото серебро платина палладий в KZT/г)"""
    global _MORNING_PRICES, _ALERTED_TODAY
    if update.message.chat.type != "private" or not is_admin(str(update.message.from_user.id)):
        return
    args = context.args
    order = ["XAU","XAG","XPT","XPD"]
    names = {"XAU":"🥇 Золото","XAG":"🥈 Серебро","XPT":"💎 Платина","XPD":"🔩 Палладий"}
    if not args or len(args) != 4:
        await update.message.reply_text(
            "Укажи 4 цены в KZT/г через пробел:\n"
            "/set_morning [золото] [серебро] [платина] [палладий]\n\n"
            "Пример: /set_morning 70500 1185 30400 21100"
        )
        return
    try:
        values = [int(a.replace(",","").replace(" ","")) for a in args]
    except ValueError:
        await update.message.reply_text("⚠️ Только целые числа, например: /set_morning 70500 1185 30400 21100")
        return
    _MORNING_PRICES = {sym: val for sym, val in zip(order, values)}
    _ALERTED_TODAY = set()
    lines = ["✅ Утренние цены сохранены как база:\n"]
    for sym, val in _MORNING_PRICES.items():
        lines.append(f"{names[sym]} — {val:,} ₸ / г")
    lines.append("\nПолдень и вечер будут сравниваться с этими значениями.")
    await update.message.reply_text("\n".join(lines))

async def open_prices_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Публикует цены по времени Алматы (UTC+5).
    Между 12-18: сначала утро (задаёт базу для алертов), потом полдень.
    До 12: только утро. После 18: только вечер."""
    if update.message.chat.type != "private" or not is_admin(str(update.message.from_user.id)):
        return
    await update.message.reply_text("⏳ Получаю цены...")
    try:
        prices, usd_kzt = await get_realtime_prices()
    except Exception as e:
        await update.message.reply_text(f"⚠️ Ошибка: {e}")
        return
    if not prices:
        await update.message.reply_text("⚠️ Не удалось получить цены.")
        return
    now = datetime.utcnow()
    months = {1:"января",2:"февраля",3:"марта",4:"апреля",5:"мая",6:"июня",
              7:"июля",8:"августа",9:"сентября",10:"октября",11:"ноября",12:"декабря"}
    lines = [f"📊 Цены · {now.day} {months[now.month]}\n"]
...
async def sync_membership(context: ContextTypes.DEFAULT_TYPE) -> dict:
    """Проходит по всем пользователям из БД, через getChatMember проверяет
    реальный статус в группе и обновляет колонку joined.
    Возвращает {checked, joined_yes, joined_no, errors}."""
    import asyncio as _asyncio
    if not GROUP_ID:
        return {"checked": 0, "joined_yes": 0, "joined_no": 0, "errors": 0}
    rows = db_fetchall("SELECT user_id FROM users")
    stats = {"checked": 0, "joined_yes": 0, "joined_no": 0, "errors": 0}
    conn = get_conn()
    cur = conn.cursor()
    for r in rows:
        uid = r.get("user_id")
        if not uid:
            continue
        try:
            member = await context.bot.get_chat_member(chat_id=GROUP_ID, user_id=int(uid))
            status = member.status
            # Заодно обновляем username/имена из свежего объекта User
            mu = member.user
            if mu:
                cur.execute(
                    "UPDATE users SET username=%s, first_name=%s, last_name=%s WHERE user_id=%s",
                    (mu.username or "", mu.first_name or "", mu.last_name or "", str(uid)),
                )
            if status in ("member", "administrator", "creator", "restricted"):
                cur.execute("UPDATE users SET joined='yes' WHERE user_id=%s", (str(uid),))
                stats["joined_yes"] += 1
            else:
                cur.execute("UPDATE users SET joined='no' WHERE user_id=%s", (str(uid),))
                stats["joined_no"] += 1
            stats["checked"] += 1
        except Exception as e:
            logger.warning(f"sync_membership: {uid} error: {e}")
            stats["errors"] += 1
        # Пауза чтобы не упереться в лимит Telegram (30 req/sec)
        await _asyncio.sleep(0.1)
    conn.commit()
    cur.close()
    conn.close()
    logger.info(f"sync_membership done: {stats}")
    return stats


async def sync_members_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /sync_members — для админа. Запускает синхронизацию вручную."""
    if update.message.chat.type != "private":
        return
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
        return
    await update.message.reply_text("⏳ Синхронизирую членство в группе... Это займёт ~2-3 секунды на 20 юзеров.")
    stats = await sync_membership(context)
    await update.message.reply_text(
        f"✅ Готово.\n\n"
        f"Проверено: {stats['checked']}\n"
        f"В группе (joined='yes'): {stats['joined_yes']}\n"
        f"Вне группы (joined='no'): {stats['joined_no']}\n"
...
async def startup_setup_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    # Вкладка «Ценовые алерты» отключена — переименование убрано, только снимаем закрепы
    if NEWS_TOPIC_ID:
        try:
            ...
            pass
    # Переименовываем вкладку «Лом и скупка» → «Лом»
    scrap_topic_id = CATEGORY_TOPICS.get("♻️ Лом")
    if scrap_topic_id:
        try:
            await context.bot.edit_forum_topic(
                chat_id=GROUP_ID, message_thread_id=scrap_topic_id, name="Лом"
            )
        except Exception as e:
            logger.error(f"rename scrap topic: {e}")
...
async def get_topic_id_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Отвечает ID текущего топика — напиши эту команду внутри нужного топика в группе."""
    thread_id = getattr(update.message, "message_thread_id", None)
    chat = update.message.chat
    chat_id = update.message.chat_id
    if thread_id:
        await update.message.reply_text(
            f"📌 Topic ID этого топика: <code>{thread_id}</code>\n\n"
            f"Скопируй число и положи в Railway Variables как нужную переменную "
            f"(например DISCUSSIONS_TOPIC_ID), потом сделай Redeploy.",
            parse_mode="HTML",
        )
        return
    if chat.type == "private":
        await update.message.reply_text(
            "ℹ️ Эту команду нужно вызывать в <b>группе</b>, в нужном топике.\n\n"
            "Как:\n"
            "1. Открой группу QazTreasure\n"
            "2. Перейди в нужный топик (например «Обсуждения»)\n"
            "3. Там напиши <code>/get_topic_id</code>\n"
            "4. Бот ответит ID этого топика",
            parse_mode="HTML",
        )
        return
    # В группе, но не в топике (или в General)
    await update.message.reply_text(
        f"ℹ️ Это General-топик (без отдельного thread_id), либо группа без тем.\n"
        f"Chat ID группы: <code>{chat_id}</code>\n\n"
        "Чтобы получить ID конкретного топика — перейди в этот топик и вызови команду оттуда.",
        parse_mode="HTML",
    )


_DISCUSSIONS_PIN_KEY = "discussions_pin_msg_id"


async def pin_discussions_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Публикует и закрепляет приветственное сообщение в топике Обсуждения.
    Если DISCUSSIONS_TOPIC_ID не задан — пинит в General-топик (без thread_id)."""
    if update.message.chat.type != "private":
        return
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
    ...
    if not GROUP_ID:
        ...
        return
    # Где пиним: в отдельном топике (если задан DISCUSSIONS_TOPIC_ID) или в General
    use_thread = bool(DISCUSSIONS_TOPIC_ID)
    text = (
        "💬 <b>Обсуждения QazTreasure</b>\n\n"
        ...
        "⛔ <b>Объявления в этом чате запрещены — будут удалены автоматически.</b>\n\n"
        "Если хочешь что-то купить или продать — в бота 🤖 @KAZ_METALSBOT"
    )
    # Если есть старый закреп — откроем (best effort, ошибки игнорируем)
    old_id = get_setting(_DISCUSSIONS_PIN_KEY)
    if old_id and str(old_id).isdigit():
        try:
            await context.bot.unpin_chat_message(chat_id=GROUP_ID, message_id=int(old_id))
        except Exception:
            pass
        try:
            await context.bot.delete_message(chat_id=GROUP_ID, message_id=int(old_id))
        except Exception:
            pass
    try:
        send_kwargs = {"chat_id": GROUP_ID, "text": text, "parse_mode": "HTML"}
        if use_thread:
            send_kwargs["message_thread_id"] = DISCUSSIONS_TOPIC_ID
        msg = await context.bot.send_message(**send_kwargs)
        # Пинит ВНУТРИ топика — закреп будет виден только в Обсуждениях, не вытесняется глобальными
        await context.bot.pin_chat_message(
            chat_id=GROUP_ID,
            message_id=msg.message_id,
            disable_notification=True,
        )
        set_setting(_DISCUSSIONS_PIN_KEY, str(msg.message_id))
        where = "в топике Обсуждений" if use_thread else "в General-топике (главном)"
        await update.message.reply_text(
            f"✅ Закреплено {where} (message_id={msg.message_id})."
        )
    except Exception as e:
        logger.error(f"pin_discussions error: {e}")
        await update.message.reply_text(
            f"⚠️ Не удалось закрепить: {e}\n\n"
            "Проверь:\n"
            "• Бот в группе как админ с правом «Закреплять сообщения»\n"
            "• DISCUSSIONS_TOPIC_ID указывает на существующий топик\n"
            "• Топик не закрыт для бота"
        )
...
def main():
    ...
    app.add_handler(CommandHandler("pin_discussions", pin_discussions_cmd))
    app.add_handler(CommandHandler("sync_members", sync_members_cmd))
    app.add_handler(CallbackQueryHandler(button_handler))
    # Антиспам в Обсуждениях — ловит группу, до приватных хендлеров
    app.add_handler(MessageHandler(filters.Chat(GROUP_ID) & ~filters.COMMAND, discussions_anti_ad_handler))
    app.add_handler(MessageHandler(filters.PHOTO, photo_handler))
    app.add_handler(MessageHandler(filters.VIDEO, video_handler))
    app.add_handler(MessageHandler(filters.CONTACT, contact_handler))
    app.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, text_handler))
    # Отслеживание входов/выходов из группы — обновляет joined в БД
    app.add_handler(ChatMemberHandler(group_member_handler, ChatMemberHandler.CHAT_MEMBER))

    if GROUP_ID and app.job_queue is not None:
        app.job_queue.run_once(startup_setup_job, when=15)
        app.job_queue.run_once(sync_membership, when=30)  # разовая сверка при старте
        app.job_queue.run_repeating(sync_membership, interval=86400, first=86400)  # затем раз в сутки
    ...
    logger.info("QazTreasure Bot запущен!")
    # allowed_updates=ALL_TYPES обязательно — иначе chat_member события не приходят
