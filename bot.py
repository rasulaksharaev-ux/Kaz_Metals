...
WELCOME_CAPTION = (
    "🏛 Добро пожаловать в <b>KAZ METALS</b> — премиальный маркетплейс "
    "драгоценностей и люкса в Казахстане.\n\n"
    "Здесь покупают и продают напрямую: брендовые часы, монеты, кольца "
    "и украшения Cartier, Rolex, Van Cleef, Bvlgari и сертифицированные камни.\n\n"
    "Без скупок и ломбардов: твой лот видят десятки покупателей, "
    "а цена металла обновляется с биржи в реальном времени.\n\n"
...
async def _finalize_registration(update_or_query, context: ContextTypes.DEFAULT_TYPE, user_id: str, city: str):
    ...
    welcome_text = (
        ...
        f"3️⃣ Вернитесь и нажмите «Я вступил»"
    )
    await context.bot.send_message(
        chat_id=int(user_id),
        text=welcome_text,
