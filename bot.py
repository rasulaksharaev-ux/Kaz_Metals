...
async def discussions_anti_ad_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if msg.chat_id != GROUP_ID:
        return
    if not DISCUSSIONS_TOPIC_ID or msg.message_thread_id != DISCUSSIONS_TOPIC_ID:
        return
    ...
    try:
        warn_msg = await context.bot.send_message(
            chat_id=GROUP_ID,
            message_thread_id=DISCUSSIONS_TOPIC_ID,
            text=warn_text,
            parse_mode="HTML",
        )
...
async def pin_discussions_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Публикует и закрепляет приветственное сообщение ВНУТРИ топика Обсуждения."""
    if update.message.chat.type != "private":
        return
    if not is_admin(str(update.message.from_user.id)):
        await update.message.reply_text("⛔ Только для администратора.")
    ...
    if not GROUP_ID:
        ...
        return
    if not DISCUSSIONS_TOPIC_ID:
        await update.message.reply_text(
            "⚠️ DISCUSSIONS_TOPIC_ID не задан в Railway Variables.\n\n"
            "Чтобы узнать ID — зайди в топик «Обсуждения» в группе и напиши любое сообщение, "
            "ответом на него боту команду /get_topic_id. Затем добавь значение в Variables и сделай Restart."
        )
        return
    text = (
        "💬 <b>Обсуждения KAZ_METALS</b>\n\n"
        "Это место для живого общения. Задавайте вопросы, делитесь наблюдениями, "
        "обсуждайте рынок — золото, серебро, платину, украшения, часы, монеты, инвестиции.\n\n"
        "⛔ <b>Объявления в этом чате запрещены.</b>\n"
        "Подача только через бота → @KAZ_METALSBOT. Сообщения-объявления удаляются автоматически.\n\n"
        "🛡 <b>Безопасность — наш приоритет.</b>\n"
        f"Каждый участник, проведший {VERIFICATION_DEALS_REQUIRED}+ успешных сделок с положительными "
        "оценками, получает статус ✅ Проверенный участник — ищите эту иконку возле ника.\n\n"
        "Полные правила безопасности — в боте: кнопка 🛡 Безопасность.\n"
        "Жалобы на участников — в боте: 👤 Мой профиль → 🚨 Подать жалобу.\n\n"
        "Хочешь купить или продать — заходи в бота 🤖"
    )
    ...
    try:
        ...
        await update.message.reply_text(
            f"✅ Закреплено в топике Обсуждений (message_id={msg.message_id}).\n\n"
            "Теперь закреп виден только внутри этого топика — глобальные закрепы его не вытеснят."
        )
    except Exception as e:
