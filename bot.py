...
def get_ad_display_status(ad_id: str, ad_status: str = "active") -> str:
    """Красивый статус объявления без изменения рабочей логики active/deleted."""
    if str(ad_status) == "deleted":
        return "🗑 Удалено"
    accepted = db_fetchone(
        "SELECT deal_id FROM deals WHERE ad_id = %s AND status IN ('offer_accepted', 'contact_opened') LIMIT 1",
        (str(ad_id),),
    )
    if accepted:
        return "✅ Сделка согласована"
    pending = db_fetchone(
        "SELECT deal_id FROM deals WHERE ad_id = %s AND status = 'offer_pending' LIMIT 1",
        (str(ad_id),),
    )
    if pending:
...
def build_ad_card(
    ...
    """Единый аккуратный формат карточки объявления."""
    description = (description or "").strip()
    city = (city or "Не указан").strip()
    price = (price or "Договорная").strip()
    return (
        f"{category}\n"
        "━━━━━━━━━━━━\n"
        f"📦 Объявление №{ad_id}\n"
        ...
        f"💰 Цена: {price} KZT\n\n"
        f"📝 Описание:\n{description}\n\n"
        f"⭐ Продавец: рейтинг {seller_rating} | отзывов {seller_deals}\n"
        "🔒 Контакты открываются только после согласия сторон."
    )

def build_ad_buttons(seller_id: str, ad_id: str, price: str, media_msg_id: str = "") -> InlineKeyboardMarkup:
    buttons = []
    if media_msg_id:
        buttons.append([InlineKeyboardButton("📸 Фото и видео", url=f"https://t.me/{MEDIA_CHANNEL_USERNAME}/{media_msg_id}")])
    buttons.append([InlineKeyboardButton(f"💰 Купить за {price}", url=f"https://t.me/{BOT_USERNAME}?start=buy_{seller_id}_{ad_id}")])
    buttons.append([InlineKeyboardButton("🤝 Предложить цену", url=f"https://t.me/{BOT_USERNAME}?start=offer_{seller_id}_{ad_id}")])
    return InlineKeyboardMarkup(buttons)


async def refresh_ad_card_in_group(context: ContextTypes.DEFAULT_TYPE, ad_id: str) -> None:
    """Обновляет карточку объявления в группе, чтобы статус был актуальным."""
    try:
        ad = db_fetchone("SELECT * FROM ads WHERE ad_id = %s", (int(ad_id),)) if str(ad_id).isdigit() else None
        if not ad or not ad.get("message_id"):
            return
        seller = get_user(str(ad.get("user_id"))) or {}
        status = get_ad_display_status(str(ad.get("ad_id")), str(ad.get("status", "active")))
        card_text = build_ad_card(
            str(ad.get("ad_id")),
            ad.get("category", ""),
            ad.get("description", ""),
            ad.get("price", ""),
            ad.get("city", ""),
            seller.get("rating", 0),
            seller.get("reviews_count", 0),
            status,
        )
        reply_markup = build_ad_buttons(
            str(ad.get("user_id")),
            str(ad.get("ad_id")),
            str(ad.get("price", "")),
            str(ad.get("media_message_id", "")),
        )
        if ad.get("photo_id"):
            await context.bot.edit_message_caption(
                chat_id=GROUP_ID,
                message_id=int(ad.get("message_id")),
                caption=card_text,
        ...
        logger.warning(f"refresh_ad_card_in_group skipped: {e}")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_how":
        await context.bot.send_message(
            ...
            text="📋 Как работает KAZMETALS\n\n"
                 ...
                 "• Проверяйте товар и документы до оплаты.\n"
                 "• Не переводите предоплату незнакомым участникам.\n"
                 "• Встречайтесь в безопасном общественном месте.\n"
                 "• Фиксируйте договоренности в переписке.\n"
                 "• При сомнениях обращайтесь к администратору.\n\n"
    ...
    if data.startswith("buy_confirm_"):
        ...
        await refresh_ad_card_in_group(context, ad_id)
        await context.bot.send_message(
            chat_id=int(user_id),
    ...
    if data.startswith("offer_accept_"):
        ...
        await refresh_ad_card_in_group(context, str(deal.get("ad_id", "")))
...
async def publish_ad(context: ContextTypes.DEFAULT_TYPE, chat_id: int, user_id: str):
    ...
    reply_markup = build_ad_buttons(user_id, str(ad_id), u.get("price", ""), media_msg_id)

    topic_id = CATEGORY_TOPICS.get(u.get("category", ""))
    kwargs = {"message_thread_id": topic_id} if topic_id else {}
    try:
        if u.get("photo_id"):
            msg = await context.bot.send_photo(chat_id=GROUP_ID, photo=u.get("photo_id"), caption=text, reply_markup=reply_markup, **kwargs)
        else:
            msg = await context.bot.send_message(chat_id=GROUP_ID, text=text, reply_markup=reply_markup, **kwargs)
