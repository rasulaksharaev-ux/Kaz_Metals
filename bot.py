...
# ── RSS-источники ──────────────────────────────────────────────────────────
# Цель: освещать ВЕСЬ рынок, а не только золото — украшения, часы, камни,
# монеты, слитки, серебро/платину, скупку/аффинаж. Баланс категорий
# дополнительно гарантируется квотами при отборе (см. _balance_categories).
from urllib.parse import quote as _urlquote


def _gnews(query: str, gl: str = "RU") -> str:
    """Google News RSS на русском (по умолчанию RU)."""
    return (
        f"https://news.google.com/rss/search?q={_urlquote(query)}"
        f"&hl=ru&gl={gl}&ceid={gl}:ru"
    )


def _gnews_en(query: str) -> str:
    """Google News RSS на английском (США) — для международного контента."""
    return (
        f"https://news.google.com/rss/search?q={_urlquote(query)}"
...
_RSS_FEEDS = [
    ...
    # ── Казахстан: site-запросы в Google News (достают zakon.kz, dknews.kz, kursiv.kz)
    # Именно так находим статьи типа «рейтинг производителей золота» и
    # «казахстанские учёные разработали технологию» — интересное для читателя ──
    (_gnews("site:zakon.kz золото серебро металл ювелир монеты", "KZ"),   "google.news"),
    (_gnews("site:dknews.kz золото металл технология добыча",    "KZ"),   "google.news"),
    (_gnews("site:kursiv.kz золото серебро металл",              "KZ"),   "google.news"),
    (_gnews("Казахстан золото добыча производство рейтинг технология", "KZ"), "google.news"),
    (_gnews("Казахстан ювелирные украшения монеты слитки НБРК",  "KZ"),   "google.news"),
    (_gnews("Казахстан металлургия горнодобыча инновации инвестиции","KZ"),"google.news"),

    # ── Цены и рынок: мировой контекст (на русском) ──
    (_gnews("золото серебро Казахстан Нацбанк НБРК", "KZ"), "google.news"),
    (_gnews("цена золота серебра рынок драгметаллы"),       "google.news"),
    (_gnews("платина палладий цена рынок"),                 "google.news"),
    (_gnews("центробанки скупают золото резервы рекорд"),   "google.news"),

    # ── Ювелирные украшения / дома моды (на русском) ──
    (_gnews("ювелирные украшения коллекция Cartier Bvlgari Tiffany"), "google.news"),
    (_gnews("ювелирный бренд новинка кольцо колье серьги тренд"),     "google.news"),

    # ── Люксовые часы (на русском) ──
    (_gnews("Rolex Patek Philippe Audemars Piguet часы аукцион новинка"), "google.news"),

    # ── Драгоценные камни / аукционы (на русском) ──
    (_gnews("бриллиант алмаз рекорд аукцион Sotheby's Christie's"), "google.news"),
    (_gnews("изумруд сапфир рубин редкий камень продан"),          "google.news"),

    # ── Монеты и слитки (на русском + KZ) ──
    (_gnews("инвестиционные монеты слитки золото монетный двор", "KZ"), "google.news"),
    (_gnews("Royal Mint Perth Mint новая золотая монета"),             "google.news"),

    # ── Лом, скупка, аффинаж ──
    (_gnews("скупка лома золота аффинаж переработка драгметаллов"), "google.news"),

    # ── Международный контент на английском (заголовки переводятся автоматически) ──
    # Google News: мировые цены и рынок металлов
    (_gnews_en("gold price XAU record market forecast"),               "google.news"),
    (_gnews_en("silver platinum palladium price market"),              "google.news"),
    # Google News: аукционы и ювелирные бренды
    (_gnews_en("luxury jewelry auction Sotheby Christie record sale"), "google.news"),
    (_gnews_en("Rolex Patek Audemars watch auction record"),           "google.news"),
    (_gnews_en("diamond gemstone rare record auction"),                "google.news"),
    # Прямые ленты: специализированные англоязычные источники
    ("https://www.kitco.com/rss/kitconewsrss.xml", "kitco.com"),
    ("https://www.mining.com/feed/",               "mining.com"),

    # ── Медь и промышленные металлы ──
    (_gnews_en("copper price LME market outlook"),               "google.news"),
    (_gnews_en("copper nickel zinc aluminium base metals market"), "google.news"),
    (_gnews("медь цена LME рынок Казахстан", "KZ"),             "google.news"),

    # ── Часы: специализированный контент ──
    (_gnews_en("luxury watch new release collection 2025 2026"), "google.news"),
    (_gnews_en("watch auction Sotheby Christie Antiquorum record"), "google.news"),

    # ── Камни: специализированный контент ──
    (_gnews_en("diamond market price Rapaport index report"),    "google.news"),
    (_gnews_en("colored gemstone auction emerald ruby sapphire record"), "google.news"),

    # ── Русскоязычные финансовые ──
        ...
        for feed_url, feed_domain in _RSS_FEEDS:
            try:
                ...
                for item in root.findall(".//item"):
                    ...
                    # Нерусский заголовок: переводим если источник известный
                    # международный, иначе пропускаем.
                    if not _is_russian(title):
                        if real_domain in _INTL_SOURCES:
...
async def _groq_filter(articles: list) -> list:
    ...
    prompt = (
        "Ты — редактор Telegram-канала о рынке драгоценных металлов в Казахстане.\n"
        "Рубрики: украшения, часы, камни (бриллианты), монеты, слитки, "
        "лом и скупка, цены на золото/серебро/платину, медь и другие металлы.\n\n"
        "ЗАДАЧА: выбрать 5-7 заголовков для дайджеста.\n\n"
        "ГЛАВНОЕ ПРАВИЛО — РАЗНООБРАЗИЕ. Лента не должна быть «только золото».\n"
        "1. Максимум 2 новости про цену золота. Остальное — другие рубрики.\n"
        "   Одно ценовое движение = одна новость. 'Золото упало' и 'Серебро подешевело'\n"
        "   в один день — один сюжет, оставь только лучший заголовок.\n"
        "2. Несколько заголовков об одном и том же — оставь только один лучший.\n"
        "3. Бери интересное обычному читателю: рекорды и аукционы, новые "
        "коллекции брендов, редкие камни и часы, монеты и слитки Нацбанка, "
        "необычные факты рынка — это живее, чем «золото подорожало на 1%».\n"
        "4. Скучные дежурные сводки «обновление цены золота» — НЕ брать.\n"
        "5. География: Казахстан > мир > Россия.\n"
        "6. Убери мусор: кражи, контрабанду, спорт, политику.\n\n"
        f"Заголовки:\n{titles_text}\n\n"
...
async def _do_fetch_and_post(bot) -> list:
    ...
    _NEWS_RUNNING = True
    try:
        return await _do_fetch_and_post_inner(bot)
    finally:
        _NEWS_RUNNING = False


async def _do_fetch_and_post_inner(bot) -> list:
    ...
    articles = _balance_categories(articles, max_total=5, gold_cap=2)
...
def main():
    ...
    if NEWS_TOPIC_ID and GROUP_ID and app.job_queue is not None:
        app.job_queue.run_repeating(daily_news_job, interval=7200, first=300)
