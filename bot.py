...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_safety":
        await context.bot.send_message(
            ...
            text=(
                ...
                "Также жалобу можно подать прямо из сообщения «Контакты продавца», "
                "которое приходит сразу после открытия сделки.\n\n"
                f"По срочным вопросам — пишите админу: @{ADMIN_USERNAME}"
            ),
            parse_mode="HTML",
            reply_markup=main_menu_keyboard(),
        )
    ...
    if data.startswith("buy_confirm_"):
        ...
        complaint_kb = InlineKeyboardMarkup([[InlineKeyboardButton("🚨 Пожаловаться на продавца", callback_data=f"complaint_start_{seller_id}_{ad_id}_{deal_id or 0}")]])
        await context.bot.send_message(
            chat_id=int(user_id),
            text=f"✅ Контакты продавца:\n{seller.get('first_name','')} {seller.get('last_name','')}\n@{seller.get('username','')}{seller_badge}\n\nСвяжитесь между собой 🤝",
            reply_markup=complaint_kb,
        )
        await context.bot.send_message(
