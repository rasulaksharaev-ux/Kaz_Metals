            ...
            for feed_url, domain in RSS_FEEDS:
                ...
                try:
                    ...
                    for item in root.findall(".//item"):
                        ...
                        # 2. Стоп-слова (криминал, санкции и т.д.)
                        ...
                        if any(is_similar_title(title, p) for p in published_titles):
                            logger.info(f"Пропуск (дубль БД): {title[:60]}")
                            continue

                        # 5. Не дублировать в текущей выборке
                        if any(is_similar_title(title, f["title"]) for f in found):
                            continue
                        ...
                        # Сразу сохраняем в БД — защита от двойного запуска
                        save_published_news(title, domain)
...
async def post_news_to_group(bot, articles: list) -> None:
    """Публикует переданный список новостей в группу."""
...
async def publish_news_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    articles = await fetch_metal_news()   # единственный вызов
    if not articles:
        await update.message.reply_text(
            "⚠️ Свежих новостей не найдено.\n"
            "Возможно всё уже публиковалось или RSS-сайты недоступны."
        )
        return
    await post_news_to_group(context.bot, articles)
