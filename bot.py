...
_RSS_FEEDS = [
    ...
    # INVESTING.COM - твой основной источник
    ("https://ru.investing.com/rss/news_14.rss",       "investing.com"),  # commodities
    ("https://ru.investing.com/rss/news_285.rss",      "investing.com"),  # metals

    # GOOGLE NEWS KZ - Казахстан + мир на русском
    (
    ...
    # СПЕЦИАЛИЗИРОВАННЫЕ - только металлы
...
async def _fetch_prices_for_pin() -> dict:
    """Получает цены металлов через Yahoo Finance (тот же метод что и алерты)."""
    metals = [
            ...
            for symbol, label in metals:
                try:
                    ...
                    async with session.get(
                        ...
                        if r.status != 200:
                            logger.error(f"Yahoo {symbol}: HTTP {r.status}")
                            continue
                        data = await r.json()

                    res = data.get("chart", {}).get("result")
                    if not res:
                        logger.error(f"Yahoo {symbol}: пустой result")
                        continue

                    closes = [
                        c for c in
                        res[0].get("indicators", {}).get("quote", [{}])[0].get("close", [])
                        if c is not None
                    ]
                    if len(closes) < 2:
                        logger.error(f"Yahoo {symbol}: closes={closes}")
                        continue
                    ...
                    logger.info(f"Цена {symbol}: ${curr:.2f} ({sign}{change:.2f}%)")

                except Exception as e:
                    logger.error(f"Price {symbol}: {e}")
    except Exception as e:
        logger.error(f"_fetch_prices_for_pin session: {e}")
    return result


async def post_pinned_prices(bot) -> str:
    """
    Публикует цены и закрепляет. Возвращает строку с результатом для отчёта.
    """
    ...
    if not prices:
        return "❌ Не удалось получить цены с Yahoo Finance"
    ...
    if old_id:
        try:
            ...
            logger.info(f"Удалено старое сообщение {old_id}")
        except Exception as e:
    ...
    try:
        ...
        return f"✅ Закреплено {len(prices)} металлов"
    except Exception as e:
        # Сообщение отправлено но не закреплено
        set_setting(_PRICES_PIN_KEY, str(msg.message_id))
        return f"⚠️ Сообщение отправлено, но закрепить не вышло: {e}"
