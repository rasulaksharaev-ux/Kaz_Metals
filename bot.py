...
async def get_realtime_prices() -> Tuple[Dict[str, Tuple[str, int, str]], int]:
    """Реальные котировки с Yahoo Finance (обновляются каждые ~15 мин).
    Использует XAUUSD=X и аналоги — спот-цены в USD/oz → KZT/г по курсу НБРК."""
    yahoo_symbols = {
        "XAU": ("XAUUSD=X", "🥇 Золото"),
        "XAG": ("XAGUSD=X", "🥈 Серебро"),
        "XPT": ("XPTUSD=X", "💎 Платина"),
        "XPD": ("XPDUSD=X", "🔩 Палладий"),
    }
    results: Dict[str, Tuple[str, int, str]] = {}
    async with aiohttp.ClientSession() as session:
        usd_kzt = await fetch_usd_kzt(session)
        if not usd_kzt:
            return results, 0
        for sym_key, (yahoo_sym, name) in yahoo_symbols.items():
            try:
                ...
                if not closes:
                    continue
                price_usd_oz = closes[-1]
                price_kzt_g  = round((price_usd_oz / 31.1035) * usd_kzt)
                results[sym_key] = (name, price_kzt_g, "г")
            except Exception as e:
                logger.error(f"realtime_price {sym_key}: {e}")
    return results, round(usd_kzt)
