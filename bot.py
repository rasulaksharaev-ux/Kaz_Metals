...
def extract_domain(url: str) -> str:
    """Извлекает домен из URL, например https://www.rbc.ru/... → rbc.ru"""
    try:
...
# Только строгие ключевые слова по теме драгметаллов и ювелирки
NEWS_KEYWORDS_PRIMARY = [
    "золото", "золот", "серебро", "серебр",
    "платина", "платин", "палладий",
    "драгоценн", "драгметалл",
    "ювелир", "ювелирн",
    "кольцо", "кольца", "колец",
    "серьги", "серьга", "серёжк",
    "браслет", "подвеска", "подвески",
    "украшени",
    "слиток", "слитк",
    "аффинаж", "аффинированн",
    "пробирн", "проба золота", "проба серебра",
    "лом золота", "лом серебра", "скупка золота",
    "монета золот", "монеты золот",
    "нбрк золото", "нацбанк золото",
]

# Только русскоязычные RSS-источники
RSS_FEEDS = [
    # ===== КАЗАХСТАН =====
...
def is_russian(text: str) -> bool:
    """Проверяет что текст на русском — минимум 50% кириллических символов."""
    if not text:
        return False
...
def title_words(title: str) -> set:
    """Извлекает значимые слова из заголовка для сравнения схожести."""
    import re as _re
    title = title.lower()
    words = _re.findall(r'[а-яёa-z]{4,}', title)
    # Убираем стоп-слова
    stopwords = {"что", "это", "как", "для", "при", "over", "with", "that", "from",
                 "будет", "после", "также", "свои", "своих", "этом", "them", "their",
                 "млрд", "млн", "трлн", "году", "года", "рублей", "рублях"}
    return {w for w in words if w not in stopwords}


def is_similar_title(title1: str, title2: str, threshold: float = 0.45) -> bool:
    """Jaccard-схожесть заголовков — если > threshold, считаем одной новостью."""
    w1 = title_words(title1)
    w2 = title_words(title2)
    if not w1 or not w2:
        return False
...
def get_published_titles() -> list:
    """Берёт заголовки новостей опубликованных за последние 7 дней."""
    try:
        rows = db_fetchall(
...
def save_published_news(title: str, domain: str) -> None:
    """Сохраняет опубликованную новость в БД."""
    import hashlib
...
def get_news_emoji(title: str) -> str:
    ...
    if any(w in t for w in ["серебро", "серебр"]):
        return "🥈"
    if any(w in t for w in ["платина", "платин"]):
        return "💎"
    if any(w in t for w in ["палладий"]):
        return "🔩"
    if any(w in t for w in ["ювелир", "кольц", "серьг", "брасле", "подвеск", "украшен"]):
        return "💍"
    if any(w in t for w in ["слиток", "аффинаж"]):
        return "🏅"
    if any(w in t for w in ["монет"]):
        return "🪙"
    if any(w in t for w in ["лом", "скупка"]):
        return "♻️"
    if any(w in t for w in ["нбрк", "нацбанк", "казахстан"]):
        return "🇰🇿"
    return "📊"


async def fetch_metal_news() -> list:
    """Парсит RSS, фильтрует: только русский язык, только тема металлов, без дублей."""
    found = []
    seen_titles = set()
    seen_domains = {}
    # Берём уже опубликованные заголовки для проверки дублей
    ...
    try:
            ...
            for feed_url, domain in RSS_FEEDS:
                ...
                try:
                    ...
                    for item in items:
                        ...
                        # 2. Только по теме металлов/ювелирки
                        title_lower = title.lower()
                        if not any(kw in title_lower for kw in NEWS_KEYWORDS_PRIMARY):
                            continue

                        # 3. Не дублировать уже опубликованное (по смыслу)
                        already_published = any(
                            is_similar_title(title, pub) for pub in published_titles
                        )
                        if already_published:
                            logger.info(f"Дубль пропущен: {title[:60]}")
                            continue

                        # 4. Не дублировать в текущей выборке
                        is_dup_in_batch = any(
                            is_similar_title(title, f["title"]) for f in found
                        )
                        if is_dup_in_batch:
                            continue
        ...
        logger.error(f"fetch_metal_news session error: {e}")

    logger.info(f"fetch_metal_news: найдено {len(found)} уникальных новостей")
    return found


async def post_news_to_group(bot) -> None:
    """Публикует новости во вкладку Новости группы и сохраняет в БД."""
    ...
    if not articles:
        logger.warning("post_news_to_group: новостей не найдено")
        return

    for article in articles:
        text = f"{article['emoji']} {article['title']}\n\n{article['domain']}"
        try:
            ...
            logger.error(f"post_news_to_group send error: {e}")

async def daily_news_job(context: ContextTypes.DEFAULT_TYPE):
    await post_news_to_group(context.bot)


async def publish_news_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Команда /publish_news — ручная публикация новостей (только для админа)."""
    if update.message.chat.type != "private":
        return
    user_id = str(update.message.from_user.id)
    if not is_admin(user_id):
        ...
        return
    await update.message.reply_text("⏳ Ищу новости по теме металлов/ювелирки...")
    articles = await fetch_metal_news()
    if not articles:
        await update.message.reply_text(
            "⚠️ Новостей не найдено.\n"
            "Возможные причины: RSS-сайты недоступны или нет новых статей по теме."
        )
        return
    await post_news_to_group(context.bot)
    titles = "\n".join([f"• {a['title'][:60]}..." for a in articles])
    await update.message.reply_text(
        f"✅ Опубликовано {len(articles)} новостей:\n\n{titles}"
    )

async def publish_prices(bot):
    # Цены публикуются только в боте через кнопку "📊 Цены на металлы"
    # В группу цены больше не публикуются
    logger.info("publish_prices: group publishing disabled, prices available in bot only")
