...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Handle deep link buy_SELLERID
    if context.args and context.args[0].startswith("buy_"):
        seller_id = context.args[0].replace("buy_", "")
    ...
    if data == "ad_confirm":
        ...
        try:
            ...
            buy_markup = InlineKeyboardMarkup([[
                InlineKeyboardButton("💬 Хочу купить", url=f"https://t.me/KAZ_METALSbot?start=buy_{user_id}")
            ]])
            if u.get("photo_id"):
...
async def publish_prices(bot):
    ...
    if prices:
        ...
        for symbol, (name, price, unit) in prices.items():
            text += f"{name}: {price:,} KZT/{unit}\n"
        text += "\nЦены за 1 грамм | Обновляется ежедневно в 9:00"
        try:
