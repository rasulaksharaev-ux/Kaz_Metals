...
async def get_realtime_prices():
    """Котировки с goldapi.io. Возвращает
    ({sym: (name, price_kzt, open_kzt, chp, unit)}, usd_kzt).
    Кэш 120 сек чтобы не упираться в лимит goldapi."""
    ...
    results = {}
    if not GOLDAPI_KEY:
        logger.error("GOLDAPI_KEY не задан")
        return results, 0
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for sym, name in metal_names.items():
            try:
                url = f"https://www.goldapi.io/api/{sym}/USD"
                async with session.get(
                    url, headers=headers, timeout=aiohttp.ClientTimeout(total=10)
                ) as r:
                    if r.status != 200:
                        logger.error(f"goldapi {sym}: {r.status}")
                        continue
                    data = await r.json()
                price_usd = float(data.get("price") or 0)
                open_usd  = float(data.get("open_price") or 0)
                chp = data.get("chp")
                if not price_usd:
                    continue
...
async def test_alert_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not prices:
        await update.message.reply_text(
            "❌ goldapi.io не вернул данные.\n\n"
            "Возможные причины:\n"
            "• GOLDAPI_KEY не задан или невалидный\n"
            "• Превышен месячный лимит запросов\n"
            "• Сетевой блок на Railway\n\n"
