...
_RSS_FEEDS = [
    ...
    ("https://dknews.kz/rss/",                               "dknews.kz"),
    # KZ люкс, мода, часы, ювелирка
    ("https://marieclaire.kz/feed/",                         "marieclaire.kz"),
    ("https://elle.kz/feed/",                                "elle.kz"),
    ("https://vogue.kz/feed/",                               "vogue.kz"),
    ("https://the-steppe.com/feed/",                         "the-steppe.com"),
...
_KW_TOPICS = [
    # Металлы (Новости, Слитки)
    "золото", "золот", "серебро", "серебр", "платина", "платин",
    ...
    "жемчуг", "янтарь", "драгоценный камень", "драгоценные камни",
    # Часы
...
_KW_STOP = [
    ...
    # Спорт (золото как чемпионство, не металл)
...
import re as _re
_STOP_PATTERNS = [
    _re.compile(r'(брилли|алмаз|золот)\w*\s+в\s+(кармане|джинс|штан|носк|кулак)', _re.I),
...
def _is_relevant(title: str) -> bool:
    """Базовый фильтр: есть ли тема группы + нет ли явного криминала."""
    tl = title.lower()
        ...
        for feed_url, feed_domain in _RSS_FEEDS:
            try:
                ...
                for item in root.findall(".//item"):
                    ...
                    # Переводим на русский если заголовок на другом языке
                    ...
                    tl = title.lower()
                    # Единая проверка релевантности (двухуровневая)
                    if not _is_relevant(title):
                        continue

                    # KZ-новости - только из KZ источников
                    ...
                    if h in seen or h in _SENT_HASHES:
                        continue
                    # Не публиковать то что уже было (по смыслу)
                    if any(_same_topic(title, p) for p in _get_published_titles()):
                        continue
                    seen.add(h)
                    raw.append({
                        "title":  title,
...
def _deduplicate(raw: list) -> list:
    """
    1. Фильтрует уже опубликованные
    2. Группирует дубли (одна новость с разных сайтов) > берёт лучший источник
    3. Ограничивает: макс 2 новости из RU-источников (tier 4-5)
    4. Возвращает топ-8 по важности
    """
    published = _get_published_titles()
    filtered = [
        a for a in raw
        if not any(_same_topic(a["title"], p) for p in published)
    ]

    # Группируем похожие статьи
    groups: list = []
    for art in filtered:
        placed = False
        for grp in groups:
            if _same_topic(art["title"], grp[0]["title"]):
                grp.append(art)
                placed = True
                break
        if not placed:
    ...
    # Из каждой группы - лучший источник (меньший tier = KZ/US/EU)
    best = []
    for grp in groups:
    ...
    # Лимит: максимум 2 статьи из tier 4-5 (российские СМИ)
    result = []
    ru_count = 0
    for art in best:
        is_ru = art["tier"] >= 4
...
async def _groq_filter(articles: list) -> list:
    """
    Бесплатный AI-фильтр через Groq API (Llama 3).
    Регистрация: console.groq.com - бесплатно, 14400 запросов/день.
    Если ключа нет - пропускает все статьи.
    """
    ...
    prompt = (
        "You are an editor of KAZ_METALS - a Telegram channel about precious metals and luxury goods in Kazakhstan.\n\n"
        "The channel has these sections:\n"
        "- Новости (market news: prices, trends)\n"
        "- Серьги (earrings: new collections, brands, trends)\n"
        "- Кольца (rings: new collections, engagement, luxury)\n"
        "- Подвеска/Браслет (pendants, bracelets)\n"
        "- Монеты (investment coins, National Bank of Kazakhstan coins)\n"
        "- Exclusive (exclusive/rare items, record sales)\n"
        "- Часы (luxury watches: Rolex, Patek, Cartier, Audemars etc)\n"
        "- Камни (gemstones: diamonds, rubies, emeralds - market news)\n"
        "- Слитки (bullion bars, refinery news)\n"
        "- Лом и скупка (scrap gold/silver buyback market)\n\n"
        "PRIORITY ORDER for news: Kazakhstan > USA/Europe > other countries\n\n"
        "KEEP if relevant to any section above:\n"
        "- Market prices, trends, records for gold/silver/platinum/palladium\n"
        "- Luxury watch news (auctions, new releases, record sales)\n"
        "- Jewelry market news (trends, major brands, collections)\n"
        "- Gemstone auctions and record sales (even unusual finds IF sold at major auction)\n"
        "- National Bank Kazakhstan coins, investment coins\n"
        "- Bullion market, scrap gold prices\n"
        "- EXCEPTIONAL stories: record-breaking sales, historic discoveries sold at auction\n\n"
        "REMOVE:\n"
        "- Smuggling/customs seizures\n"
        "- Random finds with no market value (person found small diamond in park)\n"
        "- War/politics unrelated to metal prices\n"
        "- Generic investment advice without news event\n"
        "- Russian domestic stories with no international relevance\n\n"
        f"Headlines:\n{titles_text}\n\n"
        "Reply ONLY with JSON array of numbers to KEEP. Example: [1,3,5]\n"
        "No explanations."
    )
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                ...
                if r.status != 200:
                    logger.error(f"Groq API status {r.status}")
                    return articles
                data = await r.json()
                text = data["choices"][0]["message"]["content"].strip()
                import json as _json, re as _re2
                m = _re2.search(r'\[[\d,\s]+\]', text)
                if m:
                    approved = _json.loads(m.group())
                    result = [articles[i-1] for i in approved if 1 <= i <= len(articles)]
                    logger.info(f"Groq filter: {len(articles)} > {len(result)}")
                    return result
    except Exception as e:
        logger.error(f"Groq filter error: {e}")
