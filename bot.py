...
_RSS_FEEDS = [
    ...
    # ══ GOOGLE NEWS - KZ (агрегирует лучшие KZ и мировые источники) ══
    (
        "https://news.google.com/rss/search"
        "?q=%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%BE+%D1%81%D0%B5%D1%80%D0%B5%D0%B1%D1%80%D0%BE"
        "+%D0%9A%D0%B0%D0%B7%D0%B0%D1%85%D1%81%D1%82%D0%B0%D0%BD+%D0%9D%D0%91%D0%A0%D0%9A"
        "&hl=ru&gl=KZ&ceid=KZ:ru",
        "google.news"
    ),
    (
        "https://news.google.com/rss/search"
        "?q=%D1%8E%D0%B2%D0%B5%D0%BB%D0%B8%D1%80%D0%BD%D1%8B%D0%B5+%D1%87%D0%B0%D1%81%D1%8B"
        "+%D0%B4%D1%80%D0%B0%D0%B3%D0%BE%D1%86%D0%B5%D0%BD%D0%BD%D1%8B%D0%B5+%D0%BA%D0%B0%D0%BC%D0%BD%D0%B8"
        "&hl=ru&gl=KZ&ceid=KZ:ru",
        "google.news"
    ),

    # ══ GOOGLE NEWS - МИРОВОЙ (США, Европа, Азия) ══
    (
        "https://news.google.com/rss/search"
        "?q=%D0%B7%D0%BE%D0%BB%D0%BE%D1%82%D0%BE+%D1%81%D0%B5%D1%80%D0%B5%D0%B1%D1%80%D0%BE"
        "+%D1%86%D0%B5%D0%BD%D0%B0+%D1%80%D1%8B%D0%BD%D0%BE%D0%BA"
        "&hl=ru&gl=RU&ceid=RU:ru",
        "google.news"
    ),
    (
        "https://news.google.com/rss/search"
        "?q=rolex+patek+audemars+cartier+%D0%B0%D1%83%D0%BA%D1%86%D0%B8%D0%BE%D0%BD"
        "&hl=ru&gl=RU&ceid=RU:ru",
        "google.news"
    ),

    # ══ ЯНДЕКС НОВОСТИ - только релевантные запросы ══
    (
    ...
    # ══ РОССИЯ - только 3 лучших финансовых источника ══
...
async def _fetch_prices_for_pin() -> dict:
    metals = [
        ("XAUUSD=X", "🥇 Золото (XAU/USD)"),
        ("XAGUSD=X", "🥈 Серебро (XAG/USD)"),
        ("XPTUSD=X", "💎 Платина (XPT/USD)"),
        ("XPDUSD=X", "🔩 Палладий (XPD/USD)"),
    ]
    result = {}
    try:
        async with aiohttp.ClientSession() as session:
            for symbol, label in metals:
                try:
                    ...
                    async with session.get(url, timeout=aiohttp.ClientTimeout(total=10),
                        ...
                        if r.status != 200:
                            continue
                        data = await r.json()
                    closes = [c for c in data.get("chart",{}).get("result",[{}])[0]
                              .get("indicators",{}).get("quote",[{}])[0].get("close",[]) if c]
                    if len(closes) < 2:
                        continue
                    ...
                    result[label] = f"${curr:,.2f}  {arrow} {sign}{change:.2f}%"
                except Exception as e:
...
async def post_pinned_prices(bot) -> None:
    """Публикует цены, закрепляет. Удаляет старое закрепление."""
    if not NEWS_TOPIC_ID or not GROUP_ID:
        return
    prices = await _fetch_prices_for_pin()
    if not prices:
        return
    now_str = datetime.now().strftime("%d.%m.%Y %H:%M")
    text = f"📊 <b>Цены на драгоценные металлы</b>\n🕐 Обновлено: {now_str}\n\n"
    text += "\n".join(f"{lbl}: <b>{val}</b>" for lbl, val in prices.items())
    ...
    try:
        ...
        logger.info(f"Цены закреплены msg_id={msg.message_id}")
    except Exception as e:
        logger.error(f"post_pinned_prices: {e}")


async def pinned_prices_job(context: ContextTypes.DEFAULT_TYPE):
    await post_pinned_prices(context.bot)
...
async def pin_prices_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await post_pinned_prices(context.bot)
    await update.message.reply_text("✅ Цены закреплены!")
...
async def _do_fetch_and_post(bot) -> list:
    ...
    async with _NEWS_LOCK:
        ...
        for art in articles:
            ...
            # Вариант 10: ▪️ заголовок + описание + домен
            desc = art.get("desc", "")
            if desc:
                text = (
                    f"▪️ {art['title']}\n\n"
