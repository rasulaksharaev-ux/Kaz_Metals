...
def admin_menu_keyboard() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("📢 Объявления на модерации", callback_data="adm_moderation")],
        ...
        [InlineKeyboardButton("✏️ Отправить на доработку", callback_data="adm_revision_list")],
        [InlineKeyboardButton("🗑 Удалить объявление", callback_data="adm_delete_list")],
        [InlineKeyboardButton("🤝 Спорные сделки", callback_data="adm_disputes")],
    ])
...
def db_execute(query: str, params: tuple = ()) -> int:
    try:
        ...
        logger.error(f"db_execute error: {e}")
        return 0
...
def admin_user_title(user: dict) -> str:
    ...
    return f"{name} | @{username} | {status}"


def admin_ad_title(ad: dict) -> str:
    return f"№{ad.get('ad_id')} | {ad.get('category','')} | {ad.get('price','')} KZT"


async def send_admin_menu(chat_id: int, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(chat_id=chat_id, text="🔐 Админ-панель KAZMETALS", reply_markup=admin_menu_keyboard())
...
async def admin_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_admin(str(update.message.from_user.id)):
        ...
        return
    await update.message.reply_text("🔐 Админ-панель KAZMETALS", reply_markup=admin_menu_keyboard())


async def delete_ad_messages(context: ContextTypes.DEFAULT_TYPE, ad: dict):
    try:
        if ad.get("message_id"):
            await context.bot.delete_message(chat_id=GROUP_ID, message_id=int(ad.get("message_id")))
    except Exception as e:
        logger.warning(f"admin delete group message skipped: {e}")
    try:
        ...
        logger.warning(f"admin delete media message skipped: {e}")
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("adm_") and not is_admin(user_id):
        await context.bot.send_message(chat_id=chat_id, text="⛔ Этот раздел доступен только владельцу.")
        return
    ...
    if data == "adm_stats":
        ...
        await context.bot.send_message(
            ...
            text=(
                "📊 Статистика\n\n"
                f"👥 Пользователи: {users_count.get('c', 0)}\n"
                f"📦 Активные объявления: {active_ads.get('c', 0)}\n"
                f"🤝 Сделки: {deals_count.get('c', 0)}\n"
                f"⚠️ Жалобы: {complaints_count.get('c', 0)}\n"
                f"🚫 Заблокированные: {blocked_count.get('c', 0)}"
            ),
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]),
        )
        return

    if data == "adm_users":
        users_list = db_fetchall("SELECT * FROM users ORDER BY registered_at DESC NULLS LAST LIMIT 20")
        if not users_list:
            await context.bot.send_message(chat_id=chat_id, text="Пользователей пока нет.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]))
            return
        rows = [[InlineKeyboardButton(admin_user_title(u)[:60], callback_data=f"adm_user_{u.get('user_id')}")] for u in users_list]
        rows.append([InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")])
        await context.bot.send_message(chat_id=chat_id, text="👥 Пользователи:", reply_markup=InlineKeyboardMarkup(rows))
        return

    if data.startswith("adm_user_"):
        target_id = data.replace("adm_user_", "")
        ...
        await context.bot.send_message(
            ...
            text=(
                f"👤 Пользователь\n\n"
                f"ID: {target_id}\n"
                f"Имя: {target.get('first_name','')} {target.get('last_name','')}\n"
                f"Username: @{target.get('username') or 'без username'}\n"
                f"Телефон: {target.get('phone','')}\n"
                f"Статус: {target.get('status','active')}\n"
                f"В группе: {target.get('joined','no')}\n"
                f"Рейтинг: {target.get('rating', 0)} | Отзывов: {target.get('reviews_count', 0)}"
            ),
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✅ Верифицировать", callback_data=f"adm_verify_{target_id}")],
                [InlineKeyboardButton("🚫 Заблокировать", callback_data=f"adm_block_{target_id}")],
                [InlineKeyboardButton("🔓 Разблокировать", callback_data=f"adm_unblock_{target_id}")],
                [InlineKeyboardButton("🤝 Сделки пользователя", callback_data=f"adm_deals_{target_id}")],
                [InlineKeyboardButton("🔙 Пользователи", callback_data="adm_users")],
            ]),
        )
        return
    ...
    if data.startswith("adm_verify_"):
        target_id = data.replace("adm_verify_", "")
        db_execute("UPDATE users SET status = 'active', joined = 'yes' WHERE user_id = %s", (target_id,))
        await context.bot.send_message(chat_id=chat_id, text="✅ Пользователь верифицирован.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Пользователи", callback_data="adm_users")]]))
        try:
            await context.bot.send_message(chat_id=int(target_id), text="✅ Ваш профиль верифицирован администратором.")
        except Exception:
            pass
        return
    ...
    if data.startswith("adm_block_"):
        ...
        db_execute("UPDATE users SET status = 'blocked' WHERE user_id = %s", (target_id,))
        await context.bot.send_message(chat_id=chat_id, text="🚫 Пользователь заблокирован.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Пользователи", callback_data="adm_users")]]))
        try:
            await context.bot.send_message(chat_id=int(target_id), text=f"🚫 Ваш доступ ограничен. По вопросам доступа напишите @{ADMIN_USERNAME}.")
        except Exception:
            pass
        return
    ...
    if data.startswith("adm_unblock_"):
        ...
        db_execute("UPDATE users SET status = 'active' WHERE user_id = %s", (target_id,))
        await context.bot.send_message(chat_id=chat_id, text="🔓 Пользователь разблокирован.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Пользователи", callback_data="adm_users")]]))
        try:
            await context.bot.send_message(chat_id=int(target_id), text="🔓 Ваш доступ восстановлен.")
        except Exception:
            pass
        return

    if data == "adm_blocked":
        blocked = db_fetchall("SELECT * FROM users WHERE status = 'blocked' ORDER BY registered_at DESC NULLS LAST LIMIT 20")
        if not blocked:
            await context.bot.send_message(chat_id=chat_id, text="🚫 Заблокированных пользователей нет.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]))
            return
        rows = [[InlineKeyboardButton(admin_user_title(u)[:60], callback_data=f"adm_user_{u.get('user_id')}")] for u in blocked]
        rows.append([InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")])
        await context.bot.send_message(chat_id=chat_id, text="🚫 Заблокированные:", reply_markup=InlineKeyboardMarkup(rows))
        return

    if data == "adm_moderation":
        ads = db_fetchall("SELECT * FROM ads WHERE status IN ('pending', 'moderation') ORDER BY ad_id DESC LIMIT 20")
        if not ads:
            await context.bot.send_message(chat_id=chat_id, text="📢 Объявлений на модерации нет.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]))
            return
        rows = [[InlineKeyboardButton(admin_ad_title(ad)[:60], callback_data=f"adm_ad_{ad.get('ad_id')}")] for ad in ads]
        rows.append([InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")])
        await context.bot.send_message(chat_id=chat_id, text="📢 Объявления на модерации:", reply_markup=InlineKeyboardMarkup(rows))
        return

    if data == "adm_revision_list":
        ads = db_fetchall("SELECT * FROM ads WHERE status = 'active' ORDER BY ad_id DESC LIMIT 20")
        if not ads:
            await context.bot.send_message(chat_id=chat_id, text="Активных объявлений нет.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]))
            return
        rows = [[InlineKeyboardButton(admin_ad_title(ad)[:60], callback_data=f"adm_revision_{ad.get('ad_id')}")] for ad in ads]
        rows.append([InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")])
        await context.bot.send_message(chat_id=chat_id, text="✏️ Выберите объявление для отправки на доработку:", reply_markup=InlineKeyboardMarkup(rows))
        return

    if data == "adm_delete_list":
        ads = db_fetchall("SELECT * FROM ads WHERE status = 'active' ORDER BY ad_id DESC LIMIT 20")
        if not ads:
            await context.bot.send_message(chat_id=chat_id, text="Активных объявлений нет.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]))
            return
        rows = [[InlineKeyboardButton(admin_ad_title(ad)[:60], callback_data=f"adm_delete_{ad.get('ad_id')}")] for ad in ads]
        rows.append([InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")])
        await context.bot.send_message(chat_id=chat_id, text="🗑 Выберите объявление для удаления:", reply_markup=InlineKeyboardMarkup(rows))
        return

    if data.startswith("adm_ad_"):
        ad_id = data.replace("adm_ad_", "")
        ad = db_fetchone("SELECT * FROM ads WHERE ad_id = %s", (int(ad_id),)) if ad_id.isdigit() else None
        if not ad:
        ...
        await context.bot.send_message(
            ...
            text=(
                ...
                f"Продавец ID: {ad.get('user_id','')}\n"
                f"Цена: {ad.get('price','')} KZT\n"
                f"Город: {ad.get('city','')}\n\n"
                f"{ad.get('description','')}"
            ),
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("✏️ На доработку", callback_data=f"adm_revision_{ad_id}")],
                [InlineKeyboardButton("🗑 Удалить", callback_data=f"adm_delete_{ad_id}")],
                [InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")],
            ]),
        )
        return

    if data.startswith("adm_revision_"):
        ad_id = data.replace("adm_revision_", "")
        ad = db_fetchone("SELECT * FROM ads WHERE ad_id = %s", (int(ad_id),)) if ad_id.isdigit() else None
        if not ad:
            await context.bot.send_message(chat_id=chat_id, text="Объявление не найдено.")
            return
        db_execute("UPDATE ads SET status = 'revision' WHERE ad_id = %s", (int(ad_id),))
        await delete_ad_messages(context, ad)
        await context.bot.send_message(chat_id=chat_id, text=f"✏️ Объявление №{ad_id} отправлено на доработку.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]))
        try:
            await context.bot.send_message(chat_id=int(ad.get("user_id")), text=f"✏️ Ваше объявление №{ad_id} отправлено администратором на доработку. Пожалуйста, создайте объявление заново с уточнённой информацией.")
        except Exception:
            pass
        return

    if data.startswith("adm_delete_"):
        ad_id = data.replace("adm_delete_", "")
        ad = db_fetchone("SELECT * FROM ads WHERE ad_id = %s", (int(ad_id),)) if ad_id.isdigit() else None
        if not ad:
            await context.bot.send_message(chat_id=chat_id, text="Объявление не найдено.")
            return
        await delete_ad_messages(context, ad)
        db_execute("UPDATE ads SET status = 'deleted' WHERE ad_id = %s", (int(ad_id),))
        await context.bot.send_message(chat_id=chat_id, text=f"🗑 Объявление №{ad_id} удалено.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]))
        try:
            await context.bot.send_message(chat_id=int(ad.get("user_id")), text=f"🗑 Ваше объявление №{ad_id} удалено администратором.")
        except Exception:
            pass
        return
    ...
    if data == "adm_complaints":
        ...
        if not complaints:
            await context.bot.send_message(chat_id=chat_id, text="⚠️ Жалоб пока нет.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]))
            return
        text = "⚠️ Последние жалобы:\n\n"
        for c in complaints[:10]:
            text += f"№{c.get('complaint_id')} | объявление №{c.get('ad_id')} | от {c.get('from_user_id')}\nПричина: {c.get('reason','')}\n\n"
        await context.bot.send_message(chat_id=chat_id, text=text[:3900], reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]))
        return

    if data.startswith("adm_deals_"):
        target_id = data.replace("adm_deals_", "")
        deals = db_fetchall("SELECT * FROM deals WHERE seller_id = %s OR buyer_id = %s ORDER BY deal_id DESC LIMIT 20", (target_id, target_id))
        if not deals:
            await context.bot.send_message(chat_id=chat_id, text="У пользователя пока нет сделок.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Пользователи", callback_data="adm_users")]]))
            return
        text = f"🤝 Сделки пользователя {target_id}:\n\n"
        for d in deals[:15]:
            text += f"№{d.get('deal_id')} | объявление №{d.get('ad_id')} | статус: {d.get('status')}\nПродавец: {d.get('seller_id')} | Покупатель: {d.get('buyer_id')}\n\n"
        await context.bot.send_message(chat_id=chat_id, text=text[:3900], reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Пользователи", callback_data="adm_users")]]))
        return

    if data == "adm_disputes":
        deals = db_fetchall("SELECT * FROM deals WHERE status IN ('dispute', 'disputed', 'problem', 'complaint') ORDER BY deal_id DESC LIMIT 20")
        if not deals:
            await context.bot.send_message(chat_id=chat_id, text="🤝 Спорных сделок нет.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🔙 Админ-панель", callback_data="adm_back")]]))
            return
        text = "🤝 Спорные сделки:\n\n"
        for d in deals[:15]:
            text += f"№{d.get('deal_id')} | объявление №{d.get('ad_id')} | статус: {d.get('status')}\nПродавец: {d.get('seller_id')} | Покупатель: {d.get('buyer_id')}\n\n"
    ...
    if data == "menu_ad":
        # Сразу переходим к выбору категории, без лишнего пункта «Продажа».
    ...
    if data.startswith("adtype_"):
        # Оставлено только для совместимости со старыми кнопками, если они где-то остались в чате.
        states[user_id] = {"step": "ad_category", "ad_type": "продажа"}
        await context.bot.send_message(chat_id=chat_id, text="Выберите категорию:", reply_markup=categories_keyboard("adcat"))
        return
